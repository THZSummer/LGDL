...
- 🎯 **滑动指针**：行中央固定指针线；横向滑动（普通滚轮自动转横滑 / 触控板 / Shift+滚轮）停稳后，指针所指的示例自动平滑吸附居中并选中
- 🐛 修复滑动跳过元素：两端 padding ≈ 容器半宽，使**每个元素都能滚到指针中心**（此前第 2/3 个、倒数第 2/3 个元素离行端太近、永远无法被指针指到，滑动时被跳过）；吸附几何抽为纯函数 `computeSnap` 并加回归测试（11 个元素全滚动位置可达）
