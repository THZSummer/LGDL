...
- 📐 **布局疏密与分组留白**：`RANK_SEP` 48→96、`NODE_SEP` 40→80（elkjs/dagre 通用），分层图节点更疏朗，
  **纵向堆叠的分组框之间不再贴死**（分组框头部 30+padding 20 会吃掉原本小的节点间距，RANK_SEP 加大后
  相邻框留出 ~26px 空隙，如 architecture 的「接入层/核心服务/数据层」）；`render` 分组框 `pad` 14→20（+30 头部），
  并新增**分组框边框作为标签障碍**——相邻泳道框不再紧贴（如 ecommerce-flow 的「履约域/售后域」），
  且 `售后入口`/`通知履约` 等边标签不再压住分组框边
