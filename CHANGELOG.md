...
- 📐 **布局疏密与分组留白**：`RANK_SEP` 48→72、`NODE_SEP` 40→80（elkjs/dagre 通用），分层图节点更疏朗；
  `render` 分组框 `pad` 14→20（+30 头部），并新增**分组框边框作为标签障碍**——相邻泳道框不再紧贴（如
  ecommerce-flow 的「履约域/售后域」从紧贴到留 40px 空隙），且 `售后入口`/`通知履约` 等边标签不再压住分组框边
