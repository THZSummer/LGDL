# Directory: .sddu/specs-tree-root/specs-tree-web-cli-plugin-v3-ui/

## 目录简介
web-cli-plugin v3「UI 渐进式披露升级」需求规范 —— 把侧栏默认首屏从「全部能力平铺」（现状口径 80 可点元素 / 4,870 可见...
...
├── tasks.md          # 任务分解（**总览与叶子映射，不承接执行**）：specs-tree-web-cli-plugin-v3-ui
├── specs-tree-v3-1-l0-shell-density/          # 作者两段反馈（「凌乱、没有条理」/「很复杂、很难用，信息密度太高」）的直接对应物是**默认首屏**：
├── specs-tree-v3-2-l1-disclosure-refs/          # 父问题的第三面：**五类高后果信息与普通信息同密度并列、无永不折叠常驻位**（Q-UI-003）。E
├── specs-tree-v3-3-l2-on-demand-views/          # v2 已把**四维连接树 / 122 卡命令档案 / 撤销回执**压进同一屏，v1 已有**审计**
└── specs-tree-v3-4-page-as-input/          # 现状意图表达**通道单一**：唯一入口是 `
```
...
| state.json | 状态文件 | 🟢 tracked [tasked] |
| tasks.md | 任务分解（**总览与叶子映射，不承接执行**）：specs-tree-web-cli-plugin-v3-ui — V3-1（无前置，首个开工） | ✅ 存在 |
