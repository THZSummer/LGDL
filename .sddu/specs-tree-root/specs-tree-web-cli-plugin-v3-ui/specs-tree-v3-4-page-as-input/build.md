...
> **裁决处置注（追加，2026-09-18，作者裁决 V3-VOL-3 选 B）**：R3 上报的「越过已登记的 40% 停工线」（+40.75%）**已由裁决闭合** —— 该 Feature 级 40% 累计停工线被**显式撤销**（266,500 B 基线口径的百分比线在活跃开发期已无参照意义），撤销以显式登记落地（`test/size-baseline.ts#SIDEPANEL_SIZE_RULING_V3_VOL_3` / `SIDEPANEL_FEATURE_CUMULATIVE_STOP_WORK_LINE.enforced=false` + `revokedBy='V3-VOL-3'` / `SIDEPANEL_BASELINE_META.cumulativeStopWorkLineStatus`），**不是静默绕过**；本小节的 +40.75% 历史文本**逐字保留**。
> **保留项（零改动）**：单轮 5% 容差 · 重登记五要素披露 · 相邻两轮 >15% 告警 · `ceiling = floor(当前值 × 1.05)`；反证：恢复该线 ⇒ 当前产物 375,102 B（+40.75%）判定**必然 FAIL**（`test/size-ruling-vol3.test.ts`，同一次 `npm test` 内机器重跑）。**新增义务（硬约束）**：方案 F 重构（聊天流统一承载）落地收口时必须**重定 `sidepanel` 体积基线 + 设绝对值上限**（`PENDING_ABSOLUTE_CAP`，F 立项须带进其 spec 验收）。产物 `content.js` / `pick-layer.js` / `sidepanel.js` **零字节变化**（sha256 前后一致）。
...
| 版本 | 变更说明 | 日期 | 修订人 |
|------|---------|------|--------|
| v1.7 | **裁决 V3-VOL-3 落地（2026-09-18，作者选 B；工程纪律裁决的代码化，`dist/*` 零字节变化）**：撤销「Feature 累计增幅 40% 停工线」（266,500 B 基线口径的百分比线在活跃开发期已无参照意义——R3 后实际 +40.75%，本叶 §16.7 上报的越线风险**由此闭合**）。撤销为**显式登记**（`SIDEPANEL_SIZE_RULING_V3_VOL_3` / `SIDEPANEL_FEATURE_CUMULATIVE_STOP_WORK_LINE.enforced=false + revokedBy` / 活动登记追加撤销、历史文本逐字保留），非静默绕过；**保留**单轮 5% 容差 / 五要素披露 / 相邻两轮 >15% 告警 / `ceiling=floor(当前值×1.05)`（守恒自检见 `test/size-ruling-vol3.test.ts`）；**新增义务（硬约束）**：方案 F 重构收口必须重定基线 + 设**绝对值上限**（`PENDING_ABSOLUTE_CAP`，`resolved=false` 存续 + 防静默删除 + 闭合必须带值）。反证：恢复 40% 线 ⇒ 375,102 B（+40.75%）判定**必然 FAIL**；还原 ⇒ 全绿。门禁严格串行全绿（单测 826 → **832**；density 127 / supersession 14 / l0 164 / l1 103 / l2 71 / journey 167 / insight 116 / binding 192 / hardening 24 / gate-integrity 12 / zero-injection 27 / page-input 102）；三产物 sha256 前后一致。 | 2026-09-18 | SDDU Build Agent |
