...
## 15. 后续轮次回填指针（N-10②，2026-09-16 **收口轮**追加）

> 本节由 **sddu-build 收口轮**追加，用于消除「R1 时点产物」与「R2/收口轮已修复状态」之间的时点差；**上文 §5/§13 的原文逐字保留**（历史不得改写）。

1. **§5 第 11 条 ①（「spec NFR-V3-005 未随裁决更新」）**：该问题已由 **R2 的 I-03** 修复并落盘（`spec.md` 以 `~~旧值~~ → 新值` 显式保留历史）；本轮（收口轮）**未再改动** spec 的该条。
2. **R1 的 F-01**：已由 R2 修复（`binding.mjs` 退出口径 + 元门禁 `test/gate-integrity.test.ts`）；本轮收口轮以 **F-01 强制必红实跑**再次验证（`EXIT=1` + 诊断落盘 → 还原 `PASS 192 / EXIT=0`）。
3. **R1 的 I-01~I-10**：I-01~I-08 / I-10 已由 R2 修复（见 `build.md §10.4`）；**I-09** 当时 deferred，**本轮已修**（N-05：`setEnv(patch, replace=true)` + `reset()` 真清空 env + 门禁断言）。
4. **validate R1 的 N-01~N-13**：已由本轮收口轮逐条处置（元门禁三处盲区自修 + N-04/N-05/N-07/N-08 修实现 + N-09 口径入注释 + N-10 文档订正 + N-11 加 `dereference` 保护 + N-12 扫目录推导），**N-06 不改本叶实现**，收敛到 **v3-4**（`spec.md §7.1` AC-CONV-1/AC-CONV-2 + `tasks.md` TASK-415）。逐条证据见 `build.md §11`。
5. **当前状态**：`phase=validated`（收口轮非新阶段）；门禁串行 **13/13 `exit=0`**（764/11/127/157/103/167/108/192/24/9/e2e）；体积显式重登记 `sidepanel.js 327,679 → 328,476 B`（ceiling **344,899 B**，容差 5% 未动）；`content.js 177,076` 不变。

## 修订记录

| 版本 | 变更说明 | 日期 | 修订人 |
|------|---------|------|--------|
| v1.1 | **收口轮回填指针（sddu-build，2026-09-16）**：新增 §15 —— ① §5-11① 的 spec 订正已由 R2 的 I-03 落盘；② F-01 已修并由收口轮再次强制必红验证；③ I-01~I-08/I-10 已修、**I-09 deferred 项本轮已修**（N-05）；④ validate R1 的 **N-01~N-13 已逐条处置**（N-06 收敛到 v3-4）；⑤ 当前 phase=validated + 门禁 13/13 绿 + 体积重登记 328,476/344,899。**上文原文逐字保留。** | 2026-09-16 | SDDU Build Agent |
