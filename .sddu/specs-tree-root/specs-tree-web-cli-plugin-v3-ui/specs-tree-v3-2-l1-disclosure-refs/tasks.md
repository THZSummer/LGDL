...
| 版本 | 变更说明 | 日期 | 修订人 |
|------|---------|------|--------|
| v1.2 | **修复轮 R2（review R1 的 F-01 + I-01~I-10 处置）**：任务集**不变**（TASK-201~210 全部 completed）；本轮为「门禁自身完整性」修复 —— ① **F-01**（`test/ui/binding.mjs` 失败不计入退出码，跨叶预先存在缺陷）：四条最小修复 + 实跑验收（强制变红 → exit=1；还原 → PASS 192 / exit=0）；② **新增元门禁** `test/gate-integrity.test.ts`（无 Chromium，入 `npm test`；新增 `test:gate-integrity` script 并插入 `test:v3` 串行链），把「失败必须传进退出码 / CDP send 必须有界」变成机器判据；③ I-01~I-10 逐条处置（I-09 deferred）；④ 全门禁 + 反证全套复跑至绿（760/11/127/157/**99**/167/108/192/24/e2e/**6**）。**任务清单本身未改**（未新增/未删除任务，未改波次与依赖）；详见 `build.md §10`。 | 2026-09-16 | SDDU Build Agent |
| v1.1 | **任务状态订正（修复轮）**：TASK-201~210 **全部 completed**（实现 + 门禁 + 收口）；其中 TASK-209（体积核对/重登记）与 TASK-210（收口）按**编排器裁决 V3-VOL-1** 完成终态 —— 撤销自加 `SIDEPANEL_CEILING_CAP`（降级为纯记录）、判定恢复公式 ceiling 344,062 B、四条替代守卫落地、逐断言反证（RP-L1-A~H）与既有反证（RP-V3-01/03/04/05/06/08/09 + I1）全套复跑、全门禁绿（npm test 754/754/0）。注：§4.2 体积命令示例中的旧 ceiling 数值（279,825）为**任务分解当时**的历史值，已被 TASK-209 的显式重登记链取代（历史逐字保留，不在本文档回改）。 | 2026-09-16 | SDDU Build Agent |
