...
| # | 裁决 | 执行情况 |
|---|------|---------|
| A1 | **F8 订正，不豁免**：把本叶 spec §5/§7 与 state.json#gateSet 的 `sidepanel ≤279,825` 订正为经显式重登记后的值（实测 295,225 / ceiling 306,099），**逐字保留历史行**；父 spec/plan 同数字同步订正 | ✅ **6 个文件 / 15 处**（`grep` 可核）：条文 **12 处**带 `~~历史行~~` 标记（本叶 spec §115/§150、本叶 plan §59/§152、父 spec §90/§294/§377、父 plan §75/§222/§226/§396/§720）+ 机读 state.json **3 处**（本叶 `#independence.gateSet`(+`gateSetHistory`)、父 `#evidenceFacts.sizeGuards`(+`history`)、父 `#scope.in` 的 NFR-V3-005 索引）——见 §6.2 F8 / §6.6 |
