...
| 体积（硬） | `dist/content.js` = **177,076 B = `CONTENT_MAX_BYTES`**（无容差）；`dist/sidepanel.js` = ~~266,500 B（基线 266,500 / ceiling 279,825）~~ → **295,225 B（基线 295,225 / ceiling 306,099）**（**收口轮订正 2026-09-16，validate R1 F8 + 编排器裁决**：`~~279,825~~ → 306,099`、`~~266,500~~ → 295,225`（经显式重登记订正；容差 5% 未变、`SIDEPANEL_CEILING_CAP` 只降不升、断言零删减、历史值逐字保留）。原文如下）（**裁决订正 2026-09-16，经裁决 V3-VOL-1 订正**：`~~306,099~~ → **344,062 B**`（= `floor(327,679 × 1.05)`）、`~~295,225~~ → **327,679 B**`；**cap 已降级为纯记录**（`record-only`，不参与判定）；判定 = 公式 ceiling + 四条替代守卫（公式 / 显式重登记登记册 / 增长正当性证据（逐模块归因）/ >15% 方向性告警，**已触发 +22.96% 并已回报编排器**）；历史值 279,825 / 306,099 / 266,500 / 295,225 逐字保留、容差 5% 未变、断言零删减）（容差 5%，`targetBudgetBytes`/`targetMet` = `null`） | `test/size-baseline.ts` |
...
**C. `dist/sidepanel.js`（~~基线 266,500 / ceiling 279,825~~ → 基线 295,225 / ceiling 306,099，容差 5%）**（**收口轮订正 2026-09-16，validate R1 F8 + 编排器裁决**：`~~279,825~~ → 306,099`、`~~266,500~~ → 295,225`（经显式重登记订正；容差 5% 未变、`SIDEPANEL_CEILING_CAP` 只降不升、断言零删减、历史值逐字保留）。原文如下）（**裁决订正 2026-09-16，经裁决 V3-VOL-1 订正**：`~~306,099~~ → **344,062 B**`（= `floor(327,679 × 1.05)`）、`~~295,225~~ → **327,679 B**`；**cap 已降级为纯记录**（`record-only`，不参与判定）；判定 = 公式 ceiling + 四条替代守卫（公式 / 显式重登记登记册 / 增长正当性证据（逐模块归因）/ >15% 方向性告警，**已触发 +22.96% 并已回报编排器**）；历史值 279,825 / 306,099 / 266,500 / 295,225 逐字保留、容差 5% 未变、断言零删减）

| 项 | 结论 |
|---|---|
| 预估增量 | v3-1 +8~12 KB（L0 骨架 + 风险位 + 折叠器 + 状态栏）、v3-2 +6~9 KB（L1 八类 + 引用模型）、v3-3 +4~6 KB（视图宿主 + 计数，主要复用既有实现）、v3-4 +3~5 KB（chip / 角标联动）→ **合计 +21~32 KB → 预计 287,500~298,500 B，很可能 > ceiling ~~279,825~~**（收口轮订正 2026-09-16：显式重登记后实际 ceiling = 306,099，见 spec.md §5/§7 的订正说明；本预测原文按下不删）（**裁决订正 2026-09-16，经裁决 V3-VOL-1 订正**：`~~306,099~~ → **344,062 B**`（= `floor(327,679 × 1.05)`）、`~~295,225~~ → **327,679 B**`；**cap 已降级为纯记录**（`record-only`，不参与判定）；判定 = 公式 ceiling + 四条替代守卫（公式 / 显式重登记登记册 / 增长正当性证据（逐模块归因）/ >15% 方向性告警，**已触发 +22.96% 并已回报编排器**）；历史值 279,825 / 306,099 / 266,500 / 295,225 逐字保留、容差 5% 未变、断言零删减）（如实登记为**高概率**事件） |
...
| **R3-04 `sidepanel.js` 超 ceiling（~~279,825 B~~ → ~~306,099 B~~ → **344,062 B**，2026-09-16 经裁决 V3-VOL-1 显式订正：cap 降级为纯记录、判定 = 公式 + 四守卫）** | 高 | 中 | ① 静态骨架/模板落 `sidepanel.html`（**不在 `sidepanel.js` 守卫口径**）且在 `hidden` 子树内（不污染密度）；② 逻辑复用既有 `view-model` / render 路径，不新建重复渲染层；③ 若首轮实测仍超 → 按 ADR-V3-011 走**显式重登记**（前后值 + 日期 + 来源 + 理由 + 历史保留，容差 5% 不变），**不得**删断言或放宽容差 |
...
3. **`dist/sidepanel.js`（~~基线 266,500 / ceiling 279,825~~ → ~~基线 295,225 / ceiling 306,099~~ → **基线 327,679 / ceiling 344,062 = `floor(327,679 × 1.05)` / 容差 5%**，2026-09-16 经裁决 V3-VOL-1 订正：cap 降级为纯记录、判定 = 公式 + 四守卫）**：若叶子改动导致实测超 ceiling，走**显式重登记**，落点 `test/size-baseline.ts`，必须同时满足：
