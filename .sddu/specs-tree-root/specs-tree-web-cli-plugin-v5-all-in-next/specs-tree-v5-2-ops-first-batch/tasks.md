...
> **状态口径（review R1 I-15 订正）**：本文件的 128 个 `[ ]` 是**验收清单**（FR / AC 项的逐条映射），
> **不是任务状态字段**。任务完成状态以本叶 `state.json`（`r2Artifact` / `phases`）为**唯一权威** ——
> 30/30 任务的完成记录见 `build.md`（R1 §3 + R2 §R2-2）与本叶 `state.json`。
