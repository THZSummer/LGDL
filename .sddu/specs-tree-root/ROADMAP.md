...
| **v0.8.0** | 浏览器插件孵化（**F-14 web-cli-plugin**，作者裁决 2026-09-10 由 v1.1 提前至 v0.8 作该版主题） | 门槛式启动（时间待作者重排；原 v0.8 工程质量内容后移 v0.9）；**F-14 P0~P2 全量 build 完成 + Gate-D 下线执行（2026-09-12）** | 🚧 推进中（P0~P2 全量 build 完成，待 review/validate） | **F-14 web-cli-plugin 浏览器插件**（独立于 LGDL、可作用于 LGDL 的 Web 页面、替代现有内置 AI 助手；配置 LLM key 后驱动打开网站的 web-cli 完成业务）；隐含新基础设施 = web-cli 协议发现/声明机制；**SDDU P0 全流程完成 + P1/P2 实现**（specs-tree-web-cli-plugin，46 FR / 10 NFR / 16 EC / 12 AC，16 任务 9 波次，16/16 已实现；**TASK-016 内置助手下线已执行** + 发布渠道就绪；代码在 feature/web-cli-plugin 未合入发布；见 §二 v0.8） |
...
- [ ] v0.8 收口：`@sddu-review` / `@sddu-validate` → 遗留（非阻塞：C-4/C-5 过渡期关闭 / Gate-D D-3 人工面 H6 与真实浏览器人工 UX / 商店发布 S-016）→ 合入与发布由作者执行（时间不承诺）
...
### v0.8.0（浏览器插件孵化——F-14 web-cli-plugin）🚧 推进中（P0~P2 全量 build 完成 + Gate-D 下线执行，待 review/validate；门槛式启动，时间待作者重排）
...
**版本落点**：代码在分支 **feature/web-cli-plugin（未合入 main / 未发布）**，P0 基线 HEAD `1a1c7cf`（P1/P2 本轮工作树未提交，由上层统一提交）；本 Roadmap 不编造发布时间承诺——合入与发布由作者执行。
...
| F-14 | web-cli-plugin 浏览器插件（web-cli 生态消费端）：**独立于 LGDL 的浏览器插件，可作用于 LGDL 的 Web 页面、替代现有内置 AI 助手**；配置 LLM key 后，借助打开的网站的 web-cli 协议完成业务——任何支持 web-cli 协议的网站可被 AI 驱动（不限于 LGDL 工作台）；隐含新基础设施：web-cli 协议发现/声明机制（网站声明支持 web-cli 及暴露工具，方案不预设立场） | **作者新增规划（2026-09-01，非审视池项）；作者裁决 2026-09-10 提前至 v0.8（原登记于 v1.1 开源线）** | **P1（作者指令，产品演进）** | **v0.8（作者裁决 2026-09-10 提前至 v0.8，作该版主题；门槛式）** | web-cli 生态消费端（新形态）+ 协议发现机制（新基础设施） | 🚧 推进中（P0~P2 全量 build 完成〔16/16 任务，2026-09-12〕+ Gate-D 内置助手下线执行，待 review/validate；Feature 目录 `specs-tree-web-cli-plugin`；分支 `feature/web-cli-plugin` 未合入发布〔P0 基线 HEAD `1a1c7cf`〕；测试基线 web-cli-base 483 / lgdl-web 31 / web-cli-plugin 112 / lgdl-core 267 / lgdl-render 94+1skip / lgdl-router 8 / lgdl-web-cli 84 / lgdl-web-op-cli 15，全仓 0 fail） |
...
v0.6.0 发布后依赖线（v1.6.0 更新；v1.9.0 版本位调整：v0.7 = F-25、原 v0.7 工程质量 → v0.8、原 v0.8 → v0.9；v1.12.0 再调整：v0.8 = F-14、工程质量 → v0.9、AI 增强 → v0.10）：
        ...
        ├── v0.8（浏览器插件孵化，作者裁决 2026-09-10）── F-14 web-cli-plugin（🚧 推进中：P0 validate 2026-09-11 + P1/P2 全量实现 2026-09-12〔16/16 任务〕+ TASK-016 内置助手下线执行，⚠️ 0 阻塞，待 review/validate；由 v1.1 提前至 v0.8 作该版主题）──依赖──
...
4. **v0.8（浏览器插件孵化，作者裁决 2026-09-10）推进——F-14 web-cli-plugin**（由 v1.1 提前至 v0.8 作该版主题）：✅ **P0 SDDU 全流程完成** + ✅ **P1/P2 全量实现（16/16 任务，2026-09-12，phase=builded）**（`specs-tree-web-cli-plugin`；⚠️ 有条件通过 0 阻塞；代码在 feature/web-cli-plugin 未合入发布）→ **TASK-016 已执行内置助手下线（Gate-D D-1~D-7 达标；回退=单提交 revert + flag 默认 off；EC-016 不静默）+ FR-046 发布渠道 `docs/release.md` 就绪** → 待 `@sddu-review` / `@sddu-validate` 收口 + 遗留非阻塞项（C-4/C-5 过渡期关闭 / Gate-D D-3 人工面 H6 与真实浏览器人工 UX / 商店发布 S-016）→ 合入与发布由作者执行（时间不承诺，见 §二 v0.8）；定位 = 独立于 LGDL、可作用于 LGDL Web 页面、替代现有内置 AI 助手；技术细节（协议发现机制/插件架构）不预设立场。
