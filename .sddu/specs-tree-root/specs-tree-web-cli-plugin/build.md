...
## 36. 可选权限能力：书签（读+写，写侧 ask，删除破坏性）+ 下载记录（只读）（TASK-038 / 作者裁决 2026-09-13 / Wave 30）

### 36.1 问题 / 背景（作者裁决 2026-09-13）

作者批准新增两个浏览器能力：**书签**（读 + 写，写侧一律 `ask`，删除为**破坏性**永不自动放行）与**下载记录**（**只读**，明确不做取消/暂停/删除/打开）。关键约束：**不得以静态 `permissions` 声明**——静态新增权限会在扩展**更新时使已安装扩展被 Chrome 停用**直到用户重新同意。因此二者以 **`optional_permissions`** 声明（静态安装面零变化、可单独撤销），首次使用才在**扩展页面手势内**请求。

同时，`chrome.permissions.request` **只有在扩展页面且处于用户手势中**才可能被授予；**在 service worker 内调用无手势必失败/挂起**——这是本轮最重要的实现约束（请求只能从侧栏设置视图 / options 兜底页的按钮点击处理器内发起）。

### 36.2 实现（`packages/web-cli-plugin/`；base 零改动 / 静态 `permissions` 零新增 / 零新依赖）

| 文件 | 改动 |
|------|------|
| `manifest.json` | **仅**新增 `optional_permissions: ["bookmarks","downloads"]`；静态 `permissions`、`host_permissions`、`optional_host_permissions`、`minimum_chrome_version`、`side_panel`、`options_page` **全部不变**；无 `<all_urls>`、无静态 `content_scripts` |
| `src/platform/capability-permissions.ts`（新增） | `hasCapabilityPermission`（fail-safe false）/ `requestCapabilityPermissionOnGesture`（**手势内同步**调 `api.request`，首个 `await` 之前）/ `removeCapabilityPermission` / `changeTouchesCapability`（`onAdded`/`onRemoved` 对账判据）/ `capabilityPermissionsApi`（lazy `globalThis.chrome`） |
| `src/tools/bookmarks-tools.ts`（新增） | 单工具 `bookmarks`（`group:'plugin'`，LLM 名 `^[a-zA-Z0-9_-]+$`）；子命令 `list`/`search`/`tree`=`read`（默认开）、`add`/`remove`/`move`=`write`（默认关，`ask`）；`BOOKMARKS_DESTRUCTIVE_SUBCOMMANDS={remove}` + `isBookmarksDestructive`；未授权可读「去设置开启」（不静默）；`remove` 仅单个 `--id`、`--all` 可读拒绝；`add` 仅 http(s)；URL 默认 origin+path、`--full` 显式；每子命令审计零明文（搜索词只记长度）；`describeTarget`（ask 前示人「标题 + 去参 URL + 不可逆」） |
| `src/tools/downloads-tools.ts`（新增） | 单工具 `downloads`（`group:'plugin'`）；**只读** `list`/`search`=`read`；顶层 risk `read`；cancel/pause/resume/erase/removeFile/open/show 为**明确不实现**的可读拒绝；未授权可读；文件名仅 basename；来源 URL 默认去参；上限 100 + 截断披露；审计零明文 |
| `src/background/capability-setting.ts`（新增） | 隐私开关存储（`web-cli:capability-settings`）：默认 `bookmarksRead=true` / `bookmarksWrite=false` / `downloadsRead=true`；缺省/非法回落默认（fail-safe） |
| `src/background/host.ts` | 注册/卸载 `bookmarks`/`downloads`（受开关 + 撤销抑制 `suppressCapability` 控制，关闭即从 `deriveTools()` 移除）；`isPluginDestructiveInvocation`（`bookmarks remove`→destructive；未知插件工具 fail-closed=true） |
| `src/security/auto-authorize.ts` | **硬底线 4 前移**：`risk==='write' && destructive` 的判定移到 `group!=='site'` 过滤**之前**，使插件级破坏性写同样权威地不纳入「写操作自动」 |
| `src/background/service-worker.ts` | `createBookmarksToolDeps`（真实 `chrome.bookmarks.get/getTree/search/create/remove/move` + `permissions.contains`）/ `createDownloadsToolDeps`（真实 `chrome.downloads.search`，**不**接 cancel/pause/erase/open）；`capabilities` 消息（`status`/`set`/`permission-changed`）；`permissions.onAdded`/`onRemoved` 对账（授予→按开关注册；撤销→移除 + `optional-permission` 审计）；`ask` 的 `describe` 接缝新增 `bookmarks remove` 目标摘要 |
| `src/background/messaging.ts` / `src/security/audit-sink.ts` | 消息种类 `capabilities`；审计类型 `bookmarks`/`downloads`/`optional-permission` |
| `src/ui/settings/{view,ops,panel}.ts` + `src/ui/options/{index.html,options.ts}` | 新增「能力与隐私（可选权限）」分区：`开启书签访问`/`开启下载记录访问` 按钮（**click 手势内**调 `request`）+ 读/写开关 + 状态回显（未开启/已开启/已撤销）+ 拒绝路径可读；`SETTINGS_SECTIONS` 增 `capabilities` |
| `test/parity/waivers.json` | `pluginExtras` 登记 `bookmarks`/`downloads`（否则 parity 门禁拦下） |
| `test/{bookmarks-tools,downloads-tools,capabilities,capability-wiring}.test.ts`（新增） / `test/parity.test.ts` / `test/ui/{journey,binding}.mjs` / `test/e2e/fullchain.mjs` | 见 §36.4 |

### 36.3 新增决策（D-161~D-165）

- **D-161（可选权限 vs 静态权限）**：以 `optional_permissions` 声明 `bookmarks`/`downloads`，**不动静态 `permissions`**。理由：静态新增会在扩展**更新时使已安装扩展被 Chrome 停用**直到重新同意；可选权限保持静态安装面零变化且**可按能力单独撤销**。**被否决**：直接进静态 `permissions`（更新即停用，且无法单独撤销）；不声明权限只在 UI 隐藏（`chrome.bookmarks`/`downloads` 调用必失败，等于假能力）。
- **D-162（请求只能在扩展页面手势内发起）**：`requestCapabilityPermissionOnGesture` 在 click 处理器内**同步**调用 `api.request`（首个 `await` 之前），随后由扩展页发 `capabilities/permission-changed` 让 background 重读真实授权并对账；**SW 内绝不 `request`**。**被否决**：在 SW 里请求（无手势必失败/挂起）；请求后不重读真实授权、只信前端状态（会与真实权限漂移）。
- **D-163（撤销即时对账，绝不静默残留）**：`permissions.onRemoved`（浏览器/用户撤销）→ `suppressCapability(cap,true)` 立即从 `deriveTools()` 移除 + `optional-permission` 审计；`onAdded` / 设置「开启」→ 按隐私开关重新注册。**被否决**：只在 UI 标记「已撤销」而工具仍在工具面（会静默残留、派发到未授权 API）。
- **D-164（破坏性写硬底线对插件级工具权威 + 前移）**：`bookmarks remove` 判定 destructive 并接入 `auto-authorize`；判定**移到 group 过滤之前**，使「写操作自动」开启时删除书签**仍必须人工确认**，且对插件级工具（`group:'plugin'`）同样生效。`isPluginDestructiveInvocation` 对未知插件工具 fail-closed 返回 destructive。**被否决**：把 `remove` 归为普通 `write`（写自动会放行删除，不可逆）；仅在工具层做提示而不接硬底线。
- **D-165（只读 downloads 明确不实现破坏性动词 + 审计零明文）**：`downloads` 只做 `list`/`search`，cancel/pause/resume/erase/removeFile/open/show 一律可读拒绝（绝不静默 no-op）；书签 URL 默认 origin+path（`--full` 显式）、下载文件名仅 basename、搜索词只记长度。**被否决**：顺手实现 `downloads.erase`/`removeFile`（越权扩张且破坏性）；把完整本地路径/带参 URL 进入审计或上下文。

### 36.4 门禁与验证（本轮复跑原文摘录；串行执行，避免 OOM）

- `npx tsc --noEmit`（`packages/web-cli-plugin`）：**0 error**（EXIT=0）。
- `npm test --workspace @lgdl/web-cli-plugin`：**tests 488 / pass 488 / fail 0**（上一轮 452，**+36**；含 `test/bookmarks-tools.test.ts`、`test/downloads-tools.test.ts`、`test/capabilities.test.ts`、`test/capability-wiring.test.ts`）。
- `npm run test:ui`（journey）：**UI journey PASS — 149 assertions**（上一轮 141，**+8**：`#54a`~`#54h` 设置视图能力分区 / 未授权回显「未开启」/ 默认读开写关 / 拒绝路径可读 / 点击真实调用 `chrome.permissions.request({permissions:['bookmarks']})`）。
- `npm run test:binding`：**binding PASS — 153 assertions**（上一轮 143，**+10**：`#54B1`~`#54B10`；真实 dist + 真实 `http://localhost:5173` + mock LLM；工具面 23 个含 `bookmarks`/`downloads`；真实读书签去参**零明文**；`remove` 二次确认摘要含标题 + 「不可逆」；**写自动开启时删书签仍弹确认** `#54B8`；真实删除两个书签；真实读下载记录）。
- `npm run test:e2e`：**R8 E2E PASS**（真实 dist full chain，A/B 场景）；新增 bookmarks（list/add/remove 破坏性仍 ask）/downloads（list）真机断言。
- 全仓 `npm test`：**EXIT=0**（各 workspace 全绿，base **483 零回归**）。
- 红线核验：`manifest.json` 静态 `permissions` **零新增**（仅 `optional_permissions` +2）；无 `<all_urls>`、无静态 `content_scripts`、`host_permissions`/`optional_host_permissions`/`minimum_chrome_version` 不变；`packages/web-cli-base/**` **零改动**；无新依赖；无明文 key；无静默失败。
...
### 37.1 本轮范围与 manifest diff

作者 2026-09-13 批准：`notifications`（读+写）、`clipboardRead` + `clipboardWrite`（读+写，**读侧隐私开关默认关**）。声明方式沿用第 5 轮（TASK-038）既有模式：**`optional_permissions` + 扩展页手势内用时请求**；**静态 `permissions` 零新增**（避免 Chrome 更新时停用扩展、可按能力单独撤销）。

`packages/web-cli-plugin/manifest.json` diff（静态面零变化）：仅 `optional_permissions` 由 `["bookmarks","downloads"]` → `["bookmarks","downloads","notifications","clipboardRead","clipboardWrite"]`；`permissions` / `host_permissions` / `optional_host_permissions` / `minimum_chrome_version` / 无 `<all_urls>` / 无静态 `content_scripts` 全部不变。

**明确未批、未加**：`history` / `cookies` / `declarativeNetRequest` / `debugger` / `tabGroups`（不得顺手加）。

### 37.2 「可 optional」资格核实（目标 Chromium 实测，2026-09-13）

Chrome 对个别权限是否允许放进 `optional_permissions` 有约束，故先核实。用仓库自带 Chromium（`chromium-1234`，**151.0.7922.34**）加载最小 MV3 扩展，在**扩展页真实手势**（CDP `Runtime.evaluate` `userGesture:true`）内调用 `chrome.permissions.request`，逐项记录：

| 权限 | 结果 | 结论 |
|------|------|------|
| `notifications` | PENDING（弹出真实授权提示，无即时错误） | **可 optional** |
| `clipboardRead` | PENDING | **可 optional** |
| `clipboardWrite` | PENDING | **可 optional** |
| `debugger` | `Only permissions specified in the manifest may be requested.` | 不可 optional（对照） |
| `proxy` | 同上 | 不可 optional（对照） |
| `geolocation` | 同上 | 不可 optional（对照） |
| `declarativeNetRequest` | 同上 | 不可 optional（对照） |
| `totally-fake-permission-xyz` | `'…' is not a recognized permission.` | 不认识（对照） |

- 结论：本插件本轮三个权限**都能** optional，且**未偷偷塞进静态 `permissions`**。对照组的「既有 permission 名但不允许 optional」被明确区分（说明该探测不是假阴性）。
- 附带观测：`chrome.notifications` 在**未授予**时于 SW 内 `undefined`，授予后可用；SW 内 `navigator.clipboard` **不存在**（这正是剪贴板必须走扩展页的根因）。
- 本轮另有 `tabGroups` 也返回 PENDING（技术上可 optional），但**作者未批**，故未加。

### 37.3 实现路径（复用 vs 插件侧）

- **`notify` → 插件侧 `chrome.notifications`**：base `notify`（`packages/web-cli-base/src/notify.ts`）是页内 `Notification` 面（`env.notify` → 浏览器 `Notification` 构造器），在扩展 SW 不可用；**不复用 base 条目**，改为插件侧 `src/tools/notify-tools.ts`（子命令 `list` / `send` / `clear`），保留基线要求的 `send` 子命令以通过对账门禁。
- **`clipboard` → 扩展页 `navigator.clipboard`（回退 `document.execCommand`）**：SW 无 `navigator.clipboard`，MV3 也无宿主剪贴板 API，故 `src/tools/clipboard-tools.ts` 通过注入的 `readText`/`writeText` 缝，把纯文本读/写**转发到扩展页**执行（新增 `src/platform/clipboard-page.ts`，侧栏与 options 页共用处理器；SW 发 `clipboard-op`，扩展页 `sendResponse` 回执并**如实标注实际路径**）。无法到达扩展页时返回可读「请保持侧栏打开」拒绝（不静默）。
  - **功能裁剪**：`write-html` / `write-image` / `paste-read` **不实现**——保留在 schema（基线子命令对账需要），但一律返回可读「未实现」，绝不静默/假装成功。
- 复用第 5 轮机制（未重造）：`capability-permissions.ts`（手势内 request）、`capability-setting.ts`（隐私开关 + 默认值）、`host.ts` 注册/`suppressCapability`、`permissions.onAdded`/`onRemoved` 对账、`waivers.json` `pluginExtras` 登记（`notify`/`clipboard` 是**基线同名**，属「已提供」，故**不登记 pluginExtra**，改为把 `capability-matrix.md` 状态由「待批准权限」更新为「已提供」）。

### 37.4 逐子命令 risk 表与安全硬要求

| 工具 | 子命令 | risk | 说明 |
|------|--------|------|------|
| `notify` | `list` | `read` | 列出通知 id + 系统权限级别（不回显正文） |
| `notify` | `send` | `write` | 创建单条通知（`--title` 必填；`--body` 可选） |
| `notify` | `clear` | `write` | 清除**单条**（`--id`；`--all`/批量可读拒绝） |
| `clipboard` | `read` | **`state`** | 读取剪贴板纯文本；**默认关**；**永不自动放行** |
| `clipboard` | `write` | `write` | 写入剪贴板纯文本（`--text` 必填） |
| `clipboard` | `write-html` / `write-image` / `paste-read` | `write` / `write` / `state` | **裁剪**：可读「未实现」 |

硬要求（均有专门断言钉住）：
1. **剪贴板读 = `state` 档，永不自动授权**：`state`/`evaluate` 不纳入自动开关；即便「读操作自动」开启，剪贴板读**仍然 ask**。断言：`test/clipboard-tools.test.ts` 的「state tier is never auto-authorized（纯决策：plugin 组不 allow、site 组 `hardDeny`）」+「read-auto ON still ASKS（host 集成：`asks()===1` 且 `auto-authorize allow` 记录为 0）」。
2. **内容零明文**：剪贴板内容 / 通知正文**不进审计与日志**（只记长度/路径）。工具自审记录 `chars=`/`titleLen=`/`bodyLen=`；并且 `src/security/confirm.ts` 新增 `scrubContentArgs`——把 `clipboard`/`notify` 的 `text`/`title`/`body` 等**内容参数**替换为「已省略 N 字符」后才生成确认摘要与 `confirm` 审计事件（否则 `summarizeArgs` 只按 key 名匹配敏感词，`text`/`title` 会漏网明文）。断言：`JSON.stringify(audit.events)` 不含明文 + `scrubContentArgs` 纯函数断言。
3. **risk 档不放宽**：`clipboard` 顶层 `risk:'state'`（保守回退），写类不因「写操作自动」放行（插件级工具不纳入按 origin 自动授权，沿用既有 hard floor）。

### 37.5 隐私开关、对账与文档

- 默认：通知**开**（仍需授权）；剪贴板**读关 / 写开**（`capability-setting.ts`，`web-cli:capability-settings`）。关闭即从 `deriveTools()` 移除（不只是前端隐藏）。
- 未授权可读文案指向「⚙ 设置 → 能力与隐私 → 开启通知 / 开启剪贴板访问」；`onRemoved` 撤销即摘除工具并审计，`onAdded`/`permission-changed` 重授即按开关恢复。
- UI：侧栏设置视图新增「系统通知」「剪贴板」两行（按钮 + 开关 + 状态回显，`settings-cap-notify-*` / `settings-cap-clipboard-*`）；options 兜底页同步（`cap-notify-*` / `cap-clipboard-*`）。
- 文档同步：`docs/capability-matrix.md`（第 26/27 行 待批准 → **已提供**；汇总「待批准权限 2 → 0」；§2 重写为「已批准并实施」+ 资格核实结论）、`docs/compliance.md` §13、`docs/dev.md` §18、`docs/release.md` §6/§8。
...
|------|------|
| `npx tsc --noEmit` | 0 error |
| 插件 `npm test` | **514 pass / 0 fail**（488 → 514，+26；新增 `notify-tools`/`clipboard-tools` 测试文件 + capabilities/capability-wiring 扩展） |
| `test:ui` | **155 assertions**（149 → 155，#54i~#54n；断言只增不减） |
| `test:binding` | **157 assertions**（153 → 157，真实 dist + 真实 Chromium + 静态权限副本；新增 #54B11~#54B14：`contains=true` + `notify`/`clipboard` 进入真实发给 LLM 的工具面 + `chrome.notifications.getPermissionLevel()=granted` + 侧栏 `navigator.clipboard` 可达；真实往返 `wc-clip-probe` 成功为观测项） |
| `test:hardening` | **24** |
| `test:e2e` | **PASS**（新增 `notify list` 走真实 `chrome.notifications`、`clipboard write` **真实写入成功**〔观测 `document.execCommand(copy)`，13 字符〕、`clipboard read` 达真实扩展页路径；headless 焦点限制与副本偏差均已披露） |
| 全仓 `build` + `test` | 0 fail（base **483 零回归**） |

红线：base 零改动、静态 `permissions` 零新增、无 `<all_urls>`、无静态 `content_scripts`、无新依赖、无明文 key、无静默失败。

### 37.7 新增决策（D-168~D-172）

- **D-168（先核实「可 optional」资格再落声明）**：在写 manifest 前用目标 Chromium 真实手势实测 `notifications`/`clipboardRead`/`clipboardWrite` 可 optional，并设「既有权限名但不允许 optional」的正对照（`debugger`/`proxy`/`geolocation`/`declarativeNetRequest`）以排除假阴性。**被否决**：直接假设可 optional（Chrome 对个别权限有约束，错了会让扩展加载/请求失败）；把不能 optional 的权限偷偷塞进静态 `permissions`（红线）。
- **D-169（notify 用插件侧 `chrome.notifications`，不复用 base 页内条目）**：base `notify` 是页内 `Notification` 面，SW 不可用；复用＝注册一个永远执行不了的工具。改为插件侧实现并保留基线 `send` 子命令。**被否决**：复用 base 条目（假能力）；不实现而继续挂「待批准」（作者已批准）。
- **D-170（clipart 走扩展页 `navigator.clipboard`，并如实标注路径）**：SW 无 `navigator.clipboard`、MV3 无宿主剪贴板 API → 转发到扩展页执行；优先异步 API，失败回退 `document.execCommand`，输出标注实际路径；侧栏与 options 共用 `clipboard-page.ts`。**被否决**：在 SW 里假装可读（必然失败）；为剪贴板引入 `offscreen` 权限（越权扩张，作者未批）。
- **D-171（剪贴板读钉死 `state` 档 + 永不自动授权）**：读侧隐私敏感 → 顶层 `state`、`read:'state'`；`state`/`evaluate` 不纳入自动开关，即便「读操作自动」开仍 ask；`test/clipboard-tools.test.ts` 同时用纯决策与 host 集成两条路径断言。**被否决**：把读降为 `read`/`ui`（会被自动放行或误纳入自动档）；只在 UI 文案上提示而门禁不钉。
- **D-172（内容零明文的兜底 scrub 而非只靠 key 名匹配）**：现有 `summarizeArgs` 只按 key 名含敏感词才脱敏，`text`/`title`/`body` 会原样进确认摘要与 `confirm` 审计；新增 `scrubContentArgs`（对 `clipboard`/`notify` 的内容键替换为「已省略 N 字符」）在**生成摘要与发送前**执行。**被否决**：把 `text`/`title` 加进全局敏感词（误伤其他工具的合法参数，且仍非按工具权威）；只在工具自审里记长度而放任确认桥明文（审计仍会落明文）。
...
作者原话：可选权限申请「**不够直观**」、点没点「**效果不够明显**」、且「**缺少取消权限**」的操作。修复**统一适用于全部 5 个可选权限**（bookmarks 读/写、downloads 只读、notifications、clipboardRead/clipboardWrite），不是只改书签。

`packages/web-cli-plugin/manifest.json` **零 diff**（不新增权限）；不新增依赖；`packages/web-cli-base/**` 零改动。

### 38.2 三态按钮（实测权限态优先于本地开关）

- 每次渲染都在扩展页**实测** `chrome.permissions.contains()`：`SettingsOps.loadCapabilities()` → `measuredGrants()`（逐能力 `hasCapabilityPermission`）→ `applyMeasuredGrants(view, measured)`（以实测覆盖 background 回报的 `granted`，`granted=true` 同时清 `revoked`）。
- 纯判定 `capabilityActionView(cap, granted, revoked)`（`src/ui/settings/view.ts`）：
  - 未授权 → `mode:'request'`，`授权 Chrome ＜能力＞权限`；
  - 已授权 → `mode:'revoke'`，`撤销 Chrome 权限` + `✅ 已授权`（不显示授权按钮）；
  - 已撤销 → 回到 `request` + 可读「上次已撤销 …」。
- **持久化隐私开关不参与授权判定**：`granted=false` 时复选框置未勾选 + 禁用，状态行显示 `…开关：权限未授予（不生效）`，杜绝「开关显示为开但权限没了」。判定点：`view.ts` `capabilityActionView` / `applyMeasuredGrants` / `capabilityTogglesText`；UI 应用点：`panel.ts` `renderCapabilityStatus()`、`options.ts` `renderCapabilities()`。

### 38.3 明确、持久的回执

`src/ui/settings/view.ts` 的 `capabilityGrantReceipt` / `capabilityDeniedReceipt` / `capabilityRevokeReceipt` / `capabilityRevokeFailureReceipt` / `capabilityStateNote` 为**唯一文案源**，侧栏（`settings-cap-<cap>-receipt`）与 options（`cap-<cap>-receipt`）共同渲染：

| 事件 | 回执原文（示例：书签） |
|------|------|
| 授权成功 | `✅ 已开启书签访问：Chrome 权限已授予（助手工具已进入 LLM 工具面）` |
| 用户拒绝 | `✖ 未开启：Chrome 未授予 bookmarks 权限；可再次点击重试` |
| 撤销成功 | `✅ 已撤销书签权限：Chrome 权限已移除（助手工具已从 LLM 工具面移除）` |
| 撤销失败 | `✖ 撤销书签权限失败：＜原因＞（Chrome 权限仍保留；可重试）` |

动作回执与按钮**同步切换**（成功后按钮立即由「授权…」变「撤销 Chrome 权限」；撤销成功后切回）。

### 38.4 撤销路径（真实移除而非只改 UI）

- `chrome.permissions.remove` **不需要用户手势** → 由 `SettingsOps.revokeCapability(cap)` 在扩展页直接调用（`bookmarks`→`['bookmarks']`、`clipboard`→`['clipboardRead','clipboardWrite']`），再发既有 `capabilities/permission-changed` 对账。
- background 对账：`permissions.onRemoved`（外部撤销）与 `capabilities/permission-changed`（产品撤销）都执行 `suppressCapability(cap, true)` → 工具**立即离开 `deriveTools()`**、派发被可读拒绝、写 `optional-permission/revoked` 审计（复用第 5 轮机制，未重造）。
- 面板刷新：`onAdded`/`onRemoved` 对账后 `chrome.runtime.sendMessage({kind:'capability-changed'})`（新消息 kind）→ 侧栏 `sidepanel.ts` 调 `settingsHandle.refreshCapabilities()`、options 页 `refreshCapabilities()`，**就地刷新**（无需重开面板）。
- 撤销失败（如对静态必需权限调用 `remove` 抛 `You cannot remove required permissions.`）→ 可读失败回执，不静默。

### 38.5 解释性文案（解决「不直观」）

`CAPABILITY_EXPLANATION` 为每个能力行给出一句可读说明：**该权限让助手能做什么** + **为什么 Chrome 会弹窗**（Chrome 列为可选权限 + 必须由你在扩展页面点一次）。例：`「书签访问」= 让助手读取/整理你的书签栏与书签管理器条目；Chrome 把 bookmarks 列为可选权限，必须由你在扩展页面点一次才会弹窗授权。`

### 38.6 一致性

侧栏设置视图（`panel.ts`，主）与 `options.html`（兜底）**共用** `src/ui/settings/{view,ops}.ts` 的判定/操作（`capabilityActionView` / receipts / `capabilityStateNote` / `revokeCapability`），未分叉第二套。
...
|------|------|
| `npx tsc --noEmit` | 0 error |
| 插件 `npm test` | **523 pass / 0 fail**（514 → 523，+9：`capabilities.test.ts` +4 三态/回执/实测优先 + 新增 `test/capability-revoke.test.ts` 5 用例〔remove 路径/成功·失败回执/实测优先/工具随权限增删〕） |
| `test:ui` | **167 assertions**（155 → 167，+12：#54b2/#54b3/#54e/#54f/#54g2/#54j/#54l 语义强化 + #54o~#54w 三态切换/撤销回执/真实 remove/撤销后工具消失；断言只增不减） |
| `test:binding` | **163 assertions**（157 → 163，+6：真实 dist #0j~#0o——bookmarks 未授予 `contains=false`、撤销前工具面含 bookmarks、真实 `permissions.remove` 解析 true、remove 后 `contains=false`、对账后工具面不含 bookmarks、`optional-permission/revoked` 审计） |
| `test:hardening` | **24**（不回归） |
| `test:e2e` | **PASS**（R8 全链；e2e 未改，仅回归复跑） |
| 全仓 `build` + `test` | 0 fail（base **483 零回归**） |

红线：base 零改动、`manifest.json` 零 diff（不新增权限）、无新依赖、无 `<all_urls>`、无静态 `content_scripts`、无明文 key、无静默失败。

### 38.8 新增决策（D-173~D-177）

- **D-173（三态以扩展页实测 `contains` 为准，本地开关不得决定授权）**：UI 每次渲染都在扩展页实测权限态；`applyMeasuredGrants` 以实测覆盖 background 回报，隐私开关仅决定「工具是否暴露」。**被否决**：用 background 回报 + 本地开关渲染按钮（「开关开着但权限没了」会显示为已授权，正是作者反馈的误导）。
- **D-174（一个三态按钮 + 持久回执，不与状态脱节）**：同一按钮按 `dataset.mode` 在「授权（手势 request）」与「撤销（无手势 remove）」间切换；动作回执写入独立持久回执区（`*-receipt`），刷新时回落为与实测态一致的状态说明。**被否决**：加第二个按钮并排（两个按钮同时显示会让「当前是否已授权」更难判定）；只在 status 行临时提示（正是「回执不明显」的根因）。
- **D-175（撤销走 `permissions.remove` + `permission-changed` 双路对账）**：remove 无需手势，页面直接调用；随后显式发 `permission-changed` 让 background `suppressCapability(cap,true)`，不依赖 `onRemoved` 事件时序。**被否决**：只靠 `onRemoved`（时序不可靠，且对「从未授予」的 remove 不触发变更事件）；在 SW 里 remove（虽无需手势，但与既有「所有权限变更由扩展页驱动」的分工不一致）。
- **D-176（撤销后隐私开关回落「未授予」语义）**：`granted=false` 时复选框不勾选 + 禁用，状态行改写为「权限未授予（不生效）」。**被否决**：保留开关为「开」仅加灰（作者原话即该状态「不够直观」，继续显示为开会误以为权限仍在）。
- **D-177（`onAdded`/`onRemoved` 推送 `capability-changed`，面板就地刷新）**：新增轻量 push 消息 kind，侧栏/选项页收到即 `refreshCapabilities()`（重测 `contains`）。**被否决**：要求用户重开面板才看到变化（作者要求「无需重开」）；后台轮询权限态（无必要且耗电）。
...
| v2.9 | **可选权限能力：书签（读+写，写侧 ask，删除破坏性）+ 下载记录（只读）**（§36，TASK-038，Wave 30，作者裁决 2026-09-13）：以 **`optional_permissions`** 声明（**静态 `permissions` 零新增**；避免扩展更新被停用、可按能力单独撤销）；`chrome.permissions.request` **只在扩展页面点击手势内**发起（`src/platform/capability-permissions.ts`；SW 绝不调用），随后 `capabilities/permission-changed` 重读真实授权并对账，`permissions.onAdded`/`onRemoved` 即时对账（撤销即从 `deriveTools()` 移除 + `optional-permission` 审计）；默认书签读开/写关、下载记录读开；未授权返回可读「去设置开启」（不静默）。`bookmarks remove` 为 **destructive** → `auto-authorize` 硬底线（判定**前移到 group 过滤之前**，「写操作自动」开仍 ask）+ 禁止批量 + 仅 http(s) add；`downloads` **只读**、明确不做取消/删除/打开；审计零明文（URL 去参、文件名仅 basename、搜索词只记长度）；`test/parity/waivers.json` 登记 `pluginExtras`。新增 `test/{bookmarks-tools,downloads-tools,capabilities,capability-wiring}.test.ts`；`test:ui` 141→**149**（#54a~#54h）、`test:binding` 143→**153**（测试副本静态权限 + 真实 `chrome.bookmarks`/`chrome.downloads` + 写自动仍弹确认 #54B8）、`test:e2e` **PASS**（副本偏差已披露）；插件 452→**488 pass / 0 fail**、`tsc --noEmit` 0 error、`test:hardening` **24**、全仓 build+test **0 fail**（base **483 零回归**）；**base 零改动 / 静态 `permissions` 零新增 / 无新依赖 / 无明文 key / 无静默失败**；docs dev §17·§3.0 / compliance §12 / release §6 / capability-matrix §3 同步；D-161~D-165；**手势弹窗与 FR-054 条文待补已如实披露（§36.5）**。 |
| v3.0 | **可选权限能力：系统通知（读+写）+ 剪贴板（读+写，读侧默认关）**（§37，TASK-039，Wave 31，作者裁决 2026-09-13）：沿用第 5 轮 `optional_permissions` + 用时请求模式（**静态 `permissions` 零新增**）。**先实测核实**三个新权限在目标 Chromium（151）均可 optional（`debugger`/`proxy`/`geolocation`/`declarativeNetRequest` 为正对照不可 optional）；`notify` 走插件侧 `chrome.notifications`（base notify 为页内 Notification 面、SW 不可用），子命令 `list`/`send`/`clear`；`clipboard` 纯文本 `read`/`write` 经扩展页 `navigator.clipboard`（回退 `document.execCommand`，新增 `src/platform/clipboard-page.ts` 供侧栏与 options 共用），`write-html`/`write-image`/`paste-read` 裁剪为可读「未实现」。**剪贴板读 = `state` 档，永不自动放行**（「读操作自动」开仍 ask，纯决策 + host 集成双断言）；剪贴板/通知内容**零审计明文**（工具自审只记长度，另新增 `scrubContentArgs` 把内容参数替换为「已省略 N 字符」后才入确认摘要/`confirm` 审计）。默认通知开 / 剪贴板读关写开；关闭即从 `deriveTools()` 移除；`onAdded`/`onRemoved` 对账。`capability-matrix` 第 26/27 行由「待批准权限」→「已提供」（汇总待批准 2→0）；新增 `test/{notify-tools,clipboard-tools}.test.ts` + capabilities/capability-wiring 扩展；`tsc --noEmit` 0 error、插件 488→**514 pass / 0 fail**、`test:ui` 149→**155**（#54i~#54n）、`test:binding` 153→**157**（静态权限副本 + `getPermissionLevel()=granted` + 剪贴板 API 可达 + 真实往返观测成功）、`test:hardening` **24**、`test:e2e` **PASS**（notify 真机 + clipboard 写入真实成功）、全仓 build+test **0 fail**（base **483 零回归**）；**base 零改动 / 静态 `permissions` 零新增 / 无新依赖 / 无明文 key / 无静默失败**；docs dev §18 / compliance §13 / release §6·§8 / capability-matrix §2 同步；D-168~D-172；**手势弹窗、扩展页依赖、headless 读空与 FR-055 条文待补已如实披露（§37.8）**。 |
| v3.1 | **可选权限申请 UX 直观化：三态按钮 + 明确回执 + 应用内撤销**（§38，TASK-040，Wave 32，用户 UX 反馈 2026-09-13）：作者反馈「申请不直观 / 点没点成不明显 / 缺少取消权限」，修复**统一覆盖全部 5 个可选权限**（bookmarks 读+写、downloads 只读、notifications、clipboardRead/clipboardWrite）。**三态按钮以扩展页实测 `chrome.permissions.contains()` 为准**（`SettingsOps.loadCapabilities()` → `measuredGrants()` → `applyMeasuredGrants()`；隐私开关**不决定授权**）：未授权 →「授权 Chrome ＜能力＞权限」；已授权 →「撤销 Chrome 权限」+ `✅ 已授权`；已撤销 → 回到未授权 +「上次已撤销」说明。**明确持久回执**（`capabilityGrantReceipt`/`capabilityDeniedReceipt`/`capabilityRevokeReceipt`/`capabilityRevokeFailureReceipt`/`capabilityStateNote`，唯一起源，侧栏 `*-receipt` + options `*-receipt` 共用）；按钮与回执同步切换。**应用内撤销**：`chrome.permissions.remove`（无需手势）→ `permissions.onRemoved` + `capabilities/permission-changed` 双路对账 → `suppressCapability(cap,true)` 使工具**即时离开 `deriveTools()`** + 派发被拒 + `optional-permission/revoked` 审计；撤销后隐私开关回落「未授予」语义（不勾选 + 禁用）；新增 `capability-changed` 推送使面板就地刷新（无需重开）。**每能力解释文案**（做什么 + 为什么 Chrome 弹窗/必须点一次）。侧栏设置视图与 options 兜底页共用同一套 `src/ui/settings/{view,ops}.ts` 逻辑，未分叉。红线：`manifest.json` **零 diff**（不新增权限）、无新依赖、base 零改动。门禁（串行）：`tsc --noEmit` 0 error、插件 514→**523 pass / 0 fail**（+9：`test/capabilities.test.ts` +4 三态/回执/实测优先 + 新增 `test/capability-revoke.test.ts` 5）、`test:ui` 155→**167**（+12）、`test:binding` 157→**163**（+6 真实 `permissions.remove` + 工具移除 + 审计）、`test:hardening` **24**、`test:e2e` **PASS**、全仓 build+test **0 fail**（base **483 零回归**）；docs dev §19 / compliance §14 / tasks TASK-040 同步；D-173~D-177；**原生授权弹窗与「已授权→onRemoved」过渡仍归人工面已如实披露（§38.9）**。 | 2026-09-13 | SDDU Build Agent |
