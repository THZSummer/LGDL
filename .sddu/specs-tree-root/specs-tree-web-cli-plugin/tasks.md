...
### TASK-033: 设置面板改为侧栏内视图（移除 `openOptionsPage` 主入口 / 设置逻辑抽共享模块 / options 降级为兜底薄壳）（用户诉求，v1.10 增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（体验改造 + 消除双实现漂移） |
| **前置依赖** | TASK-032（自动探测推送）/ TASK-030（按 origin 自动授权）/ TASK-028（面板内测试连接）/ TASK-026（tabs 开关）/ TASK-025（会话分组）/ TASK-019（诊断）/ TASK-018（LLM 保存链路） |
| **执行波次** | Wave 25（v1.10 增补） |
| **对应 FR** | FR-017（侧栏 UI）/ FR-033/FR-035（LLM 配置）/ FR-036（配置消息面）/ FR-046（发布物构成）/ FR-047（自动探测不回归）/ FR-049（tabs 开关）/ FR-052（按 origin 自动授权不回归）/ EC-011（导航失效语义不回归） |
| **ADR** | ADR-005（BYOK key-store）/ ADR-011（8 厂商）/ ADR-017（自动授权）/ 新 D-136~D-140（见 build.md §31.3） |
| **TB 映射** | —（非 plan TB，用户诉求「不建议跳到 `chrome-extension://…/options.html`，体验不好，直接在拓展当前页面展示」驱动） |

**描述**: 侧栏设置入口 `#open-options` → `view-model.openSettingsPage()` → `chrome.runtime.openOptionsPage()` 打开**新标签页** `options.html`，用户离开面板后难以返回，体验断裂；且设置逻辑在 `options.ts` 与侧栏间存在双实现漂移风险。改造：① 侧栏新增**设置视图**（`#settings-view`），顶部 `#open-settings`（「⚙ 设置」）与视图内 `#settings-back`（「← 返回对话」）**只切换显隐、不重建 DOM**；`view-switch.ts` 纯逻辑捕获/恢复 `#log.scrollTop` 与 `#input` 草稿，保证已渲染消息、滚动位置、输入草稿全保留。② **不再调用 `openOptionsPage()`**（侧栏与 `view-model.ts` 零调用路径，页面内计数 + target 数双重断言）。③ 设置逻辑抽共享模块 `src/ui/settings/`：`view.ts`（纯视图模型）+ `ops.ts`（`llm-test`/`tabs-setting`/`auto-auth`/`sessions`/`session-group`/`diag`/`llm-status` + key-store，依赖注入、node 可测）+ `panel.ts`（面板渲染，`settings-*` 前缀 id 避免与聊天控件重复 id）+ `diagnostics.ts`（由 `ui/options/` 迁入）+ `styles.ts`；`options.html` 保留为**功能完整的兜底页**（Chrome 扩展程序选项/宽屏排障），其逻辑改为复用同一共享模块，消除分叉。④ 面板覆盖：LLM 配置（厂商/Key 掩码与已存状态/模型/Base URL/maxRounds/保存/测试连接/结果/清除）、按站点自动授权（当前 origin 读/写 + 列表可关闭）、标签页管理、会话分组（列表/新建/并入）、环境自检（运行/结果/复制）、合规与迁移（`<details>`）；窄屏纵向堆叠。⑤ 文案同步（引导第 1 步、LLM 按钮）指向面板内设置。⑥ **顺带修复** TASK-032 探针推送经 `refreshState` 的 `invalidated` 重复写入覆盖「已授权」回执的真实缺陷（`chat-state.ts` 仅在失效 false→true 跃迁提示），使 `test:binding` #4b/#4c 由 FAIL 转 PASS。**base 零改动 / manifest 零改动（`options_page` 保留） / 无新依赖 / 无新权限 / 无 `<all_urls>` / 无明文 key / 无静默失败**。

**涉及文件**: NEW `src/ui/settings/{view.ts,ops.ts,panel.ts,view-switch.ts,styles.ts}`；MOVED `src/ui/options/diagnostics.ts → src/ui/settings/diagnostics.ts`；MODIFY `src/ui/sidepanel/{index.html,sidepanel.ts,view-model.ts,chat-state.ts}`、`src/ui/options/options.ts`、`test/settings.test.ts`(NEW)、`test/sidepanel.test.ts`、`test/sidepanel-view.test.ts`、`test/tabs-wiring.test.ts`、`test/diagnostics.test.ts`、`test/ui/journey.mjs`（#33a~#33o）、`test/ui/binding.mjs`（#33B1~#33B11）、`docs/dev.md`（§3.1.1/§3.2/§4/§11.1）、`docs/release.md`（§2/§5.2/§7）。

**验收标准**: `test/settings.test.ts` 覆盖共享 view 纯函数 / 分区清单与 panel 渲染 id / 写读走既有通道（种类集合精确断言）/ 越界可读失败 / 诊断零明文 / 无 `openOptionsPage` 调用路径 / 协议无新增 `settings` 通道 / `options_page` 保留，全绿；`test:ui` 断言「点「⚙ 设置」在同一页渲染设置视图、`openOptionsPage` 计数=0、page target 数不变、URL 仍 sidepanel.html」「返回对话后消息文本/草稿/滚动位置保留」「面板内保存 + 测试连接可用」，既有断言零删除（总数 121→136）；`test:binding` 真实扩展内完成配置读取/测试连接/自动授权勾选且零标签页跳转（114→125，保留既有）；`test:hardening` 24 / `test:e2e` A/B / `test:parity` 不回归；`tsc --noEmit` 0 error；插件 `npm test` 389 pass / 0 fail、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、manifest 零 diff、无新依赖、无新权限、无明文 key、无静默失败、options 功能不退化。

---

### TASK-034: chrome screenshot 走真实像素（`captureVisibleTab`，插件侧提供者）+ 插件层修正 chrome help 过时绝对表述（用户实测，v1.11 增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（扩展宿主能力补齐 + 工具面文案修正；base 零改动） |
| **前置依赖** | TASK-029（chrome/dom 工具面 + dom-op 远程代理）/ TASK-026（`tabs` 工具）/ ADR-016（浏览器能力补齐） |
| **执行波次** | Wave 26（v1.11 增补） |
| **对应 FR** | FR-051（浏览器能力）/ FR-028（截图；真实像素替代近似）/ FR-008（不可达可读降级）/ FR-005（risk 不放宽）/ NFR-002（无新依赖/无新权限） |
| **ADR** | ADR-003（截图输出策略不变）/ ADR-008（executor 零 DOM 触碰）/ ADR-016（扩展宿主生态位）/ 新 D-141~D-145（见 build.md §32.3） |
| **TB 映射** | —（非 plan TB，用户实测「插件已是扩展宿主，应得真实像素；chrome help 的『书签不可承载』过时且误导」驱动） |

**描述**: base `chrome` 工具的 `screenshot` 走 `env.dom.ops.screenshot`（content script 内 `createBrowserDomOps()` 的 `foreignObject+canvas` **近似**实现：外部图片/CSS 变量/滚动态不保真）。插件现在是**扩展宿主**→ 可得 `chrome.tabs.captureVisibleTab` **真实像素**。改造（**`packages/web-cli-base/**` 零改动**）：① 插件侧新增 `src/platform/real-screenshot.ts`，用**透传 `Proxy`** 包装 `env.dom.ops`，**只覆盖 `screenshot`**：`mode=viewport` → `captureVisibleTab(windowId,{format:'png'})`；`mode=element` → 先经既有 `dom-op` 通道取目标元素 bounding rect（插件侧 `wcliScreenshotRect`，复用 base `readElement` 定位语法，含 dpr/viewport），再在 SW 用 `OffscreenCanvas`/`createImageBitmap` 裁剪；`mode=fullpage` **仍返回既有「不支持」**（D2 单独一轮）。② **前提与降级（必须可辨、不得静默）**：绑定站点为 http(s) 且 `chrome.permissions.contains` 命中 host 权限 → 真实路径；**无 host 权限 / 受限页（`chrome://` 等）/ capture 失败 / 速率限制** → **回退** base 近似路径，输出**如实标注实际路径**（`真实像素（captureVisibleTab）` vs `近似（canvas，原因：<具体原因>）`）——**绝不谎称真实**。③ **契约不变**：仍返回 `PlatformDomOpResult{ok,output,dataUrl}`；输出摘要/自动下载链/`--include-dataurl`/dataURL 不进上下文等既有策略（P-03/ADR-003）全部保持（base `deliverScreenshot` 继续生效）。④ **透传陷阱**：`browser-tools.ts` 用 `ops?.waitFor`/`ops?.extractData` 判定是否注册 `wait`/`extract`/`export` —— 包装**必须透传所有其他方法与属性可见性**（`Proxy` + 断言），否则会**静默摘掉**这些工具。⑤ 插件层包装 chrome 工具条目的 `schema.description` 与 `help`（`src/tools/chrome-host.ts`）：删除页内时代「书签/标签页·窗口/跨域导航/下载历史 = 不可承载 out（NG-002/NG-003）」绝对表述，改为「本工具只承载宿主页会话内的子集；标签页见 `tabs` 工具；书签/下载等由插件按 origin 授权后的宿主层能力承载；screenshot 两条路径如实标注」，base 原文仍正确部分保留。**零新权限**（`activeTab` 已声明）/ **manifest 零 diff** / **无新依赖** / 无明文 key / 无静默失败。

**涉及文件**: NEW `src/platform/real-screenshot.ts`、`src/tools/chrome-host.ts`、`test/real-screenshot.test.ts`；MODIFY `src/platform/browser-env.ts`（装配提供者 + 挂 `meta`）、`src/tools/browser-tools.ts`（包装 chrome 条目）、`src/content/content-script.ts`（`wcliScreenshotRect` dom-op）、`src/background/service-worker.ts`（注入 `captureVisibleTab`/权限/rect deps）、`test/browser-tools.test.ts`（D4 + 透传断言）、`test/e2e/fullchain.mjs`（真实 PNG + 两条路径标注断言；`<all_urls>` 测试偏差披露）、`docs/dev.md`（§13.7）、`docs/compliance.md`（§11）、`.sddu/.../build.md`（§32 + D-141~D-145）。

**验收标准**: `test/real-screenshot.test.ts` 覆盖提供者路径选择（已授权→真实 / 未授权→回退+如实标注 / 受限页→捕获前拒绝 / element rect+裁剪 / capture 失败→降级）+ **透传断言**（`ops.waitFor`/`ops.extractData` 与 base 同引用，`wait`/`extract`/`export` 仍注册）+ D4 文案断言（插件暴露 chrome `description`/`help` **不再包含**「不可承载」），全绿；`test:e2e` 真实断言走 `captureVisibleTab` 路径且得到**真实 PNG**（`isPromise:true` + dataURL 非空），且回退路径标注可见；`test:ui` 136 / `test:hardening` 24 / `test:binding` 125 / `test:parity` 不回归；`tsc --noEmit` 0 error；插件 `npm test` 405 pass / 0 fail、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、manifest 零 diff（无新权限）、无新依赖、无明文 key、无静默失败。

---

### TASK-035: 整页截图（D2 滚动分屏 `captureVisibleTab` 拼接）+ 原生 back/forward（D6 `tabs.goBack/goForward`）（用户要求，v1.12 浏览器能力增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（扩展宿主能力补齐；base 零改动） |
| **前置依赖** | TASK-034（`real-screenshot.ts` 提供者 + `dom-op` 通道 + `captureVisibleTab`）/ TASK-029（浏览器能力补齐） |
| **执行波次** | Wave 27（v1.12 增补） |
| **对应 FR** | FR-051（浏览器能力）/ FR-028（截图；整页）/ FR-027（back/forward）/ FR-008（不可达可读降级）/ FR-005（risk 不放宽）/ NFR-002（无新依赖/无新权限） |
| **ADR** | ADR-003（截图输出策略不变）/ ADR-008（executor 零 DOM 触碰）/ ADR-016（扩展宿主生态位）/ 新 D-146~D-150（见 build.md §33.3） |
| **TB 映射** | —（非 plan TB，用户要求「整页截图 + 原生 back/forward 都在插件侧 ops 包装层，base 零改动」驱动） |

**描述**: 同一插件侧 ops 包装层（`src/platform/real-screenshot.ts` 的透传 `Proxy`）在既有 `screenshot` 覆盖之外再覆盖 `historyNav`，并实现两件事（**`packages/web-cli-base/**` 零改动、`manifest.json` 零 diff、零新权限、无新依赖）：① **D2 整页截图**：`mode=fullpage` 且可捕获时**滚动分屏拼接**——经页面上下文（`dom-op` 扩展 `content-script.ts` 的 `wcliFullpageMetrics`/`wcliScrollTo`）取文档尺寸/视口/滚动位置，逐屏滚动 + `captureVisibleTab` + SW `OffscreenCanvas` 合成整页 PNG；**必须遵守 `MAX_CAPTURE_VISIBLE_TAB_CALLS_PER_SECOND`（2 次/秒）** → 相邻捕获起始间隔 ≥500ms，命中限流按 `[700,1500]ms` **有界退避重试**；**有上限**（`MAX_FULLPAGE_SCREENS=20` / `MAX_FULLPAGE_PIXELS=40MP`）→ 超出**捕获前**可读拒绝并给出已捕获范围，**不得无界抓取**；无论成功/失败/超限（捕获后）**始终恢复原滚动位置**，恢复失败如实披露；成功输出**必须**写出 `像素路径：真实像素（captureVisibleTab ×N 屏拼接）` + 已知局限（`position:fixed`/sticky 每屏重复、懒加载可能未加载、动画/轮播状态不一致），**不得宣称「完整/无损整页」**。因 base 执行器对 `fullpage` **先行短路**，fullpage 命令与输出/下载策略由插件层 `chrome-host.ts` 接管（复用 base **已导出**的 `summarizeScreenshotData`/`screenshotFilename`/`translateCapabilityError`）。② **D6 原生 back/forward**：`historyNav(±1)` 优先 `chrome.tabs.goBack/goForward(tabId)`（标签页级、跨导航可靠），失败（无权限/无历史/受限页）**回退** base 页面 `history.back/forward`，输出**如实标注实际路径**（`原生（tabs.goBack/goForward）` vs `页面 history（回退，原因：…）`）；原生导航可能**离开绑定 origin** → 输出版本可读说明「原 origin 会话/授权不适用于新站点、导航后重新探测、未授权需点图标」，**不得静默把会话带到别的站点**；risk 沿用 base `ui` **不放宽**。**透传纪律**：包装只覆盖 `screenshot`/`historyNav`，其余方法与属性（`waitFor`/`extractData`/`printPage`/`reloadPage`…）全透传（断言加强，防静默摘掉 `wait`/`extract`/`export`）。

**涉及文件**: MODIFY `src/platform/real-screenshot.ts`（fullpage 算法 + 常驻/退避 + D6 `historyNav` 覆盖 + 诚实标注）、`src/tools/chrome-host.ts`（fullpage 命令接管 + 交付策略 + 文案/schema 修正）、`src/content/content-script.ts`（`wcliFullpageMetrics`/`wcliScrollTo`）、`src/background/service-worker.ts`（整页几何/滚动 deps + `hostHistoryNavViaTabs`）、`docs/dev.md`（§13.7 更新 + §13.8）、`docs/compliance.md`（§11 增补）、`test/real-screenshot.test.ts`、`test/browser-tools.test.ts`、`test/e2e/fullchain.mjs`（整页 + back/forward 真机断言）；NEW `test/fullpage-screenshot.test.ts`；`.sddu/.../build.md`（§33 + D-146~D-150）。

**验收标准**: `test/fullpage-screenshot.test.ts` 覆盖屏数计算（`ceil(h/vh)`）/ 节流（`MIN=1000/MAX`，屏间 ≥500ms）/ 限流有界退避重试 / 超屏数与超像素**捕获前**可读拒绝 + 已捕获范围 / 中途失败报已捕获范围仍恢复滚动 / 恢复失败披露 / 未授权·几何失败·缺少 seam 拒绝；D6 原生优先标注 / 离开 origin 披露 / 原生失败回退带原因 / 无 seam·无 tab·非 http 回退 / 无 base `historyNav` 可读；**透传断言**（仅 `screenshot`/`historyNav` 被覆盖，其余与 base 同引用）——全绿；`test:e2e` 真机断言「≥2 屏页面 → 拼接成功（`×N 屏` 且合成尺寸 > 单屏）+ 局限声明可见 + 滚动位置已恢复」与「back/forward 原生优先 + 回退路径标注可见（`history.length≥2` 佐证）」；`test:ui` 136 / `test:hardening` 24 / `test:binding` 125 / `test:parity` 不回归；`tsc --noEmit` 0 error；插件 `npm test` 425 pass / 0 fail、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、manifest 零 diff（无新权限）、无 `<all_urls>`、无静态 `content_scripts`、无新依赖、无明文 key、无静默失败。

---

### TASK-036: events 补齐运行时 6 子命令（pause/resume/clear/budget/switch/pull-sensitive）（作者指出「工具面已对齐、运行时只有 4 个可用」，v1.13 运行时补齐增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（运行时补齐 / 事件通道 op 扩展；base 零改动） |
| **前置依赖** | TASK-029（`events` 工具面基线对齐 + content 事件桥 subscribe/pull/unsubscribe/status） |
| **执行波次** | Wave 28（v1.13 增补） |
| **对应 FR** | FR-021（事件消费）/ FR-051（运行时对齐）/ FR-005（risk 不放宽）/ FR-006（`pull-sensitive` untrusted 门禁）/ FR-008（不可达可读降级）/ NFR-002（无新依赖/无新权限） |
| **ADR** | ADR-016（扩展宿主生态位）/ 新 D-151~D-155（见 build.md §34.3） |
| **TB 映射** | —（非 plan TB，作者「唯一『工具面已对齐、运行时未对齐』缺口」判断驱动） |

**描述**: `events` 工具面在 TASK-029 已对齐基线 **11 子命令**，但**运行时只有 4 个桥操作可用**（`subscribe`/`pull`/`unsubscribe`/`status`，`list` 本地跟踪），其余 6 个（`pause`/`resume`/`clear`/`budget`/`switch`/`pull-sensitive`）在 `src/tools/remote-events.ts` 一律返回可读「暂不支持」。本轮**在插件侧补齐运行时**（`packages/web-cli-base/**` 零改动、`manifest.json` 零 diff、零新权限、无新依赖）：① 事件桥 op 集 4→10（`src/content/page-bridge.ts` 的 `WebCliEventOp` + `PageEventBridge` 方法）；② `src/tools/remote-events.ts` **删除 hardcoded 拒绝**——6 方法逐字转发页面 `env.events` hub，页面不支持时其**具体原因**原样透出（不假装成功，不泛泛「暂不支持」），订阅摘要以页面 `status` 为准；③ `src/content/content-script.ts` / `src/platform/browser-env.ts` / `service-worker.ts` 类型面随桥扩展（转发路径不变）。**语义忠于 base**（入参名/返回结构/字段/单位逐条对齐）；**risk 档不放宽**（仍由 base `createEventsToolEntry` 单一来源：`pull-sensitive`=`write`，未显式 `--trusted true` 时**不转发**，无明细返回 FR-006 特定原因且插件零缓存/零审计明文；控制类=`state`）。站点页面桥需实现这些 op（LGDL 可复用 base `createBrowserEventHub()`）；夹具 `test/fixtures/site/rpc.js` 提供**无依赖参考实现 + 合成观察源**供真机 e2e。

**涉及文件**: MODIFY `src/content/page-bridge.ts`（op 集 4→10 + `PageEventBridge` 方法）、`src/tools/remote-events.ts`（删除 hardcoded 拒绝 + 6 方法逐字转发 + 页面 status 同步 + `pull-sensitive` 特定原因/来源标注）、`src/content/content-script.ts`（op 逐字转发注释/类型面）、`src/platform/browser-env.ts`（`eventRequest` op 类型）、`test/content.test.ts`、`test/browser-tools.test.ts`、`test/fixtures/site/rpc.js`（参考事件 hub）、`test/e2e/fullchain.mjs`（13 条真机断言）、`docs/dev.md`（§16 + 变更记录）、`docs/capability-matrix.md`（第 28 行 + §4.1）；NEW `test/remote-events.test.ts`；`.sddu/.../build.md`（§34 + D-151~D-155）。

**验收标准**: 单测（node，注入式有状态桥）逐条——pause 后不投递 / resume 补投 / clear 清空 / budget 生效 / switch 切通道；页面不支持时断言**具体原因文案**；risk 档断言未放宽（`pull-sensitive`=`write`、控制类=`state`、`subscribe/list/status/pull`=`read`）；`pull-sensitive` 未 trusted 不转发 / trusted 转发并标注敏感来源 / deny 不转发 / 无明细给出特定原因；`page-bridge` 逐字转发 6 新 op（参数形状）——全绿。`test:e2e`（真 dist + 真实 Chromium）真机断言 subscribe/switch/pull（增量>0）/ pause+clear→pull（增量 0）/ resume→pull（增量>0）/ budget / `pull-sensitive` 门禁 / 无明细原因 / unsubscribe。既有 4 子命令零回归；`tsc --noEmit` 0 error；插件 `npm test` 435 pass / 0 fail、`test:ui`/`test:hardening`/`test:binding` 不回归、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、manifest 零 diff、无 `<all_urls>`、无静态 `content_scripts`、无新依赖、无明文 key、无静默失败。

---

### TASK-037: 标签页工具补齐 mute/pin/move + 放开 close（作者裁决反转 FR-049，2026-09-13；v1.14 增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（能力补齐 + 需求变更处置；base 零改动 / **零新权限**） |
| **前置依赖** | TASK-026（`tabs` 权限 + 插件级 `tabs` list/switch/open + 隐私开关） |
| **执行波次** | Wave 29（v1.14 增补） |
| **对应 FR** | FR-049（`tabs` 修订）/ **FR-053（需求变更记录）** / EC-026 / NFR-002（零新权限）/ NFR-001（审计零明文、不静默） |
| **ADR** | ADR-015（修订，v1.5）+ 新 D-156~D-160（见 build.md §35.3） |
| **TB 映射** | —（非 plan TB，作者 2026-09-13 显式撤销约束驱动） |

**描述**: `tabs` 初版明确「不做 close」；作者 2026-09-13 **明确裁决反转**——「标签页读写」包含 close、「**完全放开 close**」，并补齐单标签页写操作 `mute`/`pin`/`move`。这是作者**显式撤销此前约束**导致的**需求变更**，**不是测试降级**（`spec.md` v1.9 增补 FR-049/FR-053 + EC-026；`plan.md` v1.5 修订 ADR-015 + 安全处置；`docs/dev.md` §12.4 / `docs/compliance.md` §9.4 / `docs/capability-matrix.md` / `docs/release.md` 删除矛盾陈述）。实现：① `src/tools/tabs-tools.ts` 子命令 3→7（`mute`/`pin`/`move`/`close`）+ `subcommandRisks`（新增全 `write`；close 恒 `write`→默认 `ask`，**不放宽**）+ 入参解析（`--muted`/`--pinned true|false`；`--index`/`--window` 非负整数；`--id`/`--match` 二选一）+ `resolveTabTarget`（switch 偏好激活页；改页/关页歧义不猜）+ `resolveSingleTarget`（**禁批量** `--all` 可读拒绝）+ `redactTabTitle`（URL 型标题去 query/fragment）；② `src/background/service-worker.ts` `createTabsDeps` 新增 `muteTab`/`pinTab`/`moveTab`/`closeTab`/`describeTarget`（真实 `chrome.tabs.update/move/remove`；**零新权限**）；③ `src/security/confirm.ts` 新增 `describe` 接缝（ask 前并入「目标标题 + 去参 URL +（close）不可逆/侧栏自关」，best-effort 不阻断）；④ 旧「断言无 close」测试**替换**为「close 存在 + write→ask + 确认/审计/单标签页」（断言总数只增不减）。

**涉及文件**: MODIFY `src/tools/tabs-tools.ts`、`src/background/service-worker.ts`、`src/security/confirm.ts`、`src/ui/options/index.html`、`src/ui/settings/panel.ts`、`src/ui/settings/view.ts`、`test/tabs-tools.test.ts`、`test/tabs-wiring.test.ts`、`test/security.test.ts`、`test/ui/journey.mjs`、`test/ui/binding.mjs`、`docs/dev.md`、`docs/compliance.md`、`docs/capability-matrix.md`、`docs/release.md`、`test/parity/waivers.json`（`tabs` 豁免理由文案）；`.sddu/.../spec.md`（v1.9）、`plan.md`（v1.5）、`build.md`（§35 + D-156~D-160）、`tasks.md`（本任务）、`state.json`。**未改** `packages/web-cli-base/**`、`manifest.json`。

**验收标准**: 单测——`mute`/`pin`/`move`/`close` 入参校验（缺/非法 `--muted`/`--pinned`/`--index`/`--window`）、risk 档（close/move 为 `write`→ask）、`--id`/`--match` 二选一、**拒绝批量**（`--all`）、受限页/未知 id 可读拒绝、审计零明文（URL 与 URL 型标题去参）、隐私开关关闭时 `tabs` 从 `deriveTools()` 消失、**旧「无 close」断言已替换为「close 存在 + ask + 审计」**；`describe` enrich 与异常不阻断。`test:ui`：面板内确认流程覆盖 close（**不可逆提示可见** + 拒绝可结算）。`test:binding`（真站点 + 真扩展）：在**测试自建标签页**上真实 `mute`/`pin`/`move`/`close` + 审计断言 + `close --all` 可读拒绝；**既有 125 断言保留**（只增不减）。`tsc --noEmit` 0 error；插件 `npm test` 452 pass / 0 fail、`test:ui` 141、`test:hardening` 24、`test:binding` 143、`test:e2e` PASS、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、**manifest 零 diff（零新权限）**、无 `<all_urls>`、无静态 `content_scripts`、无新依赖、无明文 key、无静默失败。

---

### TASK-038: 可选权限能力——书签（读+写，写侧 ask，删除破坏性）+ 下载记录（只读）（作者裁决 2026-09-13；v1.15 增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（新能力 + 权限面扩张；base 零改动 / **静态 `permissions` 零新增**） |
| **前置依赖** | TASK-029（对账门禁 + `pluginExtras` 登记）/ TASK-030（`auto-authorize` 硬底线接缝） |
| **执行波次** | Wave 30（v1.15 增补） |
| **对应 FR** | FR-054（书签 / 下载记录，作者裁决 2026-09-13；**spec.md 条文待 @sddu-spec 补登**）/ NFR-002（权限最小面）/ NFR-001（审计零明文、不静默）/ FR-052 + ADR-017（破坏性写硬底线） |
| **ADR** | ADR-017（自动授权，「硬底线 4」判定前移）+ 新 D-161~D-165（见 build.md §36.3） |
| **TB 映射** | —（非 plan TB，作者 2026-09-13 批准新能力驱动） |

**描述**: 作者批准新增**书签**（读 + 写，写侧一律 `ask`，删除为**破坏性**永不自动放行）与**下载记录**（**只读**，明确不做取消/暂停/删除/打开）。关键约束：以 **`optional_permissions`** 声明（**不得**进静态 `permissions`——静态新增会在扩展更新时使已安装扩展被 Chrome 停用直到重新同意），首次使用才在**扩展页面手势内**请求（`chrome.permissions.request` 在 SW 内无手势必失败/挂起）。实现：① `manifest.json` 仅新增 `optional_permissions: ["bookmarks","downloads"]`；② `src/platform/capability-permissions.ts`（`hasCapabilityPermission` fail-safe false / `requestCapabilityPermissionOnGesture` 手势内同步请求 / `changeTouchesCapability` 变更判据）；③ `src/tools/bookmarks-tools.ts`（`list/search/tree`=read 默认开；`add/remove/move`=write 默认关、ask；`remove` destructive + 禁批量 + 仅 http(s) add；未授权可读拒绝；审计零明文）；④ `src/tools/downloads-tools.ts`（只读 `list/search`；cancel/pause/erase/open 可读拒绝）；⑤ `src/background/capability-setting.ts`（默认读开/写关）；⑥ `host.ts` 注册/卸载 + `suppressCapability` + `isPluginDestructiveInvocation`；⑦ `auto-authorize.ts` 硬底线 4 **前移到 group 过滤之前**（插件级破坏性写同样不纳入「写操作自动」）；⑧ `service-worker.ts` 真实 chrome deps + `capabilities` 消息 + `permissions.onAdded/onRemoved` 对账；⑨ UI（侧栏设置视图 + options 兜底页）按钮 + 开关 + 状态回显 + 拒绝路径可读；⑩ `test/parity/waivers.json` 登记 `pluginExtras`。

**涉及文件**: NEW `src/platform/capability-permissions.ts`、`src/background/capability-setting.ts`、`src/tools/bookmarks-tools.ts`、`src/tools/downloads-tools.ts`、`test/bookmarks-tools.test.ts`、`test/downloads-tools.test.ts`、`test/capabilities.test.ts`、`test/capability-wiring.test.ts`；MODIFY `manifest.json`、`src/background/{host,messaging,service-worker}.ts`、`src/security/{auto-authorize,audit-sink}.ts`、`src/ui/settings/{view,ops,panel}.ts`、`src/ui/options/{index.html,options.ts}`、`test/parity.test.ts`、`test/parity/waivers.json`、`test/ui/{journey,binding}.mjs`、`test/e2e/fullchain.mjs`、`docs/{compliance,dev,release,capability-matrix}.md`；`.sddu/.../build.md`（§36 + D-161~D-165）、`tasks.md`（本任务）、`state.json`。**未改** `packages/web-cli-base/**`、`spec.md`、`plan.md`。

**验收标准**: 单测——risk 档（读 read / 写 write）、未授权可读拒绝、开关关闭时工具从 `deriveTools()` 移除且派发被拒、`remove` **破坏性硬底线**（「写操作自动」开仍 ask）、`--all` 批量可读拒绝、`add` 仅 http(s)、审计零明文、`permissions.onAdded/onRemoved` 对账（撤销即移除 / 重授即按开关注册）。`test:ui`：能力分区 + 未授权回显 + 默认读开写关 + 拒绝路径可读 + 点击真实调用 `request`。`test:binding`/`test:e2e`：**测试副本 manifest** 把二者声明为静态权限（已披露）以证明「权限在时能力真实可用」+ 真实 `bookmarks remove` 写自动仍弹确认；**手势授权弹窗仍归人工面，不冒充 PASS**。`tsc --noEmit` 0 error；插件 `npm test` **488 pass / 0 fail**、`test:ui` **149**、`test:hardening` 24、`test:binding` **153**、`test:e2e` PASS、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、**静态 `permissions` 零新增（仅 `optional_permissions` +2）**、无 `<all_urls>`、无静态 `content_scripts`、`host_permissions`/`minimum_chrome_version` 不变、无新依赖、无明文 key、无静默失败。

---

### TASK-039: 可选权限能力——系统通知（`notify`，读+写）+ 剪贴板（`clipboard`，读+写，读侧默认关）（作者裁决 2026-09-13；v1.16 增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | L |
| **类型** | 🛠 实施（新能力 + 权限面扩张；base 零改动 / **静态 `permissions` 零新增**） |
| **前置依赖** | TASK-038（`optional_permissions` 手势请求机制 + 隐私开关 + 撤销对账）/ TASK-029（对账门禁）/ TASK-030（`auto-authorize` 硬底线接缝） |
| **执行波次** | Wave 31（v1.16 增补） |
| **对应 FR** | FR-055（通知 / 剪贴板，作者裁决 2026-09-13；**spec.md 条文待 @sddu-spec 补登**）/ NFR-002（权限最小面）/ NFR-001（审计零明文、不静默）/ FR-052 + ADR-017（自动授权硬底线） |
| **ADR** | ADR-017（自动授权，`state`/`evaluate` 永不自动）+ 新 D-168~D-172（见 build.md §37.7） |
| **TB 映射** | —（非 plan TB，作者 2026-09-13 批准新能力驱动） |

**描述**: 作者批准新增**系统通知**（读 + 写）与**剪贴板**（读 + 写；**读侧隐私开关默认关**，写侧默认开）。关键约束：沿用第 5 轮 **`optional_permissions` + 扩展页手势内用时请求**模式（**不得**进静态 `permissions`）；**先核实** `notifications`/`clipboardRead`/`clipboardWrite` 在目标 Chromium 是否都允许 optional（实测通过；对照 `debugger`/`proxy`/`geolocation`/`declarativeNetRequest` 不可 optional）。实现：① `manifest.json` 仅 `optional_permissions` +3（`notifications`/`clipboardRead`/`clipboardWrite`）；② `src/tools/notify-tools.ts`（插件侧 `chrome.notifications`：`list`/`send`/`clear`；保留基线 `send`；单条 clear、禁批量；内容零审计明文）；③ `src/tools/clipboard-tools.ts`（纯文本 `read`/`write`，读=`state` 档、**永不自动放行**；`write-html`/`write-image`/`paste-read` 裁剪为可读「未实现」；内容零审计明文）；④ `src/platform/clipboard-page.ts`（扩展页 `navigator.clipboard` + `document.execCommand` 回退，侧栏与 options 共用；SW 经 `clipboard-op` 转发 + 扩展页 `sendResponse`）；⑤ `capability-permissions.ts` / `capability-setting.ts`（默认通知开 / 剪贴板读关写开）；⑥ `host.ts` 注册/卸载 + `suppressCapability`；⑦ `security/confirm.ts` 新增 `scrubContentArgs`（内容参数不出现在确认摘要/`confirm` 审计）；⑧ `service-worker.ts` 真实 deps + `capabilities` 消息 + `onAdded`/`onRemoved` 对账；⑨ UI（侧栏设置视图 + options 兜底页）按钮 + 开关 + 状态回显 + 拒绝路径可读；⑩ `capability-matrix.md` 第 26/27 行由「待批准权限」→「已提供」（`notify`/`clipboard` 属基线同名，**不登记 pluginExtras**）。

**涉及文件**: NEW `src/tools/notify-tools.ts`、`src/tools/clipboard-tools.ts`、`src/platform/clipboard-page.ts`、`test/notify-tools.test.ts`、`test/clipboard-tools.test.ts`；MODIFY `manifest.json`、`src/platform/capability-permissions.ts`、`src/background/{host,messaging,service-worker,capability-setting}.ts`、`src/security/{audit-sink,confirm}.ts`、`src/ui/settings/{view,ops,panel}.ts`、`src/ui/options/{index.html,options.ts}`、`src/ui/sidepanel/sidepanel.ts`、`test/{capabilities,capability-wiring,parity}.test.ts`、`test/parity/waivers.json`、`test/ui/{journey,binding}.mjs`、`test/e2e/fullchain.mjs`、`docs/{compliance,dev,release,capability-matrix}.md`；`.sddu/.../build.md`（§37 + D-168~D-172）、`tasks.md`（本任务）、`state.json`。**未改** `packages/web-cli-base/**`、`spec.md`、`plan.md`。

**验收标准**: 单测——`notify` 读/写子命令 + risk + 未授权/开关可读拒绝 + 单条 clear/禁批量 + 审计零明文（title/body 不出现）；`clipboard` 读/写 + risk + **不允许自动授权**（纯决策 + host 集成双断言）+ 裁剪子命令拒绝 + 审计零明文（内容不出现）+ `scrubContentArgs`；开关摘除 + `onRemoved` 撤销后工具消失。`test:ui`：设置视图「开启通知 / 开启剪贴板访问」按钮 + 状态回显 + 拒绝路径 + **剪贴板读默认关**回显（断言数只增不减）。`test:binding`/`test:e2e`：**测试副本 manifest** 把它们声明为静态权限（已披露）证「权限在时真实可用」（真实 `chrome.notifications` + 真实扩展页剪贴板写入）；**手势弹窗归人工面，不冒充 PASS**。`tsc --noEmit` 0 error；插件 `npm test` **514 pass / 0 fail**、`test:ui` **155**、`test:binding` **157**、`test:hardening` 24、`test:e2e` PASS、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、**静态 `permissions` 零新增（仅 `optional_permissions` +3）**、无 `<all_urls>`、无静态 `content_scripts`、`host_permissions`/`minimum_chrome_version` 不变、无新依赖、无明文 key、无静默失败。

---

### TASK-040: 可选权限申请 UX 直观化——三态按钮 + 明确回执 + 应用内撤销（用户 UX 反馈 2026-09-13；v1.17 增补）

| 属性 | 值 |
|------|-----|
| **复杂度** | M |
| **类型** | 🛠 实施（UX 修复；base 零改动 / **`manifest.json` 零 diff** / 无新依赖） |
| **前置依赖** | TASK-038（可选权限机制 + 手势请求）/ TASK-039（notify/clipboard）/ TASK-033（设置面板共享模块） |
| **执行波次** | Wave 32（v1.17 增补） |
| **对应 FR** | FR-054 / FR-055（不变；本任务为可用性修复）/ NFR-001（不静默）/ NFR-002（权限最小面） |
| **ADR** | 新 D-173~D-177（见 build.md §38.8） |
| **TB 映射** | —（非 plan TB，用户 UX 反馈驱动） |

**描述**: 作者反馈可选权限申请「不够直观、点没点成效果不明显、缺少取消权限」。**统一适用于全部 5 个能力**（bookmarks 读/写、downloads 只读、notifications、clipboard 读/写）。实现：① 三态按钮——每次渲染在扩展页实测 `chrome.permissions.contains()`（`ops.loadCapabilities → measuredGrants → applyMeasuredGrants`），未授权「授权 Chrome ＜能力＞权限」/ 已授权「撤销 Chrome 权限」+ `✅ 已授权` / 已撤销回到未授权 + 「上次已撤销」说明；**隐私开关不参与授权判定**；② 明确持久回执（`capabilityGrantReceipt`/`capabilityDeniedReceipt`/`capabilityRevokeReceipt`/`capabilityRevokeFailureReceipt`/`capabilityStateNote`，侧栏 + options 共用），按钮与回执同步切换；③ 每能力解释文案（做什么 + 为什么 Chrome 弹窗）；④ 应用内撤销 `chrome.permissions.remove`（**无需手势**）→ `permissions.onRemoved` + `capabilities/permission-changed` 双路对账 → `suppressCapability(cap,true)` 使工具即时离开 `deriveTools()` + 派发被拒 + 审计；撤销后隐私开关回落「未授予」语义（不勾选 + 禁用）；新增 `capability-changed` 推送使面板/选项页就地刷新（无需重开）。**不新增权限**（`manifest.json` 零 diff）、不新增依赖、base 零改动；侧栏设置视图（主）与 options（兜底）共用 `src/ui/settings/{view,ops,panel}.ts`，未分叉。

**涉及文件**: MODIFY `src/ui/settings/{view,ops,panel,styles}.ts`、`src/ui/options/{index.html,options.ts}`、`src/ui/sidepanel/sidepanel.ts`、`src/background/{messaging,service-worker}.ts`、`test/{capabilities,capability-wiring}.test.ts`、`test/ui/{journey,binding}.mjs`、`docs/{dev,compliance}.md`；NEW `test/capability-revoke.test.ts`；`.sddu/.../build.md`（§38 + D-173~D-177）、`tasks.md`（本任务）、`state.json`。**未改** `packages/web-cli-base/**`、`manifest.json`、`spec.md`、`plan.md`。

**验收标准**: 单测——三态渲染（未授权/已授权/已撤销）、`remove` 调用路径（精确权限集 + `permission-changed` 对账）、成功/拒绝/撤销失败的可读回执、**「实测权限优先于本地开关」**（开关=开但 `contains=false` → 必须显示未授权，且不显示「读开关 开」）、工具随权限增删（`suppressCapability` 使 4 工具进出 `deriveTools()`）。`test:ui`：按钮文案切换（「授权…」↔「撤销 Chrome 权限」）、回执文案、**撤销后工具消失**（真实 background `capabilities.tools` 不含 bookmarks）；断言数只增不减。`test:binding`（真 dist）：真实 `permissions.remove` → 工具从工具面移除 + `optional-permission/revoked` 审计；`request` 弹窗仍归人工面（如实披露）。`tsc --noEmit` 0 error；插件 `npm test` **523 pass / 0 fail**、`test:ui` **167**、`test:binding` **163**、`test:hardening` 24、`test:e2e` PASS、全仓 `npm run build` + `npm test` 0 fail（base **483 零回归**）；红线：base 零改动、`manifest.json` 零 diff（不新增权限）、无新依赖、无明文 key、无静默失败。
...
| 统计项 | 数值 |
|--------|:--:|
| 总任务数 | 16（15 核心 + 1 可选后置 TASK-015/TB-Q）+ 7 post-validate 增补（TASK-017 UI 修复 / TASK-018 options 加固+测试连接 / TASK-019 三成因加固+诊断 / TASK-020 保存后呈现+无活跃站点自救+Key/测试连接可见 / TASK-021 工具名非法字符修复+附带疑点查清 / TASK-022 侧栏 Markdown 渲染+消息样式 / TASK-023 侧栏整体 UI/UX 重做，审查与实测反馈驱动）+ 2 v0.9 架构级增补（TASK-024 自动探测 / TASK-025 多会话，作者 2026-09-12 决策①②）+ 1 v0.9 权限扩张增补（TASK-026 标签页管理 `tabs`，作者 2026-09-12 决策③）+ 3 v0.9 增补（TASK-027 `web-fetch` 权限边界预校验 + 失败可见，用户实测 CORS 驱动 / TASK-028 侧栏自动测试当前模型配置 + 移除独立按钮 + 60s TTL 缓存，作者要求 / TASK-029 工具面基线对账门禁 + 浏览器能力补齐，作者实测 dom·浏览器截图丢失驱动）+ 1 v1.8 缺陷修复增补（TASK-031 切 tab 按 `tab.url` 驱动会话跟随，用户实测「新域名 tab 不自动建会话」驱动；TASK-030 见 build.md §28）+ 1 v1.9 体验改造增补（TASK-032 探测改为全自动，用户要求「逻辑上不需要用户手动探测」驱动）+ 1 v1.10 体验改造增补（TASK-033 设置面板改为侧栏内视图 + 设置逻辑抽共享模块 + options 降级兜底，用户诉求「不建议跳到 options.html」驱动）+ 1 v1.11 浏览器能力/文案增补（TASK-034 chrome screenshot 真实像素 + 插件层修正 chrome help 过时绝对表述，用户实测驱动）+ 1 v1.12 浏览器能力增补（TASK-035 整页截图 D2 滚动分屏 captureVisibleTab 拼接 + 原生 back/forward D6 tabs.goBack/goForward，用户要求驱动）+ 1 v1.13 运行时补齐增补（TASK-036 events 补齐运行时 6 子命令，作者「工具面已对齐、运行时只有 4 个可用」驱动）+ 1 v1.14 反转增补（TASK-037 标签页工具补齐 mute/pin/move + 放开 close，作者 2026-09-13 显式撤销「不做 close」约束驱动；**需求变更**）+ 1 v1.15 可选权限增补（TASK-038 书签〔读+写，写侧 ask，删除破坏性〕+ 下载记录〔只读〕，作者 2026-09-13 批准；以 `optional_permissions` 声明、静态 `permissions` 零新增、手势内按需请求、可按能力撤销） + 1 v1.16 可选权限增补（TASK-039 系统通知〔读+写〕+ 剪贴板〔读+写，读侧默认关〕，作者 2026-09-13 批准；沿用 `optional_permissions` 声明、静态 `permissions` 零新增、手势内按需请求、可按能力撤销；先实测核实三者均可 optional） |
| S 级 (简单) | 0 |
| M 级 (中等) | 9（001/002/003/007/008/009/012/013/015）+ 6 增补（017/018/019/020/021/022）+ 2 增补（027/028）+ 1 缺陷修复（031）+ 1 体验改造（032） |
| L 级 (复杂) | 7（004/005/006/010/011/014/016）+ 1 增补（023）+ 3 v0.9（024/025/026）+ 1 增补（029）+ 1 v1.10 增补（033）+ 1 v1.12 增补（035）+ 1 v1.13 增补（036）+ 1 v1.14 反转增补（037）+ 1 v1.15 可选权限增补（038） |
| 执行波次 | 29（Wave 0~8 + Wave 9 TASK-017 + Wave 10 TASK-018 + Wave 11 TASK-019 + Wave 12 TASK-020 + Wave 13 TASK-021 + Wave 14 TASK-022 + Wave 15 TASK-023 + Wave 16 TASK-024 + Wave 17 TASK-025 + Wave 18 TASK-026 v0.9 增补 + Wave 19 TASK-027 v0.9 缺陷修复 + Wave 20 TASK-028 v0.9 增补 + Wave 21 TASK-029 工具面对账+浏览器能力补齐 + Wave 22 TASK-030 自动授权 + Wave 23 TASK-031 切 tab 会话跟随缺陷修复 + Wave 24 TASK-032 探测全自动改造 + Wave 25 TASK-033 设置面板改为侧栏内视图 + Wave 26 TASK-034 chrome screenshot 真实像素 + chrome help 过时表述修正 + Wave 27 TASK-035 整页截图（D2）+ 原生 back/forward（D6）+ Wave 28 TASK-036 events 运行时补齐（D3）+ Wave 29 TASK-037 标签页补齐 mute/pin/move + 放开 close（D5，作者裁决反转）+ Wave 30 TASK-038 可选权限：书签（读+写，删除破坏性）+ 下载记录（只读）+ Wave 31 TASK-039 可选权限：系统通知（读+写）+ 剪贴板（读+写，读侧默认关）） |
...
| v2.4 | 追加 **TASK-033 设置面板改为侧栏内视图**（FR-017/033/035/036/046/047/049/052，v1.10 体验改造增补，非 plan TB，用户诉求「不建议跳到 `chrome-extension://…/options.html`，无法很方便回去，直接在拓展当前页面展示」驱动）。任务汇总/波次计入增补轮（Wave 25，L 级）；侧栏新增 `#settings-view`（`#open-settings`/`#settings-back` 同文档显隐切换，`view-switch.ts` 保留滚动位置与草稿，不重建 DOM）；**移除 `openOptionsPage()` 主入口**；设置逻辑抽共享模块 `src/ui/settings/`（`view.ts`/`ops.ts`/`panel.ts`/`view-switch.ts`/`styles.ts` + `diagnostics.ts` 迁入），`options.html` 保留为复用同一逻辑的兜底页（功能不退化、静态 DOM 零回归）；面板覆盖 LLM/自动授权/tabs/会话分组/诊断/合规迁移，窄屏纵向堆叠；顺带修复 TASK-032 探针推送覆盖「已授权」回执（`chat-state.ts` 失效跃迁提示；binding #4b/#4c FAIL→PASS）。**base 零改动 / manifest 零 diff（`options_page` 保留）/ 无新依赖 / 无新权限**；新增 `test/settings.test.ts`（15）、插件 373→389、`test:ui` 121→136（#33a~#33o，既有断言零删除）、`test:binding` 114→125（#33B1~#33B11，保留既有）、`test:hardening` 24、`test:e2e` A/B PASS、全仓 0 fail（base 483 零回归）。D-136~D-140 见 build.md §31.3。 | 2026-09-12 | SDDU Build Agent |
| v2.5 | 追加 **TASK-034 chrome screenshot 走真实像素（`captureVisibleTab`，插件侧提供者）+ 插件层修正 chrome help 过时绝对表述**（FR-051/FR-028/FR-008，v1.11 浏览器能力/文案增补，非 plan TB，用户实测「插件已是扩展宿主，应得真实像素；chrome help 的『书签不可承载』过时且误导」驱动）。任务汇总/波次计入增补轮（Wave 26，L 级）；插件侧新增 `src/platform/real-screenshot.ts`（透传 `Proxy` 只覆盖 `env.dom.ops.screenshot`，base 零改动）：已授权 http(s) origin → `chrome.tabs.captureVisibleTab` 真实像素（element 经 `dom-op` 取 rect + SW `OffscreenCanvas` 裁剪）；未授权/受限/失败 → 回退 base 近似并**如实标注**实际路径与原因（绝不谎称真实）；`fullpage` 仍「不支持」（D2 单独一轮）；`src/tools/chrome-host.ts` 插件层包装 chrome `description`/`help` 删除「不可承载」绝对表述。**实测**：MV3 SW 下 `captureVisibleTab` 返回 Promise 且需 `activeTab`/`<all_urls>`（仅 host 权限不足）——插件已声明 `activeTab`（图标手势授予）→ 零新权限。**base 零改动 / manifest 零 diff / 无新依赖 / 无明文 key / 无静默失败**；新增 `test/real-screenshot.test.ts`（16）+ browser-tools D4/透传断言、插件 389→405、`test:e2e` 新增真实 PNG + 两条路径标注断言（临时 dist `<all_urls>` 偏差披露）、`test:ui` 136、`test:hardening` 24、`test:binding` 125、全仓 0 fail（base 483 零回归）。D-141~D-145 见 build.md §32.3。 | 2026-09-13 | SDDU Build Agent |

| v2.6 | 追加 **TASK-035 整页截图（D2 滚动分屏 `captureVisibleTab` 拼接）+ 原生 back/forward（D6 `tabs.goBack/goForward`）**（FR-051/FR-028/FR-027/FR-008/FR-005/NFR-002，v1.12 浏览器能力增补，非 plan TB，用户要求「两者都在插件侧 ops 包装层，base 零改动、manifest 零改动、无新权限、无新依赖」驱动）。同一包装层（`src/platform/real-screenshot.ts` 透传 `Proxy`）新增覆盖 `historyNav`；因 base 执行器对 `fullpage` **先行短路**，fullpage 命令与输出/下载策略由 `chrome-host.ts` 接管（复用 base 已导出 `summarizeScreenshotData`/`screenshotFilename`/`translateCapabilityError`）：`ceil(h/vh)` 屏滚动 + `captureVisibleTab` + SW `OffscreenCanvas` 拼接；**遵守 2 次/秒**（500ms 节流 + `[700,1500]ms` 有界退避重试）、**上限**（≤20 屏 / ≤40MP，捕获前可读拒绝 + 已捕获范围）、**始终恢复滚动**（失败如实披露）；成功输出必含 `像素路径：真实像素（captureVisibleTab ×N 屏拼接）` + 局限声明（fixed/sticky 每屏重复·懒加载·动画/轮播不一致，**非「完整/无损」**）；D6 优先原生 `chrome.tabs.goBack/goForward`（失败回退页面 `history` 并标注路径；离开绑定 origin 可读说明授权/会话影响）；risk 沿用 base `ui` 不放宽。**base 零改动 / manifest 零 diff / 无 `<all_urls>` / 无静态 `content_scripts` / 无新依赖 / 无新权限 / 无明文 key / 无静默失败**；新增 `test/fullpage-screenshot.test.ts`（20）+ real-screenshot/browser-tools 断言加强、插件 405→**425**、`tsc --noEmit` 0 error、`test:e2e` 新增整页真机断言（≥2 屏 + 合成尺寸 > 单屏 + 局限可见 + 滚动已恢复）与 back/forward 原生优先 + 回退标注断言（headless Chrome 151 `tabs.goBack` 非功能已披露；原生成功由单测注入 host-nav 覆盖）、`test:ui` **136**、`test:hardening` **24**、`test:binding` **125**、全仓 build/test **0 fail**（base **483 零回归**）。D-146~D-150 见 build.md §33.3 + docs dev §13.8/compliance §11。 | 2026-09-13 | SDDU Build Agent |
| v2.7 | 追加 **TASK-036 events 补齐运行时 6 子命令（pause/resume/clear/budget/switch/pull-sensitive）**（v1.13 运行时补齐增补，非 plan TB，作者「工具面已对齐、运行时只有 4 个可用」判断驱动）。任务汇总/波次计入增补轮（Wave 28，L 级）；插件侧扩展事件桥 op 集（`page-bridge.ts` 4→10）+ `remote-events.ts` 删除 hardcoded 拒绝、6 方法逐字转发页面 `env.events` hub（页面不支持时透出其**具体原因**）；risk 沿用 base 不放宽（`pull-sensitive`=`write` 未 trusted 不转发、无明细给 FR-006 特定原因、插件零缓存/零审计明文）。新增 `test/remote-events.test.ts`（9）；`test/e2e` 新增 13 条真机断言（夹具站点参考事件 hub）。插件 425→**435**、`tsc` 0 error、`test:ui` **136** / `test:hardening` **24** / `test:binding` **125** / `test:e2e` **PASS** / 全仓 build+test **0 fail**（base 483 零回归）；**base 零改动 / manifest 零 diff / 无新依赖 / 无新权限**；D-151~D-155 见 build.md §34.3。 | 2026-09-13 | SDDU Build Agent |
| v2.8 | 追加 **TASK-037 标签页工具补齐 mute/pin/move + 放开 close（作者裁决反转 FR-049，2026-09-13）**（v1.14 反转增补，L 级，Wave 29）。作者**显式撤销**初版「明确不做 close」约束——「标签页读写」包含 close、「完全放开 close」，属**需求变更**非测试降级。子命令 3→7；`subcommandRisks` 新增 `mute/pin/move/close='write'`（**不放宽**，默认 ask）；`resolveSingleTarget` 禁批量（`--all` 可读拒绝）、`resolveTabTarget(...,preferActive=false)` 歧义不猜、`redactTabTitle` URL 型标题去参；`service-worker.ts` 新增 `muteTab/pinTab/moveTab/closeTab/describeTarget`（真实 `chrome.tabs.update/move/remove`，**零新权限**）；`confirm.ts` 新增 `describe` 接缝（ask 前并入目标标题 + 去参 URL +（close）不可逆/侧栏自关，best-effort 不阻断）；旧「无 close」断言**替换**为「close 存在 + write→ask + 确认/审计/单标签页」（断言总数只增不减）。门禁：插件 435→**452**、`test:ui` 136→**141**（#16p~#16t）、`test:binding` 125→**143**（#7m~#7t）、`test:hardening` 24、`test:e2e` PASS、全仓 0 fail（base 483 零回归）；**base 零改动 / manifest 零 diff / 无新依赖 / 无新权限**；spec v1.9（FR-049/FR-053+EC-026）/plan v1.5（ADR-015 修订）/build §35（D-156~D-160）同步。 | 2026-09-13 | SDDU Build Agent |
| v2.9 | 追加 **TASK-038 可选权限能力：书签（读+写，写侧 ask，删除破坏性）+ 下载记录（只读）**（v1.15 可选权限增补，L 级，Wave 30，作者 2026-09-13 批准）。以 **`optional_permissions`** 声明（**静态 `permissions` 零新增**——避免扩展更新被 Chrome 停用、可按能力单独撤销）；`chrome.permissions.request` **只在扩展页面点击手势内**发起（`src/platform/capability-permissions.ts`；SW 绝不调用），随后 `capabilities/permission-changed` 重读真实授权并对账，`permissions.onAdded/onRemoved` 即时对账（撤销即从 `deriveTools()` 移除 + `optional-permission` 审计）；默认书签读开/写关、下载记录读开；未授权返回可读「去设置开启」（不静默）。`bookmarks remove` 判定 **destructive** → `auto-authorize` 硬底线（判定**前移到 group 过滤之前**，「写操作自动」开启时删书签仍 ask）+ 禁止批量 + `add` 仅 http(s)；`downloads` **只读**、明确不做取消/删除/打开；审计零明文。新增 `test/{bookmarks-tools,downloads-tools,capabilities,capability-wiring}.test.ts`；`test/parity/waivers.json` 登记 `pluginExtras`；docs dev §17 / compliance §12 / release §6 / capability-matrix §3 同步；build §36（D-161~D-165）。门禁（串行）：`tsc --noEmit` 0 error；插件 452→**488 pass / 0 fail**、`test:ui` 141→**149**（#54a~#54h）、`test:binding` 143→**153**（测试副本静态权限 + 真实 `chrome.bookmarks/downloads` + 写自动仍弹确认 #54B8）、`test:hardening` 24、`test:e2e` **PASS**、全仓 build+test **0 fail**（base 483 零回归）；**base 零改动 / 静态 `permissions` 零新增 / 无新依赖 / 无明文 key / 无静默失败**。 | 2026-09-13 | SDDU Build Agent |
| v3.0 | 追加 **TASK-039 可选权限能力：系统通知（读+写）+ 剪贴板（读+写，读侧默认关）**（v1.16 可选权限增补，L 级，Wave 31，作者 2026-09-13 批准）。沿用 TASK-038 的 **`optional_permissions` + 扩展页手势内用时请求**模式（**静态 `permissions` 零新增**），并**先实测核实** `notifications`/`clipboardRead`/`clipboardWrite` 在目标 Chromium（151）均可 optional（对照 `debugger`/`proxy`/`geolocation`/`declarativeNetRequest` 不可 optional）。`notify` 走插件侧 `chrome.notifications`（base notify 为页内 Notification 面、SW 不可用；子命令 `list`/`send`/`clear`，保留基线 `send`）；`clipboard` 纯文本 `read`/`write` 经扩展页 `navigator.clipboard`（回退 `document.execCommand`；新增 `src/platform/clipboard-page.ts` 供侧栏与 options 共用），`write-html`/`write-image`/`paste-read` 裁剪为可读「未实现」。**剪贴板读 = `state` 档、永不自动放行**（「读操作自动」开仍 ask；纯决策 + host 集成双断言）；剪贴板/通知内容**零审计明文**（新增 `scrubContentArgs`）。默认通知开 / 剪贴板读关写开；关闭即从 `deriveTools()` 移除；`onAdded`/`onRemoved` 对账。`capability-matrix` 第 26/27 行「待批准权限」→「已提供」（汇总待批准 2→0；基线同名，不登记 pluginExtras）。新增 `test/{notify-tools,clipboard-tools}.test.ts` + capabilities/capability-wiring 扩展；`test:ui` 149→**155**（#54i~#54n）、`test:binding` 153→**157**（静态权限副本 + `getPermissionLevel()=granted` + 剪贴板 API 可达 + 真实往返观测成功）、`test:e2e` **PASS**（notify 真机 + clipboard 写入真实成功）。门禁（串行）：`tsc --noEmit` 0 error；插件 488→**514 pass / 0 fail**、`test:hardening` 24、全仓 build+test **0 fail**（base 483 零回归）；**base 零改动 / 静态 `permissions` 零新增 / 无新依赖 / 无明文 key / 无静默失败**；build §37（D-168~D-172）。 | 2026-09-13 | SDDU Build Agent |
| v3.1 | 追加 **TASK-040 可选权限申请 UX 直观化：三态按钮 + 明确回执 + 应用内撤销**（v1.17 UX 修复增补，M 级，Wave 32，用户 UX 反馈 2026-09-13）。作者反馈「申请不直观 / 点没点成不明显 / 缺少取消权限」，修复**统一覆盖全部 5 个可选权限**（bookmarks 读+写、downloads 只读、notifications、clipboard 读+写）。三态按钮以扩展页**实测** `chrome.permissions.contains()` 为准（`applyMeasuredGrants` 覆盖 background 回报；隐私开关不决定授权）——未授权「授权 Chrome ＜能力＞权限」、已授权「撤销 Chrome 权限」+ `✅ 已授权`、已撤销回到未授权 +「上次已撤销」；明确持久回执（授权成功/拒绝/撤销成功/撤销失败，侧栏 + options 共用）；每能力解释「做什么 + 为什么 Chrome 弹窗」；应用内撤销 `chrome.permissions.remove`（无需手势）→ `permissions.onRemoved` + `capabilities/permission-changed` 双路对账 → `suppressCapability` 使工具即时离开 `deriveTools()` + 派发被拒 + `optional-permission/revoked` 审计；撤销后隐私开关回落「未授予」语义；新增 `capability-changed` 推送使面板就地刷新。**不新增权限（`manifest.json` 零 diff）/ 不新增依赖 / base 零改动**；侧栏设置视图与 options 兜底页共用 `src/ui/settings/{view,ops}.ts`，未分叉。门禁（串行）：`tsc --noEmit` 0 error、插件 514→**523 pass / 0 fail**（+9：capabilities +4 + 新增 `test/capability-revoke.test.ts` 5）、`test:ui` 155→**167**（+12，断言只增不减）、`test:binding` 157→**163**（+6 真实 `permissions.remove` + 工具移除 + 审计）、`test:hardening` **24**、`test:e2e` **PASS**、全仓 build+test **0 fail**（base **483 零回归**）；build.md §38 + D-173~D-177；原生弹窗/「已授权→onRemoved」过渡仍归人工面已如实披露。 | 2026-09-13 | SDDU Build Agent |
