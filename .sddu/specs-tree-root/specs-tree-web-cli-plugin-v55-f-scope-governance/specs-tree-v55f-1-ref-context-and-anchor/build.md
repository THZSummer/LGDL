...
| 复杂度分布 | R1：S×2（`102` / `105`）/ M×10 / L×3 · R2：M×13（`116`~`121` / `125`~`129`）/ L×3（`122` / `123` / `124`） |
| 新增文件 | **9 个**（R1 5：`l1/ref-scope.ts` / `background/ref-turn.ts` / `background/ref-context.ts` / `test/ref-context-in-turn.test.ts` / `test/law9-scope-reading.test.ts`；**R2 4**：`src/tools/dom-anchor.ts` / `src/background/ref-observe.ts` / `test/dom-ref-anchor.test.ts` / `test/_spike/sg-sgo-02-probe.mjs`（探毕删除，不入库）） |
| 修改文件 | **21 个**（R1 8：源码 5 + 门禁 3；**R2 13**：源码 2（`browser-tools.ts` / `service-worker.ts` 再改）+ 门禁 11（`test/dom-ref-anchor.test.ts` 新增外：`s0-self-driven-chain.test.ts` / `ui/s0-self-driven.mjs` / `ui/fixtures/s0-chain.mjs` / `supersession-ledger.test.ts` / `op-wiring.test.ts` / `insight-no-escalation.test.ts` / `ui/law8-plaintext.mjs` / `gate-integrity.test.ts` / `size-baseline.ts` / `size-budget.test.ts` / `size-growth-evidence.test.ts` / `size-ruling-vol3.test.ts` / `ref-wiring.test.ts` / `r6-ty-experience-fix.test.ts`）+ 台账 2（`docs/v4-supersession-ledger.json` / `docs/v4-density-baseline.json`）） |
...
| MODIFY | `.../docs/v4-supersession-ledger.json` | **R2** 125 | X-SGO-1~7 台账块（3 已发生 / 4 未发生）+ 2 条显式取代条目（`V55F1-E-237..244` / `V55F1-E-VOL-1`）+ **12 项 `modifiedRanges`** + **第 9 个 `leafBases` 段**（`leafBase 68848af`，逐字登记 47 条删除行）+ 全体 `newTitle` 等价重锚（61 条）+ ⑤三值闭合前移 |
...
| TASK-V55F-125 | X-SGO-1/2/3 台账 + X-SGO-7 未发生登记 + `op-wiring` 复合读数 | M | ✅ completed（7 行：3 已发生 / 4 未发生；12 项 `modifiedRanges`；新叶段逐字登记 47 行） | FR-SGO-100/101/102/106/107/112 |
