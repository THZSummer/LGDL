...
| v1.1 | **订正注（不改写任何 ADR 正文）**：处置 review R2 的 I-11——① **ADR-V4-036 §后果** 的「`#panel-bottom` / 6 strips 容器移除」口径与实现不一致：实现**保留 5 条 strips 容器本体**（`#env-guard` / `#site-hint` / `#onboarding` / `#discovery-notice` / `#send-reason`），因为journey / binding 的**保护门禁确实读这些 id**；容器已在 `src/ui/sidepanel/host-registry.ts` 的`REGISTERED_STRUCTURAL_HOSTS` 登记为**永久结构宿主**（非过渡态 + 理由 + 通道绑定）⇒ 该 ADR §后果应读作「**移除过渡宿主标记（`data-transitional-host` × 0）+ 宿主结构性退役**，只读投影容器保留」；② **ADR-V4-035 决策 1** 的「原 L0 chip 与 L1 证据面板**被吸入** `ref` 卡 ⇒ 总数不变」口径较实现更宽：实现里 `#l0-ref-toggle` / `#l0-ref-badge` / `#l1-ref` **仍在**（由 `l1/panels.ts#paintRefs` 独立重绘，归类为「同源只读回看」），`REF_PROJECTION_POINTS` 为**自声明常量 = 3**（断言 `length === 3`），**未对 DOM 做投影点计数** ⇒ 该决策应读作「三处**写入面**（页面角标 / 流内 `ref` 卡 / 状态栏 chip）＋两处同源只读回看」；③ `src/ui/sidepanel/sidepanel.ts` 的「rows are collected during `render()`」陈旧注释同步订正（实际入口 = `refreshState()` 尾部的 `eventizeChannels()` 与一次性 `applyEnvGuard()`，且在同一函数内立即 flush）。**本行为口径订正注，ADR 正文零改写**（plan 属已完成产物）；证据见 `build.md` 附 2 §3。 | 2026-09-20 | SDDU Build Agent |
