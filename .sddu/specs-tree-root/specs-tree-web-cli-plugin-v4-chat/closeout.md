...
| 4 | **审计入口死链**（v4-3 BLOCK-04）：刚决策的 `auth` 卡审计入口点击无反应（`patchAuthCard` 造的 deps 无 `onCardAction`，且回调先 `preventDefault()` 拦掉 `href="#audit"`）；门禁只断言文案含「审计」 | **高** | 修：`patchAuthCard` 透传真实 deps（或渲染器统一追加）+ 门禁「点击后 `[data-l2-view="audit"]` 必须可见」 |
