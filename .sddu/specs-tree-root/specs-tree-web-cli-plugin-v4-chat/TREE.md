...
├── tasks.json          # 父总览（机器可读）：50 任务 / 27 波 / 4 叶 / 跨叶检查点 CP-0~CP-4
├── tasks.md          # 父任务分解（总览与叶子映射，不承接执行）
...
| tasks.md | 父任务分解（总览）：4 叶 / **50 任务** / **27 波**（S2·M29·L19）；跨叶检查点 CP-0（spike 闸门 TASK-501）~ CP-4（V3-VOL-3 收口锚 TASK-811/812）；D-005 测试守恒总账；编排器 10 条硬要求映射 | ✅ 存在 |
| tasks.json | 机器可读总览（leaves/crossLeafOrder/checkpoints/orchestratorRequirements/testConservationTotals/findings/blockers） | ✅ 存在 |
| state.json | 状态文件 | 🟢 tracked [tasked] |
