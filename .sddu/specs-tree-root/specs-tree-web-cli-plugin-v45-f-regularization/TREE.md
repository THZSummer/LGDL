...
├── TREE.md          # 本文件 - 目录导航
├── closeout.md          # F-31 web-cli-plugin v4.5「F 还原度转正」——全 Feature 总账（父收口）
...
├── tasks.md          # 任务分解：specs-tree-web-cli-plugin-v45-f-regularization（web-cli-plugin v4.5「F 还原度转正」；父 Feature 统领性任务总览）
└── specs-tree-v45-1-single-write-chronology/          # 本叶 = v4.5 的**唯一叶**，把两项核心（5 条提示带单写化 / 4 个固定位置宿主时间序化
```

## 文件说明
| 文件 | 说明 | 状态 |
|------|------|------|
| closeout.md | F-31 web-cli-plugin v4.5「F 还原度转正」——全 Feature 总账（父收口） — 1. **首屏每个事实恰出现一次** —— `#env-guard` / `#site-hint` / `#onboarding` / `#discove... | ✅ 存在 |
...
| tasks.md | 任务分解：specs-tree-web-cli-plugin-v45-f-regularization（web-cli-plugin v4.5「F 还原度转正」；父 Feature 统领性任务总览） — `*` = **spikeGate（先验闸门）**：`102` → 闸 `114`；`103` → 闸 `113`。结论 = `report-to-orc... | ✅ 存在 |
