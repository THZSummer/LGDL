...
export function validate(doc, issues = []) {
    ...
    // type
    if (!DIAGRAM_TYPES.includes(result.type)) {
        issues.push({
            severity: 'error',
 ...
 */
function parseYamlShallow(source, issues) {
 ...
 */
function parseBlock(lines, start, indent, issues) {
    const obj = {};
    let i = start;
