...
function importFlowchart(lines, issues) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
    for (const raw of lines) {
        ...
        if (node && !seen.has(node.id)) {
            nodes.push({ id: node.id, ...(node.label ? { label: node.label } : {}), ...(node.kind ? { kind: node.kind } : {}) });
            seen.add(node.id);
        }
...
function importSequence(lines, issues) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
...
function importMindmap(lines, issues) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
    const stack = [];
...
function importState(lines, issues) {
    const nodes = [];
    const edges = [];
    const idByLabel = new Map(); // label -> node id (reuse)
    const ensure = (token, terminal) => {
        ...
        let uid = id;
        let n = 1;
        while (nodes.some((x) => x.id === uid))
            uid = `${id}_${n++}`;
        nodes.push({ id: uid, label, ...(terminal ? { kind: 'end' } : { kind: 'state' }) });
        idByLabel.set(label, uid);
        return uid;
    };
    for (const raw of lines) {
        const line = raw.trim();
        if (!line || line.startsWith('stateDiagram'))
            continue;
...
function importEr(lines, issues) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
    let current = null;
    const attrs = new Map();
    const idByEntity = new Map(); // entity name -> legal id
    const ensure = (name) => {
    ...
    for (const raw of lines) {
        const line = raw.trim();
        if (!line || line.startsWith('erDiagram'))
            continue;
        // entity block: Name {
...
function importGantt(lines, issues) {
    const nodes = [];
    const edges = [];
    const groups = [];
    const seen = new Set();
    ...
    for (const raw of lines) {
        const line = raw.trim();
        if (!line || line.startsWith('gantt') || line.startsWith('dateFormat') || line.startsWith('axisFormat'))
            continue;
