...
function mermaidEr(doc) {
    ...
    for (const e of doc.edges) {
        ...
        const card = e.attrs?.cardinality ?? '1..*';
...
/** State diagram. */
function mermaidState(doc) {
    const lines = ['stateDiagram-v2'];
    const isTerminal = (id) => doc.nodes.find((n) => n.id === id)?.kind === 'end';
    for (const e of doc.edges) {
...
function mermaidGantt(doc) {
    const lines = ['gantt', '    dateFormat YYYY-MM-DD', '    axisFormat %d'];
...
export function exportMermaid(doc) {
    switch (doc.type) {
        case 'mindmap':
