...
test('V3-VOL-3 REVERSE PROOF: restoring the 40% cumulative line FAILS on the real artifact 375,102 B (+40.75%)', () => {
  assert.equal(SIDEPANEL_FINAL_ARTIFACT_BYTES, 557883, '反证必须打在**当前真实产物**上（V5.5-1 review R1 修复轮登记）');
...
test('V3-VOL-3 ⑥: 判定的 min() 优先级（绝对上限 = 硬墙，5% 公式 = 轮内软纪律）', () => {
  // 现网：min(619,520, floor(557,883 × 1.05) = 585777) = 585777（软纪律更紧）。
  const live = evaluateSidepanelSize(SIDEPANEL_BASELINE_BYTES);
  assert.equal(live.ceilingBytes, 585777, '生效上限 = min(绝对上限, 5% 公式)');
  ...
  const tight = evaluateSidepanelSize(590_000);
  assert.equal(tight.ok, false, '5% 公式之上必须 FAIL（轮内软纪律）');
  assert.equal(evaluateSidepanelSize(585777).ok, true, 'ceiling 本身仍 PASS（边界含等号）');
  assert.equal(evaluateSidepanelSize(582_305).ok, true, '旧硬编码 582,305 现落在新 ceiling 之内（历史锚点保留，边界判据见下条）');
  assert.equal(evaluateSidepanelSize(585_778).ok, false, '越 1 B 即 FAIL（585,778 > floor(557,883 × 1.05) = 585,777；边界不是宽松的）');
});
...
test('V3-VOL-3 ⑦ 反证三条（实跑口径，纯函数驱动；还原 ⇒ PASS）', () => {
  ...
  const softCase = 590_000; // > floor(557,883 × 1.05) = 585,777，仍 < 619,520
...
test('V3-VOL-3 历史保真：各轮 reason 里的「40% 停工线」逐字保留（只追加，不改写）', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 557883, 'V5.5-1 review R1 修复轮后的当前基线');
  assert.equal(
