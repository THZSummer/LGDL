...
export function blockedStateCtx(blocked) {
  switch (blocked) {
    ...
    case 'binding.stale':
      return s2Ctx({ risk: ['hardFloor'] });
    case 'ref.all-invalid':
      return s2Ctx({ ref: { validCount: 0, staleCount: 1 } });
    case 'llm.unconfigured':
      return s2Ctx({ onboarding: { firstRun: true, pendingSteps: ['配置 LLM'] } });
    case 'perm.missing':
...
export function judgeState(deps, ctx) {
  ...
  const unreachable = chips.filter((c) => !deps.opIds.includes(deps.actToOp[c.act]));
  return {
    rule: card ? card.rule : null,
    chips: chips.map((c) => ({ text: c.text, act: c.act, opId: deps.actToOp[c.act] ?? null })),
    unreachable: unreachable.map((c) => c.text),
    nextChipCount: chips.filter((c) => c.act === 'next').length,
