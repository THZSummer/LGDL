...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 602095, 'ceiling = floor(573,424 × 1.05)（V5.5-3 R2 中间登记）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 573424);
  ...
  assert.equal(SIDEPANEL_CEILING, 602095, 'ceiling = floor(573,424 × 1.05)（未加 cap 的公式值，V5.5-3 R2 登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(573424 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 602095);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 602095);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 602095, 'V5.5-3 R2 登记后 ceiling = floor(573,424 × 1.05)（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
