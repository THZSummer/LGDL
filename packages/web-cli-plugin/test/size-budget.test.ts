...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 344_899, 'ceiling = floor(328,490 × 1.05)（收口轮 +811 B 后由公式抬高）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  // v3-2 修复轮 (2026-09-16, 编排器裁决 V3-VOL-1)：按真实产物**显式提升**至
  // 327,679 B，并**撤销**自加 cap → ceiling 由公式给出（344,062 B）。
  assert.equal(SIDEPANEL_BASELINE_BYTES, 328_476);
  ...
  // 裁决 V3-VOL-1 ②：cap 已撤销 → ceiling = 公式值 344,062（**不是** 306,099）。
  assert.equal(SIDEPANEL_CEILING, 344_899, 'ceiling = floor(328,490 × 1.05)（cap 已撤销）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(328_476 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  // v3-2 修复轮（2026-09-16，裁决 V3-VOL-1）：cap 撤销 → ceiling = 公式值 344,062；
  // 重登记后的产物 327,679 B **在**判定之内（红线冲突已由裁决解除）。
  ...
  assert.equal(over.ceilingBytes, 344_899);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 344_899);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 344_899, 'v3-2 收口轮后 ceiling = 344,914（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
