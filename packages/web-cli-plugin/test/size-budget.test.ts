...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 367_374, 'ceiling = floor(349,880 × 1.05)（v3-3 L2 视图轮后由公式抬高）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 349_880);
  assert.equal(SIDEPANEL_BASELINE_META.previousBaselineBytes, 295_225);
  assert.equal(SIDEPANEL_BASELINE_META.previousCeilingBytes, 344_899);
  ...
  assert.equal(SIDEPANEL_CEILING, 367_374, 'ceiling = floor(349,880 × 1.05)（cap 已撤销）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(349_880 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 367_374);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 367_374);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 367_374, 'v3-3 build 轮后 ceiling = 367,374（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
