...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 585777, 'ceiling = floor(557,883 × 1.05)（V5.5-1 修复轮登记）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 557883);
  ...
  assert.equal(SIDEPANEL_CEILING, 585777, 'ceiling = floor(557,883 × 1.05)（未加 cap 的公式值，V5.5-1 修复轮登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(557883 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 585777);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 585777);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 585777, 'V5.5-1 修复轮登记后 ceiling = floor(557,883 × 1.05)（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
