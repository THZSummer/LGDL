...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 367_421, 'ceiling = floor(349,925 × 1.05)（v3-3 修复轮重登记后由公式抬高）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 349_925);
  assert.equal(SIDEPANEL_BASELINE_META.previousBaselineBytes, 295_225);
  assert.equal(SIDEPANEL_BASELINE_META.previousCeilingBytes, 367_374);
  ...
  assert.equal(SIDEPANEL_CEILING, 367_421, 'ceiling = floor(349,925 × 1.05)（cap 已撤销）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(349_925 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 367_421);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 367_421);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 367_421, 'v3-3 修复轮重登记后 ceiling = 367,421（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
