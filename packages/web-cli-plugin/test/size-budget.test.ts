...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  // 〖IAN-1 R1（2026-09-24）〗重 pin：ceiling = floor(598,577 × 1.05) = 628,505（中间登记）。
  assert.equal(SIDEPANEL_CEILING, 628505, 'ceiling = floor(598,577 × 1.05)（IAN-2 R1 净负登记）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 598577, '当前基线（IAN-2 R1 净负登记）');
  ...
  assert.equal(SIDEPANEL_CEILING, 628505, 'ceiling = floor(598,577 × 1.05)（未加 cap 的公式值，IAN-2 R1 净负登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(598577 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 628505);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 628505);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 628505, 'IAN-2 R1 净负登记后 ceiling = floor(598,577 × 1.05)（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
