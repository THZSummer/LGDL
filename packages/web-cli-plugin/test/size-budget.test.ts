...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 502_071, 'ceiling = floor(478,163 × 1.05)（v4-4 审查修复轮重登记后由公式抬高）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 478_163);
  ...
  assert.equal(SIDEPANEL_CEILING, 502_071, 'ceiling = floor(478,163 × 1.05)（未加 cap 的公式值，v4-4 审查修复轮重登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(478_163 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 502_071);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 502_071);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 502_071, 'v4-4 审查修复轮重登记后 ceiling = floor(478,163 × 1.05)（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
