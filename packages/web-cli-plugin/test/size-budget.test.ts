...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 446_348, 'ceiling = floor(425,094 × 1.05)（v4-2 聊天流事件模型 + review 修复轮重登记后由公式抬高）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  // content.js 177,076 B 与 pick-layer.js 33,900 B 逐字节不变（sha256 复核）。
  assert.equal(SIDEPANEL_BASELINE_BYTES, 425_094);
  ...
  assert.equal(SIDEPANEL_CEILING, 446_348, 'ceiling = floor(425,094 × 1.05)（未加 cap 的公式值，v4-2 + review 修复轮重登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(425_094 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 446_348);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 446_348);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 446_348, 'v4-2 + review 修复轮重登记后 ceiling = 446,348（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
