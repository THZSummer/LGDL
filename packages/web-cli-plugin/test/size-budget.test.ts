...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 591969, 'ceiling = floor(563,780 × 1.05)（V5.5-2 R2 本叶最终登记）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 563780);
  ...
  assert.equal(SIDEPANEL_CEILING, 591969, 'ceiling = floor(563,780 × 1.05)（未加 cap 的公式值，V5.5-2 R2 登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(563780 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 591969);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 591969);
});
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 591969, 'V5.5-2 R2 登记后 ceiling = floor(563,780 × 1.05)（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
