...
test('V2-2 size: sidepanel regression ceiling is floor(baseline × 1.05) — 判定无 cap（V3-VOL-1 ①）', () => {
  ...
  // 〖ADN-1 R1（2026-09-26）〗重 pin：ceiling = floor(603,205 × 1.05) = 633,365（中间登记）。
  assert.equal(SIDEPANEL_CEILING, 633365, 'ceiling = floor(603,205 × 1.05)（ADN-1 R1 中间登记）');
...
test('W4 size: sidepanel baseline explicitly re-registered (history retained; guard-tighten round)', () => {
  ...
  assert.equal(SIDEPANEL_BASELINE_BYTES, 603205, '当前基线（ADN-1 R1 中间登记）');
  ...
  assert.equal(SIDEPANEL_CEILING, 633365, 'ceiling = floor(603,205 × 1.05)（未加 cap 的公式值，ADN-1 R1 中间登记）');
  ...
  assert.equal(SIDEPANEL_CEILING_UNCAPPED, Math.floor(603205 * 1.05), '未加 cap 的公式值必须被记录');
...
test('W4 size REVERSE PROOF: the tightened ceiling still FAILS on one byte over', () => {
  ...
  assert.equal(over.ceilingBytes, 633365);
  ...
  assert.equal(evaluateSidepanelSize(SIDEPANEL_CEILING).ceilingBytes, 633365);
});
...
test('V2-4 size: re-registration meta carries date/source/reason and is NOT a target budget', () => {
  assert.equal(SIDEPANEL_BASELINE_META.measuredOn, '2026-09-26');
  assert.equal(SIDEPANEL_BASELINE_META.source, 'packages/web-cli-plugin/dist/sidepanel.js');
...
test('V2-4 size: ceiling stays structurally consistent (floor(baseline × 1.05), no cap)', () => {
  ...
  assert.equal(SIDEPANEL_CEILING, 633365, 'ADN-1 R1 中间登记后 ceiling = floor(603,205 × 1.05)（公式判定，cap 已撤销）');
  const over = evaluateSidepanelSize(SIDEPANEL_CEILING + 1);
