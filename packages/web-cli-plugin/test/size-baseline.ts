 ...
 * 未在预算表单列，实测 +4,935 B）。
 */
export const SIDEPANEL_BASELINE_BYTES = 542_150;
 ...
 */
export const SIDEPANEL_FINAL_ARTIFACT_BYTES = 542_150;
...
export const SIDEPANEL_GROWTH_BREAKDOWN = {
   ...
   */
  closeoutDeltaBytes: 6_329,
  newRequiredModuleBytes: 176_089,
   ...
   */
  duplicationCheckInputModuleCount: 87,
  /**
  ...
  rows: [
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 44_845, afterBytes: 93773, deltaBytes: 48928, kind: 'wiring', requiredBy: 'FR-V3-031~040 + FR-V3-045/047/048/054 + FR-V3-060~066 + AC-CONV-2 + R1 + R2 + R3 + v4-1 + v4-3（卡动作按 cardId→requestId 解析 + askFlow/timeoutOpenAsks seam + 授权卡审计入口 + revealAskFallback + clearAsk 全结算）—— FR-CHAT-040~048 · ADR-V4-017/022/030~033' },
    ...
    { module: 'src/ui/sidepanel/next-registry/pipeline.ts', beforeBytes: null, afterBytes: 3860, deltaBytes: 3860, kind: 'new-required-module', requiredBy: 'FR-ALLN-055 / FR-ALLN-034（`runOp` 四态唯一管线 + `pendingOps` FIFO 仲裁 + 快照/回滚语义位 + R5 失败三级；`op.execute(` 恰 1 调用点）· ADR-V5-002 §1' },
    { module: 'src/ui/sidepanel/next-registry/providers.ts', beforeBytes: null, afterBytes: 4452, deltaBytes: 4452, kind: 'new-required-module', requiredBy: 'FR-ALLN-030~038 / FR-ALLN-013（4 内置 provider：5 P0 恢复 + onboarding/ref-action/capability-discovery；旧 `recommend.ts` 规则表等价迁移）· ADR-V5-001' },
