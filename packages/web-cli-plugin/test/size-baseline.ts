 ...
 */
/**
 * ── V5.5-3 **R2（2026-09-23，leaf `specs-tree-v55-3-ai-driven-orchestration`；TASK-V55-308~315）** ──
 * 主题② AI 驱动编排 R2（W3+W4）：SW **有界仲裁队列**（`TURN_QUEUE_MAX = 1` + 溢出明确拒绝 + 面板
 * **草稿回填**；仲裁闭集 4 项 type-only）+ 护栏**六常量单源**（`next-registry/guard.ts`：频次 6/10min ·
 * 同因不重复 · 静默 60 s · 冷却 **re-export** 既有 10 s · 链深 2 · 回合预算 8；关断默认 ON）+
 * 越限**真抑制** + `driverSuppressedLine` 留痕 + 既有 `settings-llm` 分区内的主动性总开关。
 * 真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v553R2Rows`（Σ +6,843 + glue +46 == +6,889）。
 * 方向 = 提升；**档位 614,400 / 绝对上限 675,840 均不动**（573,424 < 614,400 ⇒ 未跨档位），
 * `authorConfirmation` 保持 `pending-author-line`（不伪称已确认）。`dist/content.js` 177,076 B 与
 * `dist/pick-layer.js` 34,358 B **逐字节不变**。本条目为**中间登记**（W5 TASK-V55-319 按三叶合计终态再登记）。
 */
/**
 * ── V5.5F-1 **R1+R2（2026-09-24，leaf `specs-tree-v55f-1-ref-context-and-anchor`；TASK-V55F-101~129）** ──
 * 范围治理「引用即范围」**底座叶**（A 列逐叶重登记）：`l1/ref-scope.ts` NEW（引用快照投影 +
 * 法九读数单源 + 写闸裁决）+ `sidepanel.ts`（`requestTurn` 唯一载荷构建点 / confirm 面范围闸 /
 * 范围留痕 / testing seam）+ `l1/ref-store.ts`（只读取用：`isActiveRef` + `activeValid`）；
 * `--ref` 锚定链（`tools/dom-anchor.ts` / `background/ref-observe.ts`）与系统段追加段
 * （`background/ref-context.ts`）全部落 **B 列**（`background.js`，**不计入本账本**）。
 * 真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2Rows`（Σ 模块 +7,070 + glue 39 == +7,109）。
 * 方向 = 提升；**档位 614,400 / 绝对上限 675,840 均不动**（585,732 < 614,400 ⇒ 未跨档位），
 * `authorConfirmation` 保持 `pending-author-line`（不伪称已确认）。`dist/content.js` 177,076 B 与
 * `dist/pick-layer.js` 34,358 B **逐字节不变**。**本叶逐叶收口登记**（不等叶2合计，FR-SGO-125）。
 *
 * 〖IAN-1 R1（2026-09-24, leaf `specs-tree-ian-1-free-input-next`；W1+W2 = TASK-IAN-101~116）〗
 * 逐叶**中间登记**（W3/R2 的 TASK-IAN-126 按全叶终态再登记五要素）：591,946 → **598,282 B**
 * （+6,336 B，+1.07%）；逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.ian1R1Rows`
 * （Σ 模块 +6,336 + glue **0** == +6,336）。⚠️ **越叶预算登记不停机**：本叶 A 列预算
...
export const SIDEPANEL_BASELINE_BYTES_TIMELINE = [
  ...
  563_145,
  // 〖V5.5-2 小修轮（2026-09-23，review R1 的 I-01~04；本叶 review 修复轮）〗超容留痕（`over-capacity` 被消费）/ 失败可重试
  //   （failed 不并入同因去重）/ 冷启动双源独立（裁定投影 ∨ 被动快照的单源纯判据）/ 读数订正 ⇒ **越档位 563,200**
  ...
  566_535,
  // 〖V5.5-3 R2（2026-09-23，TASK-V55-308~315）〗W3 仲裁（SW 有界队列 + 草稿回填）+ W4 护栏
  //   （六常量单源 / 越限抑制 / 关断偏好 / 留痕）一轮的登记值（档位与绝对上限均不变）。
  573_424,
  // 〖R6 缺陷快修轮（2026-09-23，真机体验 ty.md；P3 答案 once / P4 用户输入排队统一 / P5a 完成后同动作去重 / P5b 引用改写后重评）〗
  //   573,424 → **578,623 B**（+5,199 B，+0.91%）；逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.r6TyFixRows`
  //   （sidepanel.ts +2,513 / recommend.ts +977 / view-model.ts +453 / l1/ref-validity.ts +596 /
  //    pick-input.ts +258 / next-registry/drivers.ts +402；Σ +5,199 + glue 0 == +5,199）。
  //   档位 614,400 / 绝对上限 675,840 均不动（未跨档位），生效上限 = floor(578,623 × 1.05) = 607,554。
  578_623,
  // 〖V5.5F-1 R1+R2（2026-09-24，leaf specs-tree-v55f-1-ref-context-and-anchor；W1~W4 = TASK-V55F-101~129）〗
  //   范围治理「引用即范围」**底座叶**的逐叶收口登记值：578,623 → **585,732 B**（+7,109 B，+1.23%）；
  //   逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2Rows`（Σ 模块 +7,070 + glue 39 == +7,109）。
  //   档位 614,400 / 绝对上限 675,840 均不动（585,732 < 614,400 ⇒ 未跨档位）；
  //   生效上限 = floor(585,732 × 1.05) = **615,018**（旧生效上限 607,554 未越）。
  585_732,
  // 〖V5.5F-2 R1+R2（2026-09-24，leaf specs-tree-v55f-2-batch-consent；W1~W3 = TASK-V55F-201~216）〗
  //   任务级批量授权叶的逐叶收口登记值：585,732 → **591,946 B**（+6,214 B，+1.06%）；
  //   逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f2R1R2Rows`（Σ 模块 +6,214 + glue 0 == +6,214）。
...
export const SIDEPANEL_BASELINE_META = {
  ...
  measuredBy:
    ...
    '档位 614,400 / 绝对上限 675,840 均不动（598,282 < 614,400 ⇒ 未跨档位），生效上限 = floor(598,282 × 1.05) = **628,196**；`dist/content.js` 177,076 B / sha `52a82620…` 与 `dist/pick-layer.js` 34,358 B / sha `77796bab…` **逐字节不变**；' +
    '**越叶预算登记不停机**（ADR-IAN-010 §② 预算 +2.5~4.5 KB 被 +6.34 KB 超越，如实登记；W3/R2 的 TASK-IAN-126 按全叶终态再登记五要素）。Previous round: ' +
    'SDDU **V5.5F-2 R1+R2（2026-09-24, leaf specs-tree-v55f-2-batch-consent；A 列逐叶重登记）**: re-registered on the FINAL artifact — 585,732 → **591,946 B**（+6,214 B，+1.06%）— ' +
    '任务级批量授权叶：W1 边界先行（RL-06 / OT-⑩ 扩批量变体注入必红 + 特权 op 不入批）+ W2 计划/指纹/准入/回落/漂移/零明文/中止（`background/batch-plan.ts` NEW 落 **B 列**）+ W3 扩围二择（`ref-scope.ts` 选项/转值单源 + `sidepanel.ts` WIDEN 接线 + 批量留痕）：' +
    '`sidepanel.ts` A 列 +2,829 / `cards/auth.ts` +2,113（计划行 + 掩码 + 中止交代）/ `l1/ref-scope.ts` +532（扩围二择单源）/ `stream-model.ts` +455（`plan` 兄弟字段）/ `chat-state.ts` +285（`confirm` action 字段）；' +
    '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f2R1R2Rows`（Σ 模块 +6,214 + glue 0 == +6,214）；`background/batch-plan.ts` / `security/confirm.ts` / `service-worker.ts` 全落 **B 列**；' +
    '档位 614,400 / 绝对上限 675,840 均不动（591,946 < 614,400 ⇒ 未跨档位），生效上限 = floor(591,946 × 1.05) = **621,543**（旧生效上限 615,018 **未越** ⇒ 未触发 EC-SGO-022 任一分支）；`dist/content.js` 177,076 B / sha `52a82620…` 与 `dist/pick-layer.js` 34,358 B / sha `77796bab…` **逐字节不变**；' +
    '**本叶逐叶收口登记**。Previous round: ' +
    'SDDU **V5.5F-1 R1+R2（2026-09-24, leaf specs-tree-v55f-1-ref-context-and-anchor；A 列逐叶重登记）**: re-registered on the FINAL artifact — 578,623 → **585,732 B**（+7,109 B，+1.23%）— ' +
    '范围治理「引用即范围」底座叶：`l1/ref-scope.ts` NEW（引用快照投影 `turnRefsOf` + 法九四值读数单源 `scopeReading` + 写闸 `scopeWriteGate` + 留痕单源）+ `sidepanel.ts`（`requestTurn` 唯一载荷构建点 + confirm 面越界写 fail-closed + 范围留痕行 + `l1(scope)` seam）+ `l1/ref-store.ts`（只读取用 `isActiveRef` / `activeValid`）；' +
    '`--ref` 锚定链（`tools/dom-anchor.ts` 包装层 + `background/ref-observe.ts` 单实现 live 单节点闸）与系统段追加段（`background/ref-context.ts`）**全部落 B 列**（`background.js`，不计入本账本）；' +
    '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2Rows`（`l1/ref-scope.ts` NEW +2,572 / `sidepanel.ts` +1,765 / `l1/ref-store.ts` +149 / `../web-cli-base/dist/sensitive.js` +2,584【复用 base 掩码/敏感面，非复制】；Σ +7,070 + glue 39 == +7,109）；' +
    '档位 614,400 / 绝对上限 675,840 均不动（585,732 < 614,400 ⇒ 未跨档位），生效上限 = floor(585,732 × 1.05) = **615,018**（旧生效上限 607,554 **未越** ⇒ 未触发 EC-SGO-022 任一分支）；`dist/content.js` 177,076 B / sha `52a82620…` 与 `dist/pick-layer.js` 34,358 B / sha `77796bab…` **逐字节不变**；' +
    '**本叶逐叶收口登记**（不等叶2合计，FR-SGO-125）。Previous round: ' +
    'SDDU **R6 缺陷快修轮（2026-09-23, 真机体验 ty.md；P3 答案 once / P4 用户输入排队统一 / P5a 完成后同动作去重 / P5b 引用改写后重评）**: re-registered on the FINAL artifact — 573,424 → **578,623 B**（+5,199 B，+0.91%）— ' +
    '答案 once 语义（后台 ask 答案经 `askBridge.settle` 消费后 requestId 级去重）+ 用户输入路径与 AI 同仲裁（在飞提交走 SW 有界队列）+ 完成后同 digest 引用动作去重 + `text-changed` 引用重评（身份仍在但文本摘要被改写）；' +
    '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.r6TyFixRows`（sidepanel.ts +2,513 / recommend.ts +977 / view-model.ts +453 / l1/ref-validity.ts +596 / pick-input.ts +258 / next-registry/drivers.ts +402；Σ +5,199 + glue 0 == +5,199）；' +
    '档位 614,400 / 绝对上限 675,840 均不动（未跨档位），生效上限 = floor(578,623 × 1.05) = 607,554；`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B **逐字节不变**。Previous round: ' +
    'SDDU **V5.5-3 R2（2026-09-23, leaf specs-tree-v55-3-ai-driven-orchestration; W3+W4 = TASK-V55-308~315）**: re-registered on the FINAL artifact — 566,535 → **573,424 B**（+6,889 B，+1.22%）— ' +
    '主题② R2：W3 **有界仲裁**（`background/turn-queue.ts` NEW：`TURN_QUEUE_MAX = 1` + `classifyChatRequest` 三路径 + 溢出明确拒绝；SW `runChat` 入队 / drain / 会话切换显式拒绝；面板 `queued` / `busy-rejected` 两变体 + **草稿回填 `#input`**）+ W4 **护栏**（`next-registry/guard.ts` NEW：六常量单源 + 工厂 / 单例 + `loadProactivePref` / `saveProactivePref`；`driverSuppressedLine` 抑制留痕；`settings/panel.ts` 既有 `settings-llm` 分区内总开关；越限真抑制 + 关断后主题① 仍工作）；真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v553R2Rows`（`guard.ts` NEW +3,016 / `settings/panel.ts` +1,894 / `sidepanel.ts` +1,775 / `ai-drive.ts` +158；Σ +6,843 + glue +46 == +6,889）；档位 614,400 / 绝对上限 675,840 均不动（573,424 < 614,400 ⇒ 未跨档位），`authorConfirmation` 保持 `pending-author-line`；`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B **逐字节不变**；本条目为**中间登记**（W5 TASK-V55-319 按三叶合计终态再登记）。' +
    'Previous round: ' +
    ...
    '**越档位 ⇒ ADR-V55-011 §4 显式升档**（档位 563,200 → **614,400**、绝对上限 619,520 → **675,840**，`authorConfirmation` 保持 `pending-author-line`）。' +
  ...
  reason:
    ...
    '**2026-09-24 V5.5F-2 R1+R2（leaf specs-tree-v55f-2-batch-consent；A 列逐叶收口）显式提升重登记：585,732 → 591,946 B（+6,214 B，+1.06%）**：' +
    '任务级批量授权叶 A 列 = 计划卡渲染（`cards/auth.ts` 计划行 + 掩码 + 中止交代 +2,113）+ 计划兄弟字段（`stream-model.ts` +455）+ WIDEN 二择/批量留痕（`sidepanel.ts` +2,829）+ 扩围单源（`l1/ref-scope.ts` +532）+ `confirm` action 字段（`chat-state.ts` +285）；' +
    '`background/batch-plan.ts`（计划 + 指纹 + 准入）/ `security/confirm.ts`（计划感知桥）/ `service-worker.ts`（批次捕获）**全落 B 列**（`background.js`，不计入本账本）。' +
    '逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f2R1R2Rows`（Σ +6,214 + glue 0 == +6,214）；档位 614,400 / 绝对上限 675,840 未变，生效上限 = floor(591,946 × 1.05) = 621,543（旧上限 615,018 未越）。' +
    '`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B 逐字节不变；容差 5% 未动、断言零删减。' +
    '**2026-09-23 V5.5-2 小修轮（review R1 的 I-01~04；本叶 review 修复轮）显式提升重登记：563,145 → 563,780 B（+635 B，+0.11%）**：' +
    '① I-02 超容留痕（`over-capacity` 被消费 ⇒ 一行事实，不再静默丢弃）；② I-03 失败可重试（failed 不并入同因去重）；' +
    '③ I-04 冷启动双源独立（`llmBlockedFactApplies(ruled, passive)` 单源纯判据：SW 的 `isLlmConfigured` 裁定投影 ∨ 被动 `llm-status` 快照）；' +
    '④ I-01 读数订正（build.md §5b，零字节）。逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v552R3Rows`（Σ +635 + glue 0 == +635）。' +
    '**⚠️ 本轮越档位**（563,780 > 563,200）⇒ 按 **ADR-V55-011 §4 显式升档**：档位 → **614,400**（`ceilTo50KB(563,780)`）、' +
    '绝对上限 → **675,840**（`614,400 × 1.10`），`authorConfirmation` 保持 **`pending-author-line`**（不伪称已确认）；' +
    '生效上限 = `min(675,840, floor(563,780 × 1.05) = 591,969) = 591,969`。' +
 ...
 *
 * 〖V5.5-2 小修轮（2026-09-23，review R1 的 I-01~04）〗档位**再上移**一档（越档位 ⇒
 * ADR-V55-011 §4 显式升档）⇒ 算术下界同源前移：`ceilTo50KB(b) = 614_400 ⟺ 563_201 ≤ b ≤ 614_400`。
 * 方向仍是**只挡下移**（不挡上移）；历史档位（512,000 / 563,200）的算术边界逐字保留在上文与历史轮登记里。
 */
export const SIDEPANEL_TIER_FLOOR_BYTES = 563_201;
...
export const SIDEPANEL_RE_REGISTRATIONS: readonly SizeReRegistration[] = [
  ...
  /**
   * 〖V5.5-2 **小修轮**（2026-09-23，review R1 的 I-01~04）—— 本叶 review 修复轮〗
   *
   * ① **I-02 超容留痕**：`registerConfigSuspension` 的 `over-capacity` 返回值被**消费**（不再静默丢弃）
   *    ⇒ 留痕一行事实（`ONBOARD_SUSPENSION_RETAINED_TEXT`，口径 = **原任务优先保留**，与 `suspension.ts` 注释同源订正）；
   * ② **I-03 失败可重试**：同因去重键只认**用户主动放弃**（`recordsDeclinedCause`：cancelled / rejected）——
   *    配置失败（failed）不并入 ⇒ 同因引导保持可重试；
   * ③ **I-04 冷启动双源独立**：`llmBlockedFactApplies(ruled, passive)` 单源纯判据 —— 主动裁定 = SW 的
   *    `isLlmConfigured` 投影（`llm-unconfigured` 变体），与被动 `llm-status` 快照是 **∨**；冷启动窗口
   *    （detect 行已出、快照未回）下仍落事实 ⇒ guide chip 不缺席；
   * ④ **I-01 读数订正**：build.md §5b 的 `onboarding-deterministic` / `s0-self-driven-chain` 登记读数按实测订正
   *    （23 / 11，零字节）。
   *
   * **⚠️ 本轮越档位**（`ceilTo50KB(563,780) = 614,400` > 563,200）⇒ 按 **ADR-V55-011 §4 显式升档**：
   * 档位 563,200 → **614,400**、绝对上限 619,520 → **675,840**（`614,400 × 1.10`）；
   * `authorConfirmation` 保持 **`pending-author-line`**（**不伪称已确认**）；生效上限 =
   * `min(675,840, floor(563,780 × 1.05) = 591,969) = 591,969`。
   */
  {
    id: 'v55-2-r3',
    direction: 'raised',
    ...
    buildCommand: 'npm run build --workspace @lgdl/web-cli-plugin',
    measuredBy:
      'SDDU V5.5-2 review 小修轮（2026-09-23，leaf specs-tree-v55-2-deterministic-onboarding；review R1 的 I-01~04）',
    reason:
      '**本叶 review 小修轮显式提升重登记：563,145 → 563,780 B（+635 B，+0.11%）**。' +
      '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v552R3Rows`（Σ 模块 +635 + glue **0** == +635）：' +
      '`next-registry/onboarding-flow.ts` 1,722 → **2,125（+403）**（超容留痕文案单源 + `llmBlockedFactApplies` / `recordsDeclinedCause` 两条纯判据）；' +
      '`sidepanel.ts` 101,472 → **101,704（+232）**（返回值消费 + 留痕 dispatch + 记因口径接单源判据 + 裁定入参折叠）。' +
      '**⚠️ 本轮越档位 ⇒ ADR-V55-011 §4 显式升档**：档位 563,200 → **614,400**（`ceilTo50KB(563,780)`）、' +
      '绝对上限 619,520 → **675,840**；生效上限 = `min(675,840, floor(563,780 × 1.05) = 591,969) = 591,969`；' +
      '`authorConfirmation` 保持 **`pending-author-line`**（不伪称已确认）。升档触发点由 v55-3 前移至此叶（ADR-V55-011 §3 注「谁先越谁登记」）。' +
      '`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B **逐字节不变**；容差 5% 未动；' +
      '`SIDEPANEL_CEILING_CAP` 保持 record-only；**断言零删减**（新增 OD-17/OD-18/OD-19 各含反证 + 体积面只改数值重 pin）。' +
      '**预算口径诚实登记**：本叶预算 4,900 B / 上界 6,300 B；R1 +4,390 + R2 +872 + 小修轮 +635 = **+5,897 B** ⇒ ' +
      '**超出叶预算 997 B**（未越叶上界 6,300 B）；档位升档按 ADR 显式登记，不静默。',
    baselineBeforeBytes: 563_145,
    baselineAfterBytes: 563_780,
    ceilingBeforeBytes: 591_302,
    ceilingAfterBytes: 591_969,
    assertionNonRemovalEntries: [
      'V552-R3-CHAIN-size-baseline',
      'V552-R3-CHAIN-size-budget',
      'V552-R3-CHAIN-size-ruling-vol3',
      'V552-R3-CHAIN-size-growth-evidence',
    ],
  ...
  /**
   * 〖V5.5-3 R2（2026-09-23，leaf `specs-tree-v55-3-ai-driven-orchestration`；W3+W4 = TASK-V55-308~315）〗
   * 主题② AI 驱动编排的**中间登记**（W5 TASK-V55-319 按三叶合计终态再登记）。
   */
  {
    id: 'v55-3-r2',
    ...
    source: 'packages/web-cli-plugin/dist/sidepanel.js',
    buildCommand: 'npm run build --workspace @lgdl/web-cli-plugin',
    measuredBy: 'SDDU V5.5-3 R2 (2026-09-23, leaf specs-tree-v55-3-ai-driven-orchestration; W3+W4 = TASK-V55-308~315)',
    reason:
      '**V5.5-3 R2 显式提升重登记：566,535 → 573,424 B（+6,889 B，+1.22%）**（中间登记；W5 TASK-V55-319 按三叶合计终态再登记）。' +
      '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v553R2Rows`（Σ 模块 +6,843 + glue **46** == +6,889）：' +
      '`next-registry/guard.ts` NEW **3,016**（护栏六常量单源 + 工厂 / 单例 + 越限抑制判据 + 关断偏好读写）；' +
      '`settings/panel.ts` 37,358 → **39,252（+1,894）**（既有 `settings-llm` 分区内主动性总开关：阅读 + 单一写点 + 持久化）；' +
      '`sidepanel.ts` 102,426 → **104,201（+1,775）**（W3 `queued` / `busy-rejected` 两变体 + 草稿回填；W4 护栏抑制前置判定 + `proactivity` seam + `noteUserTurn` / `noteVeto` / 启动读偏好）；' +
      '`next-registry/ai-drive.ts` 1,511 → **1,669（+158）**（`driverSuppressedLine` 抑制留痕单源）。' +
      '**档位 614,400 / 绝对上限 675,840 不动**（本产物 573,424 B < 档位 ⇒ **无跨档位**，无需升档；生效上限随公式前移为 `floor(573,424 × 1.05) = 602,095`）。' +
      '`authorConfirmation` 保持 **`pending-author-line`**（不伪称已确认）；`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B **逐字节不变**；' +
      '容差 5% 未动；**断言零删减**（新增 `test/turn-arbitration.test.ts` 6 用例 + `test/proactivity-guard.test.ts` 7 用例；`requestTurn(` **仍恰 2**、`nextAfterSettle` 1 定义 + 10 调用点不变）。' +
      '**预算口径诚实登记（不得静默）**：本叶预算 5,900 B / 上界 7,600 B；R1 +2,755 + **R2 +6,889** = **+9,644 B** ⇒ 本叶**累计越叶预算（超 3,744 B）**；R2 单轮 +6,889 B 仍 < 单轮上界 7,600 B，且产物 573,424 B 仍在旧生效上限 594,861 B 内（"生效上限内优先"）；超额根因 = 计划侧严重低估（`guard.ts` 计划 1,700 → 实测 3,016；`settings/panel.ts` 计划 350 → 实测 1,894），**逐条如实登记、不停机**（未跨档位、无红线变化）。',
    baselineBeforeBytes: 566_535,
    baselineAfterBytes: 573_424,
    ceilingBeforeBytes: 594_861,
    ceilingAfterBytes: 602_095,
    assertionNonRemovalEntries: ['V42-E-VOL-2', 'V43-E-33', 'V551-MR-X-SELF-2'],
    historyRetainedBytes: [566_535, 563_780, 563_145, 562_273],
    ceilingUncappedFormulaBytes: 602_095,
  },
  /**
   * 〖R6 缺陷快修轮（2026-09-23，真机体验 `ty.md`）〗四缺陷快修 +
   * **P5b `text-changed` 新维度**的**显式提升重登记**。逐模块归因见
   * `SIDEPANEL_GROWTH_BREAKDOWN.r6TyFixRows`（Σ +5,199 + glue 0 == +5,199）。
   * 档位 614,400 / 绝对上限 675,840 均不动（未跨档位）；生效上限随公式前移为
   * `floor(578,623 × 1.05) = 607,554`。
   */
  {
    id: 'r6-ty-fix',
    direction: 'raised',
    roundKind: 'feature-round',
    feature: 'r6-ty-experience-fix',
    date: '2026-09-23',
    source: 'packages/web-cli-plugin/dist/sidepanel.js',
    buildCommand: 'npm run build --workspace @lgdl/web-cli-plugin',
    measuredBy: 'SDDU R6 缺陷快修轮 (2026-09-23, 真机体验 ty.md; P3/P4/P5a/P5b)',
    reason:
      '**R6 缺陷快修轮显式提升重登记：573,424 → 578,623 B（+5,199 B，+0.91%）**（真机 `ty.md` 四项缺陷）。' +
      '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.r6TyFixRows`（Σ 模块 +5,199 + glue **0** == +5,199）：' +
      '`sidepanel.ts` 104,201 → **106,714（+2,513）**（P3 答案 once 语义 + P4 用户输入排队统一 + P5a 完成台账 + P5b 只读重评接线）；' +
      '`recommend.ts` 5,076 → **6,053（+977）**（意图摘要 + 完成后同动作去重）；' +
      '`view-model.ts` 23,946 → **24,399（+453）**（composer 仅异常态禁用 + 排队文案）；' +
      '`l1/ref-validity.ts` 9,639 → **10,235（+596）**（`text-changed` 维度 + 明文文案）；' +
      '`pick-input.ts` 8,405 → **8,663（+258）**（只读 `observe`）；' +
      '`next-registry/drivers.ts` 3,307 → **3,709（+402）**（`askId` + `drivableSuspension`）。' +
      '**档位 614,400 / 绝对上限 675,840 不动**（578,623 B < 档位 ⇒ **无跨档位**，无需升档；生效上限随公式前移为 `floor(578,623 × 1.05) = 607,554`）。' +
      '`authorConfirmation` 保持 **`pending-author-line`**（不伪称已确认）；`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B **逐字节不变**；' +
      '容差 5% 未动；**断言零删减**（新增 `test/r6-ty-experience-fix.test.ts` 11 用例；`requestTurn(` **仍恰 2**、`maybeRecommend(` **仍恰 7**、`nextAfterSettle` 1 定义 + 10 调用点不变）。',
    baselineBeforeBytes: 573_424,
    baselineAfterBytes: 578_623,
    ceilingBeforeBytes: 602_095,
    ceilingAfterBytes: 607_554,
    assertionNonRemovalEntries: ['V42-E-VOL-2', 'V43-E-33', 'R6-E-SVP-1'],
    historyRetainedBytes: [573_424, 566_535],
    ceilingUncappedFormulaBytes: 607_554,
  },
  /**
   * 〖V5.5F-1 **R1+R2**（2026-09-24，leaf `specs-tree-v55f-1-ref-context-and-anchor`；TASK-V55F-101~129）〗
   * 范围治理「引用即范围」**底座叶**的**逐叶收口重登记**（FR-SGO-125：不等叶2合计）。
   * 方向 = **提升**；**档位 614,400 / 绝对上限 675,840 均不动**（585,732 < 614,400 ⇒ 未跨档位）；
   * 生效上限 = `min(675,840, floor(585,732 × 1.05) = 615,018) = 615,018`（旧生效上限 607,554 **未越**
   * ⇒ EC-SGO-022 两个分支均**未触发**：既未越生效上限、也未越档位）。`authorConfirmation` 保持
   * **`pending-author-line`**（不伪称已确认）。`dist/content.js` 177,076 B / sha `52a82620…` 与
   * `dist/pick-layer.js` 34,358 B / sha `77796bab…` **逐字节不变**；容差 5% 未动；**断言零删减**
   * （体积面只改数值重 pin + 追加本条目与 `v55f1R1R2Rows`；新门禁 3 枚：`ref-context-in-turn` /
   * `law9-scope-reading` / `dom-ref-anchor`；`requestTurn(` **仍恰 2**）。
   * 预算口径诚实登记（ADR-SGO-007 §1）：本叶 A 列预算 **6.0~9.0 KB**（上界 10.35 KB）⇒ 实测
   * **+7,109 B ≈ 6.94 KiB**，**未越预算、未越上界**；B 列（`background.js`）1,621,013 → **1,627,424 B**
   * （+6,411 B，**不计账**：`--ref` 包装 + `ref-observe` 单实现 + 系统段追加段）。
   */
  {
    id: 'v55f-1-r2',
    direction: 'raised',
    roundKind: 'feature-round',
    feature: 'specs-tree-v55f-1-ref-context-and-anchor',
    date: '2026-09-24',
    source: 'packages/web-cli-plugin/dist/sidepanel.js',
    buildCommand: 'npm run build --workspace @lgdl/web-cli-plugin',
    measuredBy: 'SDDU V5.5F-1 R1+R2（2026-09-24，leaf specs-tree-v55f-1-ref-context-and-anchor；W1~W4 = TASK-V55F-101~129）',
    reason:
      '**本叶逐叶收口显式提升重登记：578,623 → 585,732 B（+7,109 B，+1.23%）**。' +
      '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2Rows`（Σ 模块 +7,070 + glue **39** == +7,109）：' +
      '`l1/ref-scope.ts` NEW **2,572**（引用快照投影 + 法九四值读数单源 + 写闸裁决 + 留痕单源）；' +
      '`sidepanel.ts` 106,714 → **108,479（+1,765）**（`requestTurn` 唯一载荷构建点 / confirm 面越界写 fail-closed / 范围留痕行 / `l1(scope)` seam）；' +
      '`l1/ref-store.ts` 6,169 → **6,318（+149）**（只读取用 `isActiveRef` + `activeValid`）；' +
      '`../web-cli-base/dist/sensitive.js` 938 → **3,522（+2,584）**（**复用** base 的掩码/敏感面 —— `maskRefDigest` 走既有 `maskTextPayload`，非复制）。' +
      '**B 列**（`tools/dom-anchor.ts` / `background/ref-observe.ts` / `ref-context.ts` / `ref-turn.ts` / `turn-queue.ts` / `messaging.ts` type-only）全部落 `background.js` ⇒ **本产物零字节**（ADR-SGO-007 §3 / FR-SGO-121）。' +
      '**档位与绝对上限均未变**（`ceilTo50KB(585,732) = 614,400`、675,840）；生效上限 = `min(675,840, floor(585,732 × 1.05) = 615,018) = 615,018`。' +
      ...
      '**预算口径诚实登记**：本叶 A 列预算 **6.0~9.0 KB** / 上界 **10.35 KB** ⇒ 实测 **+7,109 B（6.94 KiB）未越**；' +
      'R1（W1+W2）+ R2（W3+W4）合计 = 本叶合计（逐叶登记，不等两叶合计口径）。' +
      '**EC-SGO-022 二态显式**：越**生效上限**（607,554）= **否**（585,732 < 607,554）；越**档位**（614,400）= **否**；' +
      '越**绝对上限**（675,840）= **否** ⇒ 三档均未触发（禁预填、不静默）。',
    baselineBeforeBytes: 578_623,
    baselineAfterBytes: 585_732,
    ceilingBeforeBytes: 607_554,
    ceilingAfterBytes: 615_018,
    assertionNonRemovalEntries: ['V42-E-VOL-2', 'V55F1-E-VOL-1'],
    historyRetainedBytes: [578_623, 573_424, 566_535, 563_780],
    ceilingUncappedFormulaBytes: 615_018,
  },
  /**
   * 〖V5.5F-2 **R1+R2**（2026-09-24，leaf `specs-tree-v55f-2-batch-consent`；TASK-V55F-201~216）〗
   * 任务级批量授权叶的**逐叶收口重登记**。方向 = **提升**；**档位 614,400 / 绝对上限 675,840
   * 均不动**（591,946 < 614,400 ⇒ 未跨档位）；生效上限 = `min(675,840, floor(591,946 × 1.05)
   * = 621,543) = 621,543`（旧生效上限 615,018 **未越** ⇒ EC-SGO-022 三分支均**未触发**）。
   * `authorConfirmation` 保持 **`pending-author-line`**（不伪称已确认）。`dist/content.js`
   * 177,076 B / sha `52a82620…` 与 `dist/pick-layer.js` 34,358 B / sha `77796bab…` **逐字节不变**；
   * 容差 5% 未动；**断言零删减**（体积面只改数值重 pin + 追加本条目与 `v55f2R1R2Rows`；
   * 新 node 门禁 1 枚 `batch-consent`；`CHROMIUM_GATES === 9` 逐字不动；`requestTurn(` 仍恰 2）。
   * 预算口径诚实登记（ADR-SGO-007 §1）：本叶 A 列预算 **3.5~5.5 KB**（上界 **4.0~6.3 KB**）
   * ⇒ 实测 **+6,214 B ≈ 6.07 KiB**：**越预算基线上界（5.5 KB）但未越 +15% 上界（6.3 KB）**，
   * 显式超出并如实登记（不删判据 / 不放宽容差）。B 列（`background.js`）1,627,424 → **1,635,675 B**
   * （+8,251 B，**不计账**：`batch-plan.ts` NEW + 计划感知桥 + 批次捕获）。
   */
  {
    id: 'v55f-2-r2',
    direction: 'raised',
    roundKind: 'feature-round',
    feature: 'specs-tree-v55f-2-batch-consent',
    date: '2026-09-24',
    source: 'packages/web-cli-plugin/dist/sidepanel.js',
    buildCommand: 'npm run build --workspace @lgdl/web-cli-plugin',
    measuredBy: 'SDDU V5.5F-2 R1+R2（2026-09-24，leaf specs-tree-v55f-2-batch-consent；W1~W3 = TASK-V55F-201~216）',
    reason:
      '**本叶逐叶收口显式提升重登记：585,732 → 591,946 B（+6,214 B，+1.06%）**。' +
      '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.v55f2R1R2Rows`（Σ 模块 +6,214 + glue **0** == +6,214）：' +
      '`sidepanel.ts` 108,479 → **111,308（+2,829）**（批量计划 confirm 面 + WIDEN 二择接线 + 批量留痕 + 测试 seam）；' +
      '`cards/auth.ts` 6,387 → **8,500（+2,113）**（计划行渲染 + 凭据形掩码 + 中止如实交代）；' +
      '`l1/ref-scope.ts` 2,572 → **3,104（+532）**（扩围二择选项 / 提示语 / 转值判据单源）；' +
      '`stream-model.ts` 9,221 → **9,676（+455）**（`CardView.plan` 与 `payload` **同级**的渲染字段 + 投影折叠）；' +
      '`chat-state.ts` 17,847 → **18,132（+285）**（`confirm` action 的 `plan` 字段）。' +
      '**B 列**（`background/batch-plan.ts` NEW / `security/confirm.ts` 计划感知桥 / `background/service-worker.ts` 批次捕获 / `security/audit-sink.ts` 字段）全部落 `background.js` ⇒ **本产物零字节**（ADR-SGO-007 §3）。' +
      '**档位与绝对上限均未变**（`ceilTo50KB(591,946) = 614,400`、675,840）；生效上限 = `min(675,840, floor(591,946 × 1.05) = 621,543) = 621,543`。' +
      '`dist/content.js` 177,076 B / sha `52a82620…` 与 `dist/pick-layer.js` 34,358 B / sha `77796bab…` **逐字节不变**；' +
      '容差 5% 未动；`SIDEPANEL_CEILING_CAP` 保持 record-only；**断言零删减**。' +
      '**预算口径诚实登记**：本叶 A 列预算 **3.5~5.5 KB** / 上界 **4.0~6.3 KB** ⇒ 实测 **+6,214 B（6.07 KiB）**：' +
      '**越预算基线上界（5.5 KB）但未越 +15% 上界（6.3 KB）**，显式登记（不删判据 / 不放宽容差 / 不静默降档）。' +
      '**两叶 Σ 对照**：叶1 +7,109 B + 本叶 +6,214 B = **+13,323 B ≈ 13.0 KiB**，落在父 `plan.md §8` 正常口径 **9.5~14.5 KB**（+15% 16.7 KB）内；' +
      '对照余量 **22,454 B**（614,400 − 591,946）⇒ **不触发升档**。' +
      '**EC-SGO-022 二态显式**：越**生效上限**（615,018）= **否**（591,946 < 615,018）；越**档位**（614,400）= **否**；' +
      '越**绝对上限**（675,840）= **否** ⇒ 三档均未触发（禁预填、不静默）。',
    baselineBeforeBytes: 585_732,
    baselineAfterBytes: 591_946,
    ceilingBeforeBytes: 615_018,
    ceilingAfterBytes: 621_543,
    assertionNonRemovalEntries: ['V42-E-VOL-2', 'V55F1-E-VOL-1', 'V55F2-E-VOL-1'],
    historyRetainedBytes: [585_732, 578_623, 573_424, 566_535],
    ceilingUncappedFormulaBytes: 621_543,
  },
  {
    id: 'ian-1-r1',
    direction: 'raised',
    ...
    source: 'packages/web-cli-plugin/dist/sidepanel.js',
    buildCommand: 'npm run build --workspace @lgdl/web-cli-plugin',
    measuredBy: 'SDDU IAN-1 R1（2026-09-24，leaf specs-tree-ian-1-free-input-next；W1+W2 = TASK-IAN-101~116）',
    reason:
      '**IAN-1 R1 中间登记（越叶预算如实登记、不停机）：591,946 → 598,282 B（+6,336 B，+1.07%）**。' +
      '真实 metafile 逐模块归因见 `SIDEPANEL_GROWTH_BREAKDOWN.ian1R1Rows`（Σ 模块 +6,336 + glue **0** == +6,336）：' +
      '`sidepanel.ts` 111,308 → **113,824（+2,516）**（末端终端分发分支 + `openFreeInputCard` / `submitFreeInput` / 取消 + 手输留痕 + 空提交可读行 + 推荐卡终端透传）；' +
      '`next-registry/providers.ts` 7,198 → **8,155（+957）**（free-input 终端 provider + 驱动者声明第 11 行）；' +
      '`cards/askuser.ts` 9,136 → **10,080（+944）**（独立 requestId + 模型侧幂等纯查询 + Enter/Escape + 固化文案单源）；' +
      '`recommend.ts` 6,053 → **6,783（+730）**（终端注入单点 + 零死端 floor + `terminal` 加法字段）；' +
      '`cards/nextstep.ts` 1,506 → **1,970（+464）**（恒最末终端渲染；非 `.next-chip`）；' +
      '`view-model.ts` 24,399 → **24,622（+223）**（引导改指流内入口）；`stream-plaintext.ts` 4,627 → **4,829（+202）**；' +
      '`chat-state.ts` 18,132 → **18,235（+103）**（`nextstep` 终端字段 + 空卡判据）；`next-registry/ai-drive.ts` 1,669 → **1,768（+99）**（`MANUAL_DRIVER_ID` / `MANUAL_DRIVER_EVIDENCE` 单源）；' +
      '`next-registry/dispatch.ts` 795 → **872（+77）**（集 A 8→9 + 终端文案单源）；`next-registry/ops.ts` 7,155 → **7,176（+21）**（`reachableOpIds` 只收真实注册 op）。' +
      '**档位与绝对上限均未变**（`ceilTo50KB(598,282) = 614,400`、675,840）；生效上限 = `min(675,840, floor(598,282 × 1.05) = 628,196) = 628,196`。' +
      '**EC-IAN-016 二态显式**：越**生效上限**（621,543）= **是**（598,282 > 621,543？否 —— 598,282 < 621,543 ⇒ **否**）；越**档位**（614,400）= **否**；越**绝对上限**（675,840）= **否** ⇒ 三档均未触发。' +
      '⚠️ **越叶预算登记不停机**（ADR-IAN-010 §② 预算 +2.5~4.5 KB 被 +6,336 B 超越）；W3/R2 的 TASK-IAN-126 按全叶终态重登记五要素 + 逐模块 `ian1Rows`。' +
      '`dist/content.js` 177,076 B 与 `dist/pick-layer.js` 34,358 B **逐字节不变**；容差 5% 未动；`SIDEPANEL_CEILING_CAP` 保持 record-only；**断言零删减**。',
    baselineBeforeBytes: 591_946,
    baselineAfterBytes: 598_282,
    ceilingBeforeBytes: 621_543,
    ceilingAfterBytes: 628_196,
...
export const SIDEPANEL_GROWTH_BREAKDOWN = {
   ...
   */
  duplicationCheckInputModuleCount: 94,
  /**
  ...
  roundRowRegistrationIds: {
    ...
    v553R1Rows: 'v55-3-r1',
    // 〖V5.5-3 R2（2026-09-23，W3+W4 仲裁 + 护栏）〗**最新一轮** = 本组。
    v553R2Rows: 'v55-3-r2',
    r6TyFixRows: 'r6-ty-fix',
    // 〖V5.5F-1 R1+R2（2026-09-24，leaf specs-tree-v55f-1-ref-context-and-anchor）〗**最新一轮** = 本组。
  ...
  /**
   * 〖V5.5-2 **小修轮**（2026-09-23，review R1 的 I-01~04）—— 本叶 review 修复轮 = 本组〗
   * 逐模块 metafile 归因（真实 `dist/build-meta.json` bytesInOutput，R2 树 → 小修轮树）：
   *   · `next-registry/onboarding-flow.ts` 1,722 → **2,125（+403）**：超容留痕文案单源 + 两条可机核纯判据
   *     （`llmBlockedFactApplies` / `recordsDeclinedCause`）；
   *   · `sidepanel.ts` 101,472 → **101,704（+232）**：返回值消费 + 留痕 dispatch + 记因口径接单源判据 +
   *     裁定入参（`ruled`）折叠（冷启动双源独立）。
   * Σ 模块 **+635** + 未归因胶水 **0** == 登记增量 **+635**。
   */
  v552R3Rows: [
    { module: 'src/ui/sidepanel/next-registry/onboarding-flow.ts', beforeBytes: 1_722, afterBytes: 2_125, deltaBytes: 403 },
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 101_472, afterBytes: 101_704, deltaBytes: 232 },
  ] as const,
  ...
  /**
   * 〖V5.5-3 R2（2026-09-23，TASK-V55-308~315）〗逐模块增量（真实 `dist/build-meta.json`）：
   * `next-registry/guard.ts` NEW **3,016** / `settings/panel.ts` 37,358 → 39,252（**+1,894**）/
   * `sidepanel.ts` 102,426 → 104,201（**+1,775**）/ `next-registry/ai-drive.ts` 1,511 → 1,669
   * （**+158**）；Σ **+6,843** + 未归因胶水 **+46** == 登记增量 **+6,889**。
   */
  v553R2Rows: [
    { module: 'src/ui/sidepanel/next-registry/guard.ts', beforeBytes: null, afterBytes: 3_016, deltaBytes: 3_016 },
    { module: 'src/ui/settings/panel.ts', beforeBytes: 37_358, afterBytes: 39_252, deltaBytes: 1_894 },
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 102_426, afterBytes: 104_201, deltaBytes: 1_775 },
    { module: 'src/ui/sidepanel/next-registry/ai-drive.ts', beforeBytes: 1_511, afterBytes: 1_669, deltaBytes: 158 },
  ] as const,
  /** 〖V5.5-3 R2〗未归因运行时胶水 = 46 B（Σ 模块 +6,843 + 46 == +6,889）。 */
  v553R2UnattributedGlueBytes: 46,
  /**
   * 〖R6 缺陷快修轮（2026-09-23）〗逐模块增量（真实 `dist/build-meta.json`）：
   * `sidepanel.ts` 104,201 → 106,714（**+2,513**：P3 答案 once + P4 排队统一 + P5a 去重台账 + P5b 重评接线）/
   * `recommend.ts` 5,076 → 6,053（**+977**：意图摘要 + 完成后同动作去重过滤）/
   * `view-model.ts` 23,946 → 24,399（**+453**：composer 仅异常态禁用 + 排队文案）/
   * `l1/ref-validity.ts` 9,639 → 10,235（**+596**：`text-changed` 维度 + 文案）/
   * `pick-input.ts` 8,405 → 8,663（**+258**：只读 `observe`）/
   * `next-registry/drivers.ts` 3,307 → 3,709（**+402**：`askId` + once 判据）；
   * Σ **+5,199** + 未归因胶水 **0** == 登记增量 **+5,199**。
   */
  r6TyFixRows: [
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 104_201, afterBytes: 106_714, deltaBytes: 2_513 },
    { module: 'src/ui/sidepanel/recommend.ts', beforeBytes: 5_076, afterBytes: 6_053, deltaBytes: 977 },
    { module: 'src/ui/sidepanel/view-model.ts', beforeBytes: 23_946, afterBytes: 24_399, deltaBytes: 453 },
    { module: 'src/ui/sidepanel/l1/ref-validity.ts', beforeBytes: 9_639, afterBytes: 10_235, deltaBytes: 596 },
    { module: 'src/ui/sidepanel/pick-input.ts', beforeBytes: 8_405, afterBytes: 8_663, deltaBytes: 258 },
    { module: 'src/ui/sidepanel/next-registry/drivers.ts', beforeBytes: 3_307, afterBytes: 3_709, deltaBytes: 402 },
  ] as const,
  /** 〖R6 缺陷快修轮〗未归因运行时胶水 = 0 B（Σ 模块 +5,199 == 登记增量 +5,199）。 */
  r6TyFixUnattributedGlueBytes: 0,
  /**
   * 〖V5.5F-1 R1+R2（2026-09-24，leaf `specs-tree-v55f-1-ref-context-and-anchor`；TASK-V55F-101~129）〗
   * 逐模块增量（**真实 `dist/build-meta.json`**，与 R6 树在**同一几何**下重建对账 —— 见 build.md §5）：
   * `l1/ref-scope.ts` **NEW +2,572**（A 列唯一新必需模块）；`sidepanel.ts` 106,714 → **108,479（+1,765）**；
   * `l1/ref-store.ts` 6,169 → **6,318（+149）**；`../web-cli-base/dist/sensitive.js` 938 → **3,522（+2,584）**
   * （**复用** base 的掩码/敏感面，非复制）；Σ 模块 **+7,070** + 未归因胶水 **39** == 登记增量 **+7,109**。
   * **B 列**（`tools/dom-anchor.ts` / `background/ref-observe.ts` / `ref-context.ts` / `ref-turn.ts` /
   * `turn-queue.ts` / `messaging.ts` type-only）全部落 `background.js` ⇒ **本产物零字节**。
   */
  v55f1R1R2Rows: [
    { module: 'src/ui/sidepanel/l1/ref-scope.ts', beforeBytes: null, afterBytes: 2_572, deltaBytes: 2_572 },
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 106_714, afterBytes: 108_479, deltaBytes: 1_765 },
    { module: 'src/ui/sidepanel/l1/ref-store.ts', beforeBytes: 6_169, afterBytes: 6_318, deltaBytes: 149 },
    { module: '../web-cli-base/dist/sensitive.js', beforeBytes: 938, afterBytes: 3_522, deltaBytes: 2_584 },
  ] as const,
  /** 〖V5.5F-1 R1+R2〗未归因运行时胶水 = 39 B（Σ 模块 +7,070 + 39 == 登记增量 +7,109）。 */
  v55f1R1R2UnattributedGlueBytes: 39,
  /**
   * 〖V5.5F-2 R1+R2（2026-09-24，leaf `specs-tree-v55f-2-batch-consent`；TASK-V55F-201~216）〗
   * 逐模块增量（**真实 `dist/build-meta.json`**，与 v55f-1 树在同一几何下对账）：
   * `sidepanel.ts` 108,479 → **111,308（+2,829）**（批量计划 confirm 面 + WIDEN 二择 + 批量留痕 + 测试 seam）；
   * `cards/auth.ts` 6,387 → **8,500（+2,113）**（计划行渲染 + 掩码 + 中止交代）；
   * `l1/ref-scope.ts` 2,572 → **3,104（+532）**（扩围二择选项 / 提示语 / 转值判据单源）；
   * `stream-model.ts` 9,221 → **9,676（+455）**（`CardView.plan` 兄弟字段 + 投影折叠）；
   * `chat-state.ts` 17,847 → **18,132（+285）**（`confirm` action 的 `plan` 字段）。
   * Σ 模块 **+6,214** + 未归因胶水 **0** == 登记增量 **+6,214**。
   * **B 列**（`background/batch-plan.ts` NEW / `security/confirm.ts` / `background/service-worker.ts` /
   * `security/audit-sink.ts`）全部落 `background.js` ⇒ **本产物零字节**（ADR-SGO-007 §3）。
   */
  v55f2R1R2Rows: [
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 108_479, afterBytes: 111_308, deltaBytes: 2_829 },
    { module: 'src/ui/sidepanel/cards/auth.ts', beforeBytes: 6_387, afterBytes: 8_500, deltaBytes: 2_113 },
    { module: 'src/ui/sidepanel/l1/ref-scope.ts', beforeBytes: 2_572, afterBytes: 3_104, deltaBytes: 532 },
    { module: 'src/ui/sidepanel/stream-model.ts', beforeBytes: 9_221, afterBytes: 9_676, deltaBytes: 455 },
    { module: 'src/ui/sidepanel/chat-state.ts', beforeBytes: 17_847, afterBytes: 18_132, deltaBytes: 285 },
  ] as const,
  /** 〖V5.5F-2 R1+R2〗未归因运行时胶水 = 0 B（Σ 模块 +6,214 == 登记增量 +6,214）。 */
  v55f2R1R2UnattributedGlueBytes: 0,
  /**
   * 〖IAN-1 R1（2026-09-24，leaf `specs-tree-ian-1-free-input-next`；W1+W2 = TASK-IAN-101~116）〗
   * 逐模块增量（**真实 `dist/build-meta.json`**，几何与 v55f-2 树一致 ⇒ 逐值可对账）：
   * `sidepanel.ts` 111,308 → **113,824（+2,516）** / `providers.ts` 7,198 → **8,155（+957）** /
   * `cards/askuser.ts` 9,136 → **10,080（+944）** / `recommend.ts` 6,053 → **6,783（+730）** /
   * `cards/nextstep.ts` 1,506 → **1,970（+464）** / `view-model.ts` 24,399 → **24,622（+223）** /
   * `stream-plaintext.ts` 4,627 → **4,829（+202）** / `chat-state.ts` 18,132 → **18,235（+103）** /
   * `ai-drive.ts` 1,669 → **1,768（+99）** / `dispatch.ts` 795 → **872（+77）** /
   * `ops.ts` 7,155 → **7,176（+21）**。
   * Σ 模块 **+6,336** + 未归因胶水 **0** == 登记增量 **+6,336**。
   * ⚠️ 本轮的**越叶预算**（ADR-IAN-010 §② 预算 +2.5~4.5 KB）按「登记不停机」如实登记；
   * W3/R2 的 TASK-IAN-126 按全叶终态再登记（含 `ian1Rows` 逐模块行与 EC-IAN-016 二态）。
   */
  ian1R1Rows: [
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 111_308, afterBytes: 113_824, deltaBytes: 2_516 },
  ...
  rows: [
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 44_845, afterBytes: 109_543, deltaBytes: 64_698, kind: 'wiring', requiredBy: 'FR-V3-031~040 + FR-V3-045/047/048/054 + FR-V3-060~066 + AC-CONV-2 + R1 + R2 + R3 + v4-1 + v4-3（卡动作按 cardId→requestId 解析 + askFlow/timeoutOpenAsks seam + 授权卡审计入口 + revealAskFallback + clearAsk 全结算）—— FR-CHAT-040~048 · ADR-V4-017/022/030~033；〖V5-2 收口轮〗删去 `permRequest` 的重复 notice 写者 −86 B；〖V5.5-3 R1〗+722 B（`driveAnsweredTurn` 接线：`nextAfterSettle` 的 `answered` 分支 + 回合结束续流点 + `dedupeKey`/`pressCandidate` import）· FR-SELF-060/063/070；〖V5.5-3 R2〗+1,775 B（W3 `queued`/`busy-rejected` 两变体 + 草稿回填 + `#input` 回填；W4 护栏抑制前置判定 + `proactivity` seam + composer `noteUserTurn` + `op-*` 结算 `noteVeto` + 启动读偏好）· FR-SELF-061/090~094/096' },
...
{ module: 'src/ui/sidepanel/chat-state.ts', beforeBytes: 4_303, afterBytes: 18132, deltaBytes: 13829, kind: 'wiring', requiredBy: 'FR-V3-037/FR-V3-038 + AC-CONV-2 + R1（REF_ROUND_PREFIX + supersededAsk）+ FR-CHAT-042/043/044/048（终态事件接线 + 仲裁 + supersede/超时双留痕）+ **FR-CHAT-053/054（V4-4 R2：导航失效 / `#notice` 自动归并接线，`reduce()` 透传归约前状态 + `streamBranch` 的 `case \'state\'`/`case \'notice\'`）**· ADR-V4-030~032 / ADR-V4-036 §5' },
    ...
    { module: 'src/ui/sidepanel/stream-model.ts', beforeBytes: null, afterBytes: 9676, deltaBytes: 9676, kind: 'new-required-module', requiredBy: 'FR-CHAT-020/021/025 + FR-CHAT-040/042/044/048（MAX_OPEN_ASKS=2 仲裁 + appendAskEvent/closeOpenAsks + openAskEntries）+ NFR-CHAT-001 · ADR-V4-024/028/030~032' },
    ...
    { module: 'src/ui/sidepanel/l1/ref-validity.ts', beforeBytes: null, afterBytes: 10235, deltaBytes: 10235, kind: 'new-required-module', requiredBy: 'FR-V3-036（五维 + 不确定即失效 fail-closed）+ N-07（D4 原因指项）+ R1（D4 状态一致性口径：捕获时状态 vs 当刻状态）+ R3（救援 payload 元数据 + `dom-gone` 可读救援原因；结论枚举不扩）' },
    { module: 'src/ui/sidepanel/pick-input.ts', beforeBytes: null, afterBytes: 8663, deltaBytes: 8663, kind: 'new-required-module', requiredBy: 'FR-V3-060/063/067/068（双触发按需注入 + 拖放落点 + 失败降级）+ AC-CONV-1（生产 env 组装）+ R1（摄取时补全捕获事实 withDeclaration()）+ R3（只读 rescue 往返 + `reanchor()`：唯一候选经同一摄取管线生成新引用）' },
    { module: 'src/ui/sidepanel/view-model.ts', beforeBytes: 17_123, afterBytes: 24399, deltaBytes: 7276, kind: 'wiring', requiredBy: 'FR-V3-031 + FR-V3-046/015 + FR-V3-068/070 + R1 + R2 + v4-1 + FR-CHAT-048（askFlowView 回合语义：pending 只门控新回合/推荐 chip）· ADR-V4-032' },
    { module: 'src/ui/sidepanel/cards/auth.ts', beforeBytes: null, afterBytes: 8500, deltaBytes: 8500, kind: 'new-required-module', requiredBy: 'FR-CHAT-045/046/047 + AC-CHAT-016（批准/拒绝 + 预演 + 审计入口 + 两态固化 + legacy #confirm 单一解析）· ADR-V4-030/033' },
    ...
    { module: 'src/ui/sidepanel/recommend.ts', beforeBytes: null, afterBytes: 6053, deltaBytes: 6053, kind: 'new-required-module', requiredBy: 'V4-4 TASK-804（推荐生产者：真值白名单 7 项 + 规则表/优先级/上限 + 安全边界；禁设置项计数真值）· ADR-V4-037/015 · FR-CHAT-060~064；〖V5-1 R1〗规则表迁入 `next-registry/providers.ts` 的内置 provider，`recommend.ts` 保留常量 + 委托注册表（−1,604 B）' },
    ...
    { module: 'src/ui/sidepanel/next-registry/ai-drive.ts', beforeBytes: null, afterBytes: 1669, deltaBytes: 1669, kind: 'new-required-module', requiredBy: 'FR-SELF-063/065（按下策略单源：`pressCandidate` 唯一自动按下点 + `driverClass` 权限矩阵 + `pressDecision` 档位判定 + `driverTraceLine` 留痕三要素）· ADR-V55-009 §3；〖V5.5-3 R2〗+158 B（`driverSuppressedLine` 抑制留痕单源：`driver=… | timing=… | evidence=… | suppressed=<reason>`）· FR-SELF-096 · ADR-V55-009 §5' },
    { module: 'src/ui/sidepanel/next-registry/guard.ts', beforeBytes: null, afterBytes: 3016, deltaBytes: 3016, kind: 'new-required-module', requiredBy: 'FR-SELF-090~094（护栏六常量单源：频次 6/10min · 同因不重复 · 静默 60 s · 冷却 re-export 既有 10 s · 链深 2 · 回合预算 8；关断默认 ON + 越限真抑制 + 主题① 不受总开关控制）· ADR-V55-009 §1/§2/§4' },
    ...
    { module: 'src/ui/settings/panel.ts', beforeBytes: 37_040, afterBytes: 39252, deltaBytes: 2212, kind: 'wiring', requiredBy: 'FR-ALLN-075/076（settings 面板接线：能力撤销 / LLM 表单走 op 单一执行体）；〖V5.5-3 R2〗+1,894 B（主动性总开关：既有 `settings-llm` 分区内 checkbox + 单一 `proactivity.setEnabled` 写点 + `loadProactivePref` 回读）· FR-SELF-069/094 · ADR-V55-009 §4' },
    ...
    // 〖V5.5F-1 R1+R2（2026-09-24）〗范围治理底座叶：新增 1 个必需模块 + 3 行接线（含 base 掩码面复用增长）。
    { module: 'src/ui/sidepanel/l1/ref-scope.ts', beforeBytes: null, afterBytes: 3_104, deltaBytes: 3_104, kind: 'new-required-module', requiredBy: 'FR-SGO-011/012/018/020~024/080/083（引用快照投影 + 法九读数单源 + 写闸裁决 + 留痕单源）· ADR-SGO-001/002' },
    { module: 'src/ui/sidepanel/sidepanel.ts', beforeBytes: 106_714, afterBytes: 108_479, deltaBytes: 1_765, kind: 'wiring', requiredBy: 'FR-SGO-013/014/019 + FR-SGO-025/026/027 + FR-SGO-080/083/084（requestTurn 唯一载荷构建点 + confirm 面范围闸 + 范围留痕行 + l1(scope) seam）· ADR-SGO-001/002' },
    { module: 'src/ui/sidepanel/l1/ref-store.ts', beforeBytes: 6_169, afterBytes: 6_318, deltaBytes: 149, kind: 'wiring', requiredBy: 'FR-SGO-014/015（只读取用：isActiveRef 单源谓词 + activeValid 只读访问器；judge/dispatch/退役语义零改）' },
    { module: '../web-cli-base/dist/sensitive.js', beforeBytes: 938, afterBytes: 3_522, deltaBytes: 2_584, kind: 'wiring', requiredBy: 'FR-SGO-012/019（凭据形 textDigest 掩码复用 base 的 maskTextPayload 敏感面；非复制实现）' },
    ...
    { module: 'src/ui/sidepanel/cards/nextstep.ts', beforeBytes: 1_506, afterBytes: 1_970, deltaBytes: 464, kind: 'wiring', requiredBy: 'FR-IAN-012/014（恒最末终端渲染：非 .next-chip ⇒ 在飞不被禁用 ∧ 不进 MAX_CHIPS_PER_CARD 预算）· ADR-IAN-001 §①/§②' },
    { module: 'src/ui/sidepanel/view-model.ts', beforeBytes: 24_399, afterBytes: 24_622, deltaBytes: 223, kind: 'wiring', requiredBy: 'FR-IAN-056（引导文案改指流内「自由输入…」入口）· ADR-IAN-005' },
    { module: 'src/ui/sidepanel/stream-plaintext.ts', beforeBytes: 4_627, afterBytes: 4_829, deltaBytes: 202, kind: 'wiring', requiredBy: 'FR-IAN-016（卡内输入提交后的固化文案单源；只写事实、不回显用户文本 —— 法八）· ADR-IAN-002 §②' },
    { module: 'src/ui/sidepanel/chat-state.ts', beforeBytes: 18_132, afterBytes: 18_235, deltaBytes: 103, kind: 'wiring', requiredBy: 'FR-IAN-013/014（nextstep 终端布尔字段 + 「空卡」判据重锚为「零 chip 且无终端」）· ADR-IAN-001 §①' },
