...
test('V3-VOL-1 ③ growth: the recorded per-module breakdown sums to the measured delta', () => {
  ...
  assert.equal(b.deltaBytes, 303_352);
  const bucketSum =
...
test('V3-VOL-1 ③ growth: the real esbuild metafile agrees with the recorded breakdown (when built)', (t) => {
  ...
  // ★ IAN-2 R1：最新一轮 = `ian2Rows`（净负 −548 B；其 afterBytes 必须等于真实 metafile）。
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.ian2Rows) {
...
test('V3-VOL-1 ③(V4-1) growth: the v4-1 round rows sum to `closeoutDeltaBytes` (I9)', () => {
  ...
  assert.equal(b.closeoutDeltaBytes, SIDEPANEL_BASELINE_BYTES - 599_125);
...
test('V3-VOL-1 ③(V4-1) growth: the v4-1 round `afterBytes` must match the real metafile (I9)', (t) => {
  ...
  // ★ IAN-2 R1：最新一轮 = `ian2Rows`（净负；afterBytes == 真实 metafile）。
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.ian2Rows) {
    const key = paths.find((p) => p.endsWith(row.module));
...
test('V4.5-1 R3 growth: 终轮（Δ=0）历史登记 + 最新一轮 metafile 逐值一致（Σ Δ 可核）', (t) => {
  ...
  let judged = 0;
  // ★ IAN-2 R1：最新一轮 = `ian2Rows`（净负 −548 B）；其 afterBytes 必须与真实 metafile 逐值相等。
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.ian2Rows) {
    const key = paths.find((p) => p.endsWith(row.module));
    assert.ok(key, `metafile 缺少模块 ${row.module}`);
    assert.equal(
...
test('R6 缺陷快修轮体积定稿：五要素齐备 ∧ 未跨档位 ∧ 三值同源 ∧ pending-author-line 未伪称', (t) => {
  ...
  // 〖★ IAN-2 R1（2026-09-25）〗末条前移：最新一轮 = `ian-2-r1`（IAN-1 R2 / R1 与 V5.5F-2 的历史五要素逐字保留在登记册与元数据段）。
  assert.equal(fr.id, 'ian-2-r1', '末条登记必须是 IAN-2 R1（最新一轮）');
  // ① **净负轮**：方向 `lowered` ∧ Δ == after − before < 0（不得伪装成提升/零字节）。
  assert.equal(fr.direction, 'lowered', 'IAN-2 R1 必须登记为净负轮（`lowered`）');
  assert.ok(fr.baselineAfterBytes - fr.baselineBeforeBytes < 0, 'IAN-2 R1 必须是负增量（净负）');
  ...
  assert.equal(fr.ceilingUncappedFormulaBytes, 628_505);
  ...
  const rows = SIDEPANEL_GROWTH_BREAKDOWN.ian2Rows;
  assert.ok(rows.length > 0, 'IAN-2 R1 逐模块 rows 必须非空（净负轮）');
  assert.equal(
    rows.reduce((n, r) => n + r.deltaBytes, 0) + SIDEPANEL_GROWTH_BREAKDOWN.ian2UnattributedGlueBytes,
    fr.baselineAfterBytes - fr.baselineBeforeBytes,
    'IAN-2 R1 Σ 逐模块 + glue 必须 == 登记增量（净负）',
  );
  ...
  console.log(`  ℹ IAN-2 R1 净负轮：Δ=${rows.reduce((n: number, r: { deltaBytes: number }) => n + r.deltaBytes, 0)} · 五要素 = ${SIDEPANEL_BASELINE_BYTES} / ${SIDEPANEL_CEILING} / ${SIDEPANEL_TIER_BYTES} / ${PENDING_ABSOLUTE_CAP.absoluteCeilingBytes} / pending-author-line（未跨档位）`);
});
