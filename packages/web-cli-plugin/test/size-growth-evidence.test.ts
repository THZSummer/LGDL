...
test('V3-VOL-1 ③ growth: the recorded per-module breakdown sums to the measured delta', () => {
  ...
  assert.equal(b.deltaBytes, 259_351);
  const bucketSum =
...
test('V3-VOL-1 ③ growth: every new/wiring row cites the requirement that forces it (no unexplained bytes)', () => {
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.rows) {
    ...
    if (row.kind === 'new-required-module') {
      ...
      assert.match(row.requiredBy, /FR-(V3|CHAT|V45|ALLN|SELF)-\d\d\d|NFR-(V3|CHAT|V45|ALLN|SELF)-\d\d\d/, `${row.module}: 必须引到 FR/NFR（v4.5-1 起接受 FR-V45-*：本 Feature 自己的需求编号）`);
    }
    if (row.kind === 'wiring') {
...
test('V3-VOL-1 ③ growth: the real esbuild metafile agrees with the recorded breakdown (when built)', (t) => {
  ...
  // （v5-2 各轮的历史 rows 逐字保留在 `v52*Rows` 与 `SIDEPANEL_BASELINE_META` 的历史段）。
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.v551R1Rows) {
...
test('V3-VOL-1 ③(V4-1) growth: the v4-1 round rows sum to `closeoutDeltaBytes` (I9)', () => {
  ...
  // 〖R4 缺陷修复轮（2026-09-22）〗最新一轮 = `r4-selector-fix`（547,558 → **549,609**，+2,051）
  // ⇒ 与它的登记条目同源复算；V5-3 review R1（546,370 → 547,558）作为历史锚点逐字保留在注释里。
  assert.equal(b.closeoutDeltaBytes, SIDEPANEL_BASELINE_BYTES - 549_609);
...
test('V4.5-1 R3 growth: 终轮（Δ=0）历史登记 + 最新一轮 metafile 逐值一致（Σ Δ 可核）', (t) => {
  ...
  // ② 逐模块：与**最新一轮**（V5-3 R2 末叶收口轮）登记的 afterBytes 逐值相等 ⇒ 真实 metafile
