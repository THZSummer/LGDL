...
test('V3-VOL-1 ③ growth: the recorded per-module breakdown sums to the measured delta', () => {
  ...
  assert.equal(b.deltaBytes, 290_507);
  const bucketSum =
...
test('V3-VOL-1 ③ growth: every new/wiring row cites the requirement that forces it (no unexplained bytes)', () => {
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.rows) {
    ...
    if (row.kind === 'new-required-module') {
      ...
      assert.match(row.requiredBy, /FR-(V3|CHAT|V45|ALLN|SELF|SGO)-\d\d\d|NFR-(V3|CHAT|V45|ALLN|SELF|SGO)-\d\d\d/, `${row.module}: 必须引到 FR/NFR（v4.5-1 起接受 FR-V45-*；V5.5F-1 起接受 FR-SGO-*：范围治理叶自己的需求编号）`);
    }
    if (row.kind === 'wiring') {
...
test('V3-VOL-1 ③ growth: the real esbuild metafile agrees with the recorded breakdown (when built)', (t) => {
  ...
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2Rows) {
...
test('V3-VOL-1 ③(V4-1) growth: the v4-1 round rows sum to `closeoutDeltaBytes` (I9)', () => {
  ...
  assert.equal(b.closeoutDeltaBytes, SIDEPANEL_BASELINE_BYTES - 578_623);
...
test('R6 缺陷快修轮体积定稿：五要素齐备 ∧ 未跨档位 ∧ 三值同源 ∧ pending-author-line 未伪称', (t) => {
  ...
  assert.equal(fr.id, 'v55f-1-r2', '末条登记必须是 V5.5F-1 R1+R2（最新一轮）');
  // ① 提升轮：方向 `raised` ∧ Δ == after − before（不得伪装成零字节/净减）。
  assert.equal(fr.direction, 'raised', 'V5.5F-1 R1+R2 必须登记为提升轮（`raised`）');
  assert.ok(fr.baselineAfterBytes - fr.baselineBeforeBytes > 0, 'V5.5F-1 R1+R2 必须是正增量');
  ...
  assert.equal(fr.ceilingUncappedFormulaBytes, 615_018);
  ...
  // ④ 最新一轮逐模块归因：Σ + glue == 登记增量（提升轮）。
  const rows = SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2Rows;
  assert.ok(rows.length > 0, 'V5.5F-1 R1+R2 逐模块 rows 必须非空（提升轮）');
  assert.equal(
    rows.reduce((n, r) => n + r.deltaBytes, 0) + SIDEPANEL_GROWTH_BREAKDOWN.v55f1R1R2UnattributedGlueBytes,
    fr.baselineAfterBytes - fr.baselineBeforeBytes,
    'V5.5F-1 R1+R2 Σ 逐模块 + glue 必须 == 登记增量',
  );
