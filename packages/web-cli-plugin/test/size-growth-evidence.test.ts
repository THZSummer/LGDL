...
test('V3-VOL-1 ③ growth: the recorded per-module breakdown sums to the measured delta', () => {
  ...
  assert.equal(b.deltaBytes, 267_920);
  const bucketSum =
...
test('V3-VOL-1 ③ growth: the real esbuild metafile agrees with the recorded breakdown (when built)', (t) => {
  ...
  for (const row of SIDEPANEL_GROWTH_BREAKDOWN.v552R2Rows) {
...
test('V3-VOL-1 ③(V4-1) growth: the v4-1 round rows sum to `closeoutDeltaBytes` (I9)', () => {
  ...
  // （562,273 → 563,145，+872）；R1 的中间登记值 557,883 → 562,273 逐字保留在 `SIDEPANEL_RE_REGISTRATIONS` 与 TIMELINE。
  assert.equal(b.closeoutDeltaBytes, SIDEPANEL_BASELINE_BYTES - 562_273);
