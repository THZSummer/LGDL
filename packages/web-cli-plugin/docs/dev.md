...
| 维度 | 阈值 | 落点 / 机制 |
|------|------|-------------|
| content script 注入体积 | `content.js` ≤ **64 KB**（IIFE，按需注入；无静态 `content_scripts`，空闲零开销）〔**NFR-007 目标，当前未达成**：实测 ≈1.02 MiB，超约 16.4×，D31〕 | `build.mjs` 产物；`test/perf-budget.test.ts`（有 dist 时按 **基线回归守卫** 断言，基线快照 `test/perf-baseline.ts`；**64KB 目标 ≠ 回归基线**，不得以基线值宣布达标） |
...
### 8.2 实测（2026-09-12 初测；2026-09-13 复核订正体积；2026-09-13 V2-4 回填 sidepanel/background）

| 对象 | 实测 | 阈值 | 判定 |
|------|------|------|:--:|
| `dist/content.js` | **1,073,453 B**（≈1,048.3 KiB ≈1.02 MiB）〔初测（2026-09-12）**33.9 KB**（34,711 B）；后因 base DOM 值导入 + 内联 LLM SDK 增至 1.0MB 级；**2026-09-13 V2-4 复核实测仍为 1,073,453 B（零增长）**〕 | ≤ 64 KB（**目标预算**） | ❌ **未达成**（超 ≈16.4×，D31） |
| `dist/sidepanel.js` | **1,159,856 B**（≈1.11 MiB）〔初测 **15.1 KB**；v1 时点 1,068,165 B；V2-2 时点 1,085,389 B；v2 P0 收口 1,110,744 B；**2026-09-13 V2-4 回填实测 1,132,748 B**（基线 = 1,132,748 B / ceiling = **1,189,385 B**）；**2026-09-13 v2 R2 第 2 轮回填实测 1,159,856 B**（来源 `stat -c %s packages/web-cli-plugin/dist/sidepanel.js`；`test/size-baseline.ts` 基线 = 1,159,856 B / ceiling = **1,217,848 B**；历史 1,068,165 / 1,085,389 / 1,110,744 / 1,132,748 全保留，**以现状为准**）〕 | — | 记录 |
| `dist/background.js` | **1,403,170 B**（≈1.34 MiB）〔初测 **968,442 B**（≈945.7 KB）；§8.2 原记录 1,372,225 B（时点早于 v2 各子 Feature）；**2026-09-13 V2-4 回填实测 1,403,170 B**（V2-4 additive 注入 `catalogMeta` 的**实测增量 = +549 B**，见 `build.md` §管理口径）〕 | ≤ 1.2 MB | ⚠️ 超目标（仅记录，无硬断言） |
| `dist/options.js` | **978,471 B**（≈955.5 KiB）〔初测 **893.9 KB**〕 | — | 记录 |
