...
### 8.2 实测（2026-09-12 初测；2026-09-13 复核订正体积；2026-09-13 V2-4 回填 sidepanel/background；2026-09-14 LLM SDK 惰性化后全量复测）
...
| 对象 | 实测 | 阈值 | 判定 |
|------|------|------|:--:|
| `dist/content.js` | **177,076 B**（≈172.9 KiB）〔初测（2026-09-12）**33.9 KB**（34,711 B）；后续因 base DOM 值导入 + 内联 LLM SDK 增至 **1,073,453 B**（≈1.02 MiB），并保持零增长至 2026-09-13；**2026-09-14 LLM SDK 惰性化后复测 177,076 B（−896,377 B / −83.5%）**〕 | ≤ 64 KB（**目标预算**） | ❌ **仍未达成**（**超 2.70×**；D31 未消除。历史叙述「超 ≈16.4×」对应 1,073,453 B 时点，保留） |
| `dist/sidepanel.js` | **266,500 B**（≈260.3 KiB）〔初测 **15.1 KB**；v1 时点 1,068,165 B；V2-2 1,085,389 B；v2 P0 收口 1,110,744 B；V2-4 1,132,748 B；v2 R2 第 2 轮 1,159,856 B；R2 收口时点 1,162,942 B；**2026-09-14 LLM SDK 惰性化后复测 266,500 B（−896,442 B / −77.1%）**（来源 `stat -c %s packages/web-cli-plugin/dist/sidepanel.js`；`test/size-baseline.ts` 回归基线 = 1,159,856 B / ceiling = **1,217,848 B**，**上限式守卫原样保留、未放宽**；历史值全保留，**以现状为准**）〕 | — | 记录 |
...
| `dist/options.js` | **82,093 B**（≈80.2 KiB）〔初测 **893.9 KB**（978,471 B 时点）；**2026-09-14 惰性化后 82,093 B（−896,378 B / −91.6%）**〕 | — | 记录 |
...
>
> **订正说明（2026-09-14，LLM SDK 惰性化）**：`content.js` 实测由 **1,073,453 B → 177,076 B**，**NFR-007 的 64 KiB 目标仍未达成**（2.70×，D31 保留、未消除）。**是否达标不随体积下降自动宣布**：`test/perf-baseline.ts` 的 `targetMet` 与 `test/size-budget.test.ts` 的 `≤ 64 KiB` 一致性断言均按**实测**判定（当前 `targetMet=false` 与实测一致，故**未改动任何阈值/断言**）；`test/size-baseline.ts` 的 `CONTENT_MAX_BYTES`（1,073,453 B 硬上限）与 `SIDEPANEL_CEILING`（1,217,848 B）为**上限式**守卫，本轮**原样保留、未放宽、未删除**。
>
