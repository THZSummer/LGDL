...
| 维度 | 阈值 | 落点 / 机制 |
|------|------|-------------|
| content script 注入体积 | `content.js` ≤ **64 KB**（IIFE，按需注入；无静态 `content_scripts`，空闲零开销）〔**NFR-007 目标，当前仍未达成**：**现状实测 177,076 B**（≈172.9 KiB），超 **2.70×**，D31 仍未消除；历史值 1,073,453 B（≈1.02 MiB，超 ≈16.4×）——2026-09-14 base LLM SDK 惰性化后 **下降 896,377 B（−83.5%）**〕 | `build.mjs` 产物；`test/perf-budget.test.ts`（有 dist 时按 **基线回归守卫** 断言，基线快照 `test/perf-baseline.ts` = **177,076 B** / 容差 5%；**64KB 目标 ≠ 回归基线**，不得以基线值宣布达标）；硬上限 `test/size-baseline.ts` `CONTENT_MAX_BYTES` = **177,076 B**（无容差，+1 字节即 FAIL；2026-09-14 收紧）；防回潮结构门禁见 base `src/llm-lazy-sdk.test.ts` |
...
### 8.2 实测（2026-09-12 初测；2026-09-13 复核订正体积；2026-09-13 V2-4 回填 sidepanel/background；2026-09-14 LLM SDK 惰性化后全量复测；2026-09-14 守卫收紧轮）

> **现状订正（2026-09-14，LLM SDK 惰性化）：** 下表「实测」列为**当前实测值**；括号内保留历史轮次值。触发变更：作者 2026-09-14 显式授权修改 `packages/web-cli-base/**`（仅本性能修复），把 `src/llm.ts` 顶层静态 `import OpenAI from 'openai'` / `import Anthropic from '@anthropic-ai/sdk'` 改为 `import type` + `chat()` 内 `await import(...)`。**同一根因**（base barrel `index.ts` 急切 re-export `llm.ts`）此前把 `content.js` 撑到 1.0 MiB 级（NFR-007 未达成 D31）并让 lgdl-web dev 首屏因 Vite 中途发现重依赖触发 optimizeDeps reload。改动细节 / 证据链 / 门禁原文见 `build.md §R3-perf`。

| 对象 | 实测 | 阈值 | 判定 |
|------|------|------|:--:|
| `dist/content.js` | **177,076 B**（≈172.9 KiB）〔初测（2026-09-12）**33.9 KB**（34,711 B）；后续因 base DOM 值导入 + 内联 LLM SDK 增至 **1,073,453 B**（≈1.02 MiB），并保持零增长至 2026-09-13；**2026-09-14 LLM SDK 惰性化后复测 177,076 B（−896,377 B / −83.5%）**；**同日守卫收紧：`CONTENT_MAX_BYTES` 1,073,453 → 177,076 B**（来源 `stat -c %s dist/content.js`，`npm run build --workspace @lgdl/web-cli-plugin`）〕 | ≤ 64 KB（**目标预算**） | ❌ **仍未达成**（**超 2.70×**；D31 未消除。历史叙述「超 ≈16.4×」对应 1,073,453 B 时点，保留） |
| `dist/sidepanel.js` | **266,500 B**（≈260.3 KiB）〔初测 **15.1 KB**；v1 时点 1,068,165 B；V2-2 1,085,389 B；v2 P0 收口 1,110,744 B；V2-4 1,132,748 B；v2 R2 第 2 轮 1,159,856 B；R2 收口时点 1,162,942 B；**2026-09-14 LLM SDK 惰性化后复测 266,500 B（−896,442 B / −77.1%）**（来源 `stat -c %s packages/web-cli-plugin/dist/sidepanel.js`；**同日守卫收紧：`test/size-baseline.ts` 回归基线 1,159,856 → 266,500 B / ceiling 1,217,848 → 279,825 B**〔= floor(266,500 × 1.05)，容差 5% 不变〕，方向**只收紧**；历史值 1,068,165 / 1,085,389 / 1,110,744 / 1,132,748 / 1,159,856 / 1,162,942 全保留，**以现状为准**）〕 | — | 记录 |
| `dist/background.js` | **1,587,839 B**（≈1.51 MiB）〔初测 **968,442 B**（≈945.7 KB）；2026-09-13 V2-4 回填实测 1,403,170 B；R2 收口时点 1,432,228 B；**2026-09-14 LLM SDK 惰性化后 1,587,839 B（+155,611 B / +10.9%）**——原因：`background` 仍**真实使用** `chat()`，其动态 import 在 esbuild **无 code-splitting** 下被内联为惰性模块，命名空间需完整保留，故**不再能对两个 SDK 的未用导出做 tree-shaking**（换得后台仅在真正调用时才求值）〕 | ≤ 1.2 MB | ⚠️ 超目标（仅记录，无硬断言；与惰性化前同为「已超」，增量如实登记） |
| `dist/options.js` | **82,093 B**（≈80.2 KiB）〔初测 **893.9 KB**（978,471 B 时点）；**2026-09-14 惰性化后 82,093 B（−896,378 B / −91.6%）**〕 | — | 记录（**无体积守卫**：`test/size-budget.test.ts` / `test/size-baseline.ts` 未对 `options.js` 设上限/基线，故本轮无可收紧项——如实说明，非静默遗漏） |
| 50 次已授权读派发 | ~0.9 ms/call（200 次 180.9 ms；含 setup）〔初测，2026-09-12〕 | < 5 ms/call | ✅ |
| 500 turn 提交后快照 | 40 turn / JSON < 64 KB | ≤ 40 turn | ✅ |
| 会话历史上限 | 40 | 40 | ✅ |

> **订正说明（2026-09-13）**：上表体积为 `npm run build` 后 `dist/` 真实字节复核。`content.js` 的体积回归守卫**基线**（`test/perf-baseline.ts` = 1,073,453 B / 容差 5%）与**目标预算**（64 KB，NFR-007）是**两个不同概念**：守卫防新增膨胀，目标仍未达成。另 3 条阈值（上下文 10 / 会话 40 / 审计 500 / 50 次派发）本轮实测仍 PASS，未订正。
>
> **订正说明（2026-09-14，LLM SDK 惰性化）**：`content.js` 实测由 **1,073,453 B → 177,076 B**，**NFR-007 的 64 KiB 目标仍未达成**（2.70×，D31 保留、未消除）。**是否达标不随体积下降自动宣布**：`test/perf-baseline.ts` 的 `targetMet` 与 `test/size-budget.test.ts` 的 `≤ 64 KiB` 一致性断言均按**实测**判定（当前 `targetMet=false` 与实测一致，故**该轮未改动任何阈值/断言**）；`test/size-baseline.ts` 的 `CONTENT_MAX_BYTES`（当时 1,073,453 B 硬上限）与 `SIDEPANEL_CEILING`（当时 1,217,848 B）为**上限式**守卫，**该轮**原样保留、未放宽、未删除。〔**该轮结论已被下一段「守卫收紧轮」部分订正，历史保留、以现状为准。**〕
>
> **订正说明（2026-09-14，守卫收紧轮 —— 锁死 SDK 惰性化战果）**：上一轮如实登记的残留（`content.js` 回归上限仍是 1,073,453 B、sidepanel ceiling 仍是 1,217,848 B）意味着**体积可悄悄长回 ~1 MiB 而守卫不响**。本轮按实测值**显式收紧**（方向只准更严，容差/断言零放宽、零删除）：
> - `test/size-baseline.ts` `CONTENT_MAX_BYTES`：**1,073,453 → 177,076 B**（硬上限、无容差；反证 `177,077` 必须 FAIL，且旧值 1,073,453 B 亦已 FAIL）。
> - `test/size-baseline.ts` `SIDEPANEL_BASELINE_BYTES`：**1,159,856 → 266,500 B**；`SIDEPANEL_CEILING` **1,217,848 → 279,825 B**（= floor(266,500 × 1.05)，容差 5% 不变）；`HISTORY` 追加保留 1,159,856 / 1,162,942（连同 1,068,165 / 1,085,389 / 1,110,744 / 1,132,748）；`targetBudgetBytes/targetMet` 仍 `null`。
> - `test/perf-baseline.ts` `CONTENT_BUNDLE_BASELINE_BYTES`：**1,073,453 → 177,076 B**（同一产物、同一漏洞面，一并收紧；反证旧值已 FAIL；`targetMet` 仍按实测 `false`，D31 保留）。
> - `dist/options.js` **无体积守卫**（未设上限/基线）→ 本轮**无可收紧项**，如实说明。
> - `dist/background.js` **只记录、无硬断言**（既有约定）→ 本轮**只登记** +155,611 B 的原因与取舍，未新增断言。
> **所有反证自测随新值更新并已实跑证明仍会 FAIL**（`npm test --workspace @lgdl/web-cli-plugin` 696/696 · 0 fail）。测量日期 2026-09-14、来源命令 `npm run build --workspace @lgdl/web-cli-plugin` + `stat -c %s dist/*.js`。
>
...
>
> **已知副作用（2026-09-13，v2 P0 收口登记）**：`test:hardening` 的 C 场景会**中途重新执行 `npm run build`**（用于验证「未重载扩展 → 构建戳不一致」诊断），因此跑完后 `dist/*` 的 build stamp 会变化（本次实测 `2026-09-13T09:05:42Z` → `09:25:16Z`）。**字节数不变**（`content.js` 1,073,453 B / `sidepanel.js` 1,110,744 B），无功能影响，但该门禁对 `dist/` **并非纯只读**——**属既有行为**（非 v2 引入），**仅如实登记、不改 hardening 逻辑**。若在串行门禁中紧随其后做体积/哈希断言，请以**重新 build 后**的产物为准。
...
- **探测失败/未知**（`unknown`：声明无效 / 版本不匹配 / 暂时不可达）：显示**可读原因** + **全自动重试状态**（「正在自动探测…（第 N 次重试）」；TASK-032 起已移除手动「重新探测」按钮，详见 §15）。
...
| 已在目标站点但发现态非 `supported` | 由 `#discovery-notice` 说明（未声明 = 设计如此非故障；未知 = 可读原因 + 自动重试中） | 等待自动重试完成，或换到声明了协议的站点（TASK-032 起探测全自动，无手动入口） |
...
### 12.4 标签页管理工具 `tabs` 与隐私开关（FR-049，作者决策③ 2026-09-12；**v1.5 作者裁决反转 2026-09-13 放开 close**）

作者已同意新增 `tabs` 权限（接受 Chrome 安装提示「读取您的浏览记录」），用于**插件级**工具 `tabs`。它是 v0.9 增补中**唯一新增**的 LLM 工具，且**不受 origin 授权门禁**（无需站点绑定即可用），但其敏感子命令仍走 policy 风险档。

> **⚠ 作者裁决反转（2026-09-13）**：作者明确撤销初版「**明确不做 `close`**」的约束——「标签页读写」包含 close，**完全放开 `close`**，并补齐单标签页写操作 `mute`/`pin`/`move`。这是作者**显式撤销此前约束**导致的**需求变更**（见 `spec.md` FR-049/FR-053 + EC-026、`plan.md` ADR-015 修订），**不是测试降级**；安全门禁**未放宽**（见下），**零新权限**（`chrome.tabs.remove/update/move` 在既有 `tabs` 权限下可用、`manifest.json` 零 diff）。此前断言「无 close」的测试已**替换**为「close 存在 + write→ask + 确认/审计/单标签页」（断言总数只增不减）。

| 子命令 | risk | 行为 | 使用方法 |
|--------|:----:|------|---------|
| `list` | `read`（allow） | 列出打开的标签页（id / 标题 / URL / 是否激活 / 已授权 / 会话；静音/固定态） | `tabs list`；默认**仅 origin+path**（去 query/fragment）；`tabs list --full true` 返回完整 URL（隐私影响已在输出/文档披露） |
| `switch` | `ui`（ask） | 激活指定标签页 → 复用 `bindTab`（origin→注入→`bindOrigin`）→ **切到该 origin 的会话** | `tabs switch --id <tabId>` 或 `--match <域名/URL 片段>`；受限页（`chrome://` 等）可读拒绝 |
| `open` | `write`（ask） | 打开新标签页并尽力自动绑定；确认摘要显示目标 URL | `tabs open --url <http(s) URL>`；**仅 http(s)**，`javascript:`/`data:`/`file:`/`chrome:`/`about:` 等可读拒绝 |
| `mute` | `write`（ask） | 静音 / 取消静音**单个**标签页 | `tabs mute (--id|--match) --muted true\|false` |
| `pin` | `write`（ask） | 固定 / 取消固定**单个**标签页 | `tabs pin (--id|--match) --pinned true\|false` |
| `move` | `write`（ask） | 移动**单个**标签页到位置/窗口 | `tabs move (--id|--match) [--index <n>] [--window <id>]`（至少一个目标） |
| `close` | `write`（ask） | **关闭单个**标签页（不可逆；一次只关一个） | `tabs close (--id <tabId> \| --match <域名/URL 片段>)`；受限页/未知 id 可读拒绝 |

- **close 安全处置（v1.5，严格 fail-safe）**：① risk 恒 `write`→默认 `ask`（`subcommandRisks`，**不得放宽**，`tabs` 为 `group:'plugin'` 不纳入自动授权）；② **禁止批量**——`--all` / 任何批量意图 → 可读拒绝（`tabs-batch-rejected`）；③ `--match` **歧义不猜**（命中 ≥2 个 → 可读拒绝并列出候选，**不自动挑激活页**）；④ 确认摘要经 `ConfirmBridgeOptions.describe` 显示「目标标签页 [id] **标题** — **去 query/fragment 的 URL**」并提示「**关闭不可逆；若它是当前侧栏所在页面，侧栏也会一并关闭**」；⑤ 受限页 / 未知 `--id` → 可读拒绝，不静默；⑥ 每个子命令入审计（`type:'tabs'`，零明文：URL 去 query/fragment，**URL 型标题同样去参**）。
- **隐私默认与标题脱敏**：`list` 只返回 `origin + path`；浏览器对无标题页会用完整 URL 作标题，`redactTabTitle()` 对 URL 型标题统一投影为 origin+path，避免 query/fragment 从标题泄漏到列表/确认摘要/审计。
- **安全/隐私边界**：不读取浏览历史（`tabs` 只覆盖已打开标签页）；不注入未授权站点；每个子命令入审计（零明文，URL 以去 query 的形式记录）。
...
- **实现位置**：`src/tools/tabs-tools.ts`（工具与纯逻辑：子命令/risk/入参解析/目标解析/去参/单标签页与禁批量）、`src/background/tabs-setting.ts`（开关存储）、`service-worker.ts` `createTabsDeps`（真实 `chrome.tabs` 调用：query/update/move/remove + `describeTarget` + 复用绑定链）、`security/confirm.ts` `describe` 接缝（ask 前补充目标摘要）、`host.ts` `setTabsEnabled`。
- **回归门禁**：`test/tabs-tools.test.ts`（子命令/risk/mute·pin·move·close 入参/二选一/拒绝批量/受限拒绝/去 query/开关/审计零明文）、`test/tabs-wiring.test.ts`（静态接线/权限面/**close 存在 + write→ask + 单标签页 + 不可逆**）、`test/security.test.ts`（`describe` 摘要 enrich 与异常不阻断）、`test/ui/journey.mjs`（面板内 close 确认：不可逆可见 + 拒绝可结算）、`test/ui/binding.mjs`（真实站点自建标签页上真实 `tabs list`/`switch`/`mute`/`pin`/`move`/`close` + 审计断言，含 `close --all` 可读拒绝）。
...
| 2.1 | **FR-049（作者决策③ 2026-09-12）**：新增 `tabs` 权限与插件级标签页管理工具 `tabs`（list/switch/open，**不含 close**；**该「不含 close」约束已于 2.9（2026-09-13 作者裁决反转）作废**）；list 隐私默认去 query/fragment（`--full` 显式）；switch 复用绑定链并切会话；open 仅 http(s)；options 新增隐私开关（关闭即从工具面移除）；补 §12.4 + 更新 §3.1/§12.1 权限纪律；`tabs-tools.test.ts` / `tabs-wiring.test.ts` + `test:binding` 新增真实 `tabs list`/`tabs switch`。 |
...
| 3.5 | **v2 R2 第 2 轮（2026-09-13，真层级树 UI + 档案分层 + binding 覆盖链）**：树抽屉由「分组标题 + 扁平列表」改为**真层级树**（`tree-view.ts` 消费 `ownershipTree` 的嵌套模型 + `tree-drawer.ts` 自建 `role=tree/treeitem` + `aria-expanded`/`aria-level` + 方向键/Enter/Home/End + roving tabindex + 面包屑 + 惰性渲染，**零 `innerHTML`**）；命令行**逐层三态覆盖控件**（`button[data-action-id=set-command-policy][data-policy=allow|ask|deny]`，放宽类经 `#tree-confirm` 二次确认），**硬底线行零控件 + `.tree-clamp-reason` 可读**（R2 deny 分层，取代原「deny 一律无控件」）；档案卡**同款分层**（硬底线卡零 `[data-policy]` + 原因；可覆盖卡三档；`#tree-drawer` 用自有 `.tree-archive-policy*` 类，保持 `.tree-archive` 零 `.tree-control`/零 `button[data-action-id]` 红线）+ **默认档 vs 生效档分列**。① §8.2 回填 `dist/sidepanel.js` **实测 1,159,856 B**（基线 = 1,159,856 B / ceiling = **1,217,848 B**；历史 1,068,165 / 1,085,389 / 1,110,744 / 1,132,748 全保留）+ `content.js` **1,073,453 B（零增长）**；② `test:insight` 断言数 70 → **R2 第 2 轮实测 102**（新增 `#I-20a~k` 作者两例逐层展开/键盘/面包屑/分层/多状态布局守卫 + `#I-21a~e` 档案分层/分列；取代 S13~S16 与 S11）→ **R2 修复轮（A1/A2/A3）再 +6 → 实测 108**（`#I-20a2`/`#I-20d2`/`#I-21b2`/`#I-22a~c`；**以现状为准**，历史 70/102 保留；本行原记「R2 修复轮后实测 102」系第 2 轮时点笔误，已订正为 108）；③ `test:binding` 180 → **192**（追加 `#22a~l` 覆盖链：三档 → 真实 dispatch 生效 → reset → 持久化；既有 #0/#19/#20/#21 零删改）；④ `docs/smoke-checklist.md` 新增 §8 人工面 **V2-H-10~14**（`⏳ 待人工`）。**`policy.ts`/`auto-authorize.ts` sha256 不变 / `manifest.json` 零 diff / `src/content/**` 零改动 / 无新依赖 / 零新权限**。 |
| 3.6 | **v2 R2 收口（2026-09-14，纯文档/状态收口，零 `src/`、零 `test/` 改动）**：R2 全链（spec `a955a7f` → plan `7617687` → tasks `76d0d61` → build `ce60fd1`/`493f9d0`/`e35d852`/`ff32683` → review `5d9e6f1` ⚠️ 有条件通过 0 阻塞 → 修复 `02ee715`/`fa062ab`/`649f746` → **validate `a0d2629` ✅ 通过 0 阻塞**）收口。①**3 处低龄偏差订正/登记**：`docs/smoke-checklist.md` §5 `test:insight` 断言数 **102 → 108**（历史 45/52/70/102 保留，以现状为准，本文件同批订正）；`docs/r2-supersession-ledger.json` `counts.nodeTests` 补**跨口径说明**（`before=646` = 排除正则 `.test(` 静态；`afterR2=690` 同口径；`afterR2Fix=693` = 运行期 `node --test`；门禁下界 = `before`，`currentNodeTestCount()` 取 `\btest(` 含正则现状 776，**数字不改**）；legacy bare `catch` 如实登记为**已知项**（`src/**` 14 处 `.catch(() => {})` best-effort 推送 + v1 `test/ui/journey.mjs:589` 既有 `catch (e) {}`；**非 R2 引入、不改代码、不伪造状态**）。②**测试基线**：插件 **693/693 · 0 fail**、`test:insight` **108**、`test:ui` 167、`test:hardening` 24、`test:binding` **192**、`test:e2e` PASS；`content.js` **1,073,453 B（零增长）**、`sidepanel.js` **1,162,942 B**（ceiling 1,217,848）。③**人工面未执行、如实登记不冒充 PASS**：`V2-H-10~14` + 承前 `V2-H-A~D`/`V2-H-1~6`/`V2-H-7~9` + `H0~H10` + **T1 缺口**（树侧能力撤销成功端到端），全部 `⏳ 未执行（headless 不可合成）`。④**本轮零 `src/`、零 `test/` 改动 → 未跑 Chromium 门禁**（以 `git diff --quiet -- packages/web-cli-plugin/src packages/web-cli-plugin/test` 零 diff 核实，如实标注不冒充通过）；**不合 main、不发布**（`NG-V2-009` 保持）。 |
| 3.7 | **性能根因修复：base LLM SDK 惰性化（2026-09-14，作者显式授权修改 `packages/web-cli-base/**`，仅本修复）**：`packages/web-cli-base/src/llm.ts` 顶层静态 `import OpenAI from 'openai'` / `import Anthropic from '@anthropic-ai/sdk'` → `import type`（编译擦除）+ `chat()` 内按需 `await import(...)`。**同一根因**：base barrel `index.ts:30` 急切 re-export `llm.ts`，任何只要 barrel 中一个小工具的消费方（lgdl-web `bridge.ts:22` 要 `createBrowserEventHub`；插件 content script 要 `createBrowserDomOps`）都被迫急切求值两个重 SDK → ① lgdl-web dev 首屏 Vite 中途发现重依赖触发 optimizeDeps reload；② `content.js` 撑到 1,073,453 B。**公开 API 与行为零变化**（导出名集合运行期 270/270 完全一致；`dist/index.d.ts` 导出 537/537 一致；`dist/llm.d.ts` 逐字节相同；`dist/llm.js` 仅改加载时机）。① §8.1/§8.2 体积复测并**现状订正**：`content.js` **1,073,453 → 177,076 B**（−83.5%，**仍超 64 KiB 目标 2.70×，D31 未消除、如实保留**）、`sidepanel.js` **1,162,942 → 266,500 B**、`options.js` **978,471 → 82,093 B**、`background.js` **1,432,228 → 1,587,839 B**（+10.9%：后台仍真实使用 `chat()`，无 code-splitting 时动态 import 被内联惰性模块，命名空间完整保留故 SDK 未用导出不再 tree-shake——如实登记）；② **新增结构性防回潮门禁**（base `src/llm-lazy-sdk.test.ts`，只增不减）：源码/dist/求值模块图三层断言 + 反证；③ **未改动任何阈值/断言**：`CONTENT_MAX_BYTES`（1,073,453 硬上限）与 `SIDEPANEL_CEILING`（1,217,848）原样保留，`targetMet` 仍按实测 `false`（未达标，不宣布达成）；④ 门禁严格串行全绿（日志落盘）；**dev 首屏改善无法在 headless 量化 → 未实测，仅给作者验证配方**；**不合 main、不发布**。 |
| 3.8 | **守卫收紧轮（2026-09-14，锁死 3.7 的 SDK 惰性化战果；纯 `test/` 门禁 + 文档/状态，零 `src/`）**：3.7 如实登记的残留（`CONTENT_MAX_BYTES` 仍 1,073,453、`SIDEPANEL_CEILING` 仍 1,217,848）意味着体积**可悄悄长回 ~1 MiB 而守卫不响**。本轮按实测值**显式收紧**（方向只准更严；容差与断言零放宽、零删除；前后值 + 测量日期 + 来源命令 + 理由 + 历史保留 + 反证齐备）：① `test/size-baseline.ts` `CONTENT_MAX_BYTES` **1,073,453 → 177,076 B**（无容差；反证 `177,077` 必须 FAIL，旧值 1,073,453 B 亦已 FAIL）；② `SIDEPANEL_BASELINE_BYTES` **1,159,856 → 266,500 B**、`SIDEPANEL_CEILING` **1,217,848 → 279,825 B**（= floor(266,500×1.05)，容差 5% 不变）、`HISTORY` 追加保留 1,159,856 / 1,162,942（连同 1,068,165 / 1,085,389 / 1,110,744 / 1,132,748）、`previousBaselineBytes=1,159,856` / `previousCeilingBytes=1,217,848` / `direction='tightened'`、`targetBudgetBytes/targetMet` 仍 `null`；③ `test/perf-baseline.ts` `CONTENT_BUNDLE_BASELINE_BYTES` **1,073,453 → 177,076 B**（同一产物/同一漏洞面一并收紧；旧值已 FAIL；`targetMet` 仍按实测 `false`，D31 保留未消除）；④ `dist/options.js` **无体积守卫** → 无可收紧项（如实说明）；`dist/background.js` **只记录、无硬断言** → 只登记 +155,611 B 原因与取舍（无 code-splitting 下动态 import 内联惰性命名空间 → SDK 未用导出不再 tree-shake；换得后台仅真正调用时求值）；⑤ 订正文档/状态漂移（**保留历史行 + 以现状为准**）：本文件 §8.1/§8.2 + `build.md §R3P-7/§R3P-12/新增 §R3P-13` + `ROADMAP.md v1.24.0` + v2 父/四叶 `state.json` 追加本轮记录；⑥ `test/insight-no-escalation.test.ts:132` 的 legacy「base 零改动」断言**保留并加注释**登记语义变更（该红线已于 2026-09-14 经作者放行调整、仅限性能修复；现仅用于检测**未提交**的 base 漂移）；⑦ 门禁严格串行全绿（完整日志落盘）：build 0 + 体积 content 177,076 / sidepanel 266,500 / options 82,093 / background 1,587,839；插件 `npm test` **696/696 · 0 fail**（693 + 3 条新反证）、`test:insight` 108、`test:ui` 167、`test:hardening` 24、`test:binding` 192、`test:e2e` PASS、base **490/490**、全仓 **1686 tests / 1685 pass / 0 fail / 1 skip**；**本轮 base 零改动**（`git diff --quiet -- packages/web-cli-base` 零 diff）、**不合 main、不发布**。 |
