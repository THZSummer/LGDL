...
### 8.2 实测（2026-09-12 初测；2026-09-13 复核订正体积）

| 对象 | 实测 | 阈值 | 判定 |
|------|------|------|:--:|
| `dist/content.js` | **1,073,453 B**（≈1,048.3 KiB ≈1.02 MiB）〔初测（2026-09-12）**33.9 KB**（34,711 B）；后因 base DOM 值导入 + 内联 LLM SDK 增至 1.0MB 级〕 | ≤ 64 KB（**目标预算**） | ❌ **未达成**（超 ≈16.4×，D31） |
| `dist/sidepanel.js` | **1,110,744 B**（≈1.06 MiB）〔初测 **15.1 KB**；v1 时点 1,068,165 B；V2-2 时点 1,085,389 B；**2026-09-13 v2 P0 收口回填实测 1,110,744 B**（来源 `stat -c %s packages/web-cli-plugin/dist/sidepanel.js`；`test/size-baseline.ts` 基线 = 1,110,744 B / ceiling = 1,166,281 B；**以现状为准**，历史值保留）〕 | — | 记录 |
| `dist/background.js` | **1,372,225 B**（≈1.31 MiB）〔初测 **968,442 B**（≈945.7 KB）〕 | ≤ 1.2 MB | ⚠️ 超目标（仅记录，无硬断言） |
