...
见机读文件 `knownLimitations`（**5** 条）：过渡形态的卡口径（`#l0-decision` 不计为卡，v4-2 落 7 主类卡后重审 —— 该重审已作为 **TASK-613** 移交 v4-2，见 v4-1 收口轮 N-03）/ `empty` 档 chars 视口差 / 风险格对夹具序敏感 / **〖N-01（收口轮）〗口径双轨期说明**（产品口径 `DENSITY_MEASURE_SOURCE` 与未排除口径 `c1Probe` 的各自正当用途）/ **〖N-02（收口轮）〗豁免守卫是双层防线**（guard 即时拦带标记控件 + 阶段 F 基线兜底未标记控件，两层均已实测）。

> 〖收口轮（2026-09-19）〗本 `.md` 与机读文件同步：31 格实测值 / 阈值 `7/15 · 9/20 · 17/35` / 几何下界 488px / 体积 385,319 · 404,584 **逐字不变**（零放宽）；仅 §7 的 `knownLimitations` 由 3 条增至 5 条。
