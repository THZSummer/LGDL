...
|---|-------------|------|---------|:----:|:----:|:--:|
| 1 | `web-fetch` | 基础 web 获取 | **插件侧受控 seam** `src/tools/web-fetch-tool.ts` **替换** base 同名内建（`host.ts` 传 `builtins:['sleep','web-cli-help']` + `webFetch` 依赖）：仅**同源相对路径**（解析到绑定 origin）与**已授予 host 权限**的 http(s) 域名可读；未授权域名**零请求 + 可读拒绝 + 授权指引**（FR-050/EC-023）；同源优先走页面上下文（content script 同源 fetch）。未传 seam 的宿主（node 单测）仍回退 base 内建 | 对齐（边界显式） | P0 | 否 |
...
>
> **FR-050 边界修正（2026-09-12）**：第 1 行原写 `web-fetch`「随 base 默认内建下发」——真实扩展里已在插件侧**替换为受控 seam**（预校验 host 权限，未授权域名零请求），并修正该工具的**真实可用边界**：仅同源/已授权域名可用，任意站点不可用。详见 `docs/dev.md` §10.9。
