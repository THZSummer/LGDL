...
> 作者 2026-09-12 决策③：**同意新增 `tabs` 权限**（接受安装警告），用于插件级标签页管理工具 `tabs`。这是相对上一分发包**唯一**的权限变化。**2026-09-13 作者裁决反转**：`tabs` 放开 `close` 并补齐 `mute`/`pin`/`move`——**未再新增任何权限**（`chrome.tabs.remove/update/move` 在既有 `tabs` 权限下可用），权限面与本文档 §5 表格不变。
...
| 差异 | 说明 |
|------|------|
| 新增助手工具 `tabs` | `list`（列出标签页，默认仅 origin+path）/ `switch`（切页并切会话）/ `open`（打开新 http(s) 标签页）/ `mute` / `pin` / `move` / `close`（关闭单个标签页，`write`→ask，摘要含标题+去参 URL+不可逆，禁止批量） |
...
| 1.1 | FR-049（作者决策③ 2026-09-12）：新增 `tabs` 权限与插件级标签页管理工具（list/switch/open，不含 close；**该「不含 close」约束已于 2026-09-13 作者裁决反转作废——见下条**）；补 §5 安装警告变化（「读取您的浏览记录」）与用户可感知差异；§2 权限面同步。 |
