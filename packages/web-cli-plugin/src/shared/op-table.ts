...
/**
 * The nine rows, compactly: `[id, layer, fail, audit]` in `OP_SPECS` order. `mode` is
 * constant across the batch (see the module note), so the mapper attaches it rather
 * than repeating it in nine tuples. (Comments inside a literal survive bundling and
 * cost bytes; the row-by-row table lives in the module JSDoc above.)
 */
const OP_ROWS: readonly (readonly [string, OpLayer, OpFail, boolean])[] = Object.freeze([
  ['op.turn', 'panel', 'card-boundary', false],
  ['op.pick', 'panel', 'card-boundary', false],
  ['op.describe', 'panel', 'card-boundary', false],
  ['op.authorize', 'sw', 'snapshot-rollback', true],
  ['op.rebind', 'panel', 'card-boundary', false],
  ['op.help', 'panel', 'card-boundary', false],
  ['op.llm-config', 'panel', 'snapshot-rollback', true],
  ['op.perm.request', 'sw', 'snapshot-rollback', true],
  ['op.revoke', 'panel', 'snapshot-rollback', true],
]);
...
export const OP_DESCRIPTORS: readonly OpDescriptor[] = Object.freeze(
  OP_ROWS.map(([id, layer, fail, audit]) => Object.freeze({ id, layer, mode: 'waterfall' as OpMode, fail, audit })),
);
