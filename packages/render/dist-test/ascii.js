...
function drawBox(g, x, y, w, h, rounded = false) {
 ...
 */
export function renderAscii(doc, layout) {
    void layout;
    // ---- BFS ranks ----
    ...
    for (const n of doc.nodes)
        ranks[rankOf.get(n.id) ?? 0].push(n.id);
    // ---- box building ----
    const boxLines = (id) => {
    ...
    for (let ri = 0; ri < ranks.length; ri++) {
        ...
        if (ri < ranks.length - 1) {
            ...
            for (const e of downEdges) {
                if (!targetsBySource.has(e.from))
                    targetsBySource.set(e.from, []);
...
function drawDiamond(g, x, y, w, h, labelText) {
