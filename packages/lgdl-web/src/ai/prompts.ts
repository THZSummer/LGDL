 ...
 *    提供工具分工、做事流程、常见陷阱
 * 2. 实操（--help / web-cli-help）＝战术：命令用法一律 --help 按需查询（lgdl-web-cli <cmd>
 *    --help 等，自文档化，不依赖反复读文档）；工具清单经 web-cli-help 动态发现，不写死枚举
 */
...
- **执行**：调用工具（function calling）。工具集见本会话 function 列表（模型天然可见）；一键了解全部工具用 \`web-cli-help\`（列出所有工具 + 顶层命令），某工具命令用 \`<tool> --help\` 或 \`web-cli-help <tool>\`
...
《使用指南》一节——先读它，了解工具体系（web-cli-help / README-CLI）、做事流程与常见陷阱（战略层）。
...
- 不确定命令或参数时，**先 \`--help\`**：
  ...
  - CLI 文本：\`lgdl-web-cli <cmd> --help\`（顶层 \`lgdl-web-cli --help\` 列全部子命令）、
    \`lgdl-web-op-cli --help\`；其余工具用法一律各工具 \`--help\`（或 \`web-cli-help <tool>\` 查单工具）
