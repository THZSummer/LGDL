...
import { parseWebCliCommand, parseWebCliBatch } from './protocol.js';
import { formatStatus, parseLgdl } from '@lgdl/lgdl-core';
...
test('parseWebCliCommand: fetch-doc is NOT a web-cli subcommand (moved to web-fetch)', () => {
  const r = parseWebCliCommand('lgdl-web-cli fetch-doc --doc main --path x');
  assert.equal(r.kind, 'error');
  if (r.kind === 'error') assert.match(r.message, /web-fetch/);
});
