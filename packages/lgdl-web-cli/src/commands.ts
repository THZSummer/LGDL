/**
 * 命令定义注册表 —— lgdl-cli 与 lgdl-web-cli 共享的业务逻辑层（LGDL 面，F-13 ② 自 base 迁出）。
 *
 * 两端各自只做「输入适配」：
 *   - lgdl-cli（packages/lgdl-cli）：commander 解析 argv → 调 buildOperation
 *   - lgdl-web-cli（packages/lgdl-web-cli）：文本解析 → 调 buildOperation
 *
 * 本文件统一：参数必填校验、no-change 校验、op 构造、attrs/member 解析——
 * 业务逻辑只写一次，两端行为严格一致。
 *
 * （自 @lgdl/lgdl-core commands.ts → @lgdl/web-cli-base commands.ts 迁入，
 * F-13 ② 随 LGDL 面迁回业务包；defaultKindFor 注入化为 buildOperation 第 4 参
 * kindResolver，内置默认 = 现状行为逐字节。CommandSpec/KindResolver 机制类型
 * 自 @lgdl/web-cli-base 导入（机制留 base）。）
 */
import type { LgdlOperation, LgdlMember, LgdlAttrs } from '@lgdl/lgdl-core';
import type { CommandSpec, KindResolver } from '@lgdl/web-cli-base';

/** 一个命令的参数（--file/--doc 由两端各自处理，不在此列）。 */
export type { CommandSpec, KindResolver } from '@lgdl/web-cli-base';
