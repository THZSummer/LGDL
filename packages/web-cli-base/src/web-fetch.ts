 ...
 * 的原始文本。文本格式：`web-fetch --path <path>`（如 lgdl/web/workbench/README-CLI.md）。
 *
...
export function parseWebFetchCommand(line: string): ParsedWebFetch {
  ...
  if (tokens[0] !== 'web-fetch') {
    return {
      ok: false,
      error: `缺少前缀 "web-fetch"（独立基础工具：web-fetch --path <path>，如 lgdl/web/workbench/README-CLI.md）`,
    };
  }
  ...
  try {
    ...
    if (!path) {
      return { ok: false, error: '缺少必填参数 --path <path>（web-fetch 必须显式传 path，无默认文档；如 --path lgdl/web/workbench/README-CLI.md；--help 查看用法）' };
    }
...
export async function executeWebFetch(path: string): Promise<{
  ...
  const lines: string[] = [];
  if (!path) {
    lines.push('✖ web-fetch 缺少必填参数 --path：调用时必须显式传 --path（无默认文档）。正确示例：web-fetch --path lgdl/web/workbench/README-CLI.md');
