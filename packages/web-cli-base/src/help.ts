 ...
 * （webOpHelp / webFetchHelp 留 web 侧——对应工具 lgdl-web-op-cli / lgdl-web-fetch
 * 定义在 web，ADR-007。）
 *
 * 增量命令（add-node 等 9 个）的参数规格复用本包 commands.ts 的 COMMANDS
 * 注册表（required/optional/changeKeys），此处只补充中文说明与示例——单一数据源
 * （R-009：COMMANDS 来源切换为新包注册表，EC-010）。
 *
 * （自 packages/web/src/ai/help.ts:13-209 webCliHelp 面迁入，纯位置迁移零语义改动。）
 */
