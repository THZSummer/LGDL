...
## 逐图修复记录（第二轮：一图一轮，doubao 复验）

| 图 | 处理 | 复验 |
|---|---|---|
| mindmap | 分支配色（橙/蓝/绿）+ 字号层级（17/14/12）| ✅ 配色+层级确认；三方向布局为 3 分支固有，接受 |
| sequence | 激活框（参与者消息区间竖条）+ 返回虚线 | ✅ 全部符合规范 |
| uml-class | **卡片高度随成员数**（原固定 60px 溢出）+ 类名加粗 | ✅ 无溢出、栏位正确 |
| architecture | 间距不均 = dagre cluster 固有，接受 | - |
| datastream | 泳道垂直居中 + **箭头可见性修复**（边终点到目标边界）| ✅ 箭头清晰、泳道平衡 |
| er | 基数拆分（1 靠源、* 靠目标）| ✅ 位置正确 |
| state | 初始伪状态（实心圆+箭头）| ✅ 已显示；迁移标签轻微粘连记低优 |
| gantt | 任务名固定左列 + 窄条文字外置 | ✅ 三项全过 |
| microservices | 分组四色 + 聚合边标签 11px 白底；缩放模糊为缩略图固有 | ✅ 配色确认 |

**共性修复**（所有图受益）：center-to-center 边终点缩到目标边界（箭头不再被节点盖住）—— mindmap/swimlane/grid。
